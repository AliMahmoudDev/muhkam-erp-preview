...
```
muhkampro.com/             → MUHKAM ULTIMATE  (نظام كامل)
muhkampro.com/muhkam-advanced/ → MUHKAM ADVANCED  (نظام مبسط)
muhkampro.com/api/         → Express API      (مشترك بين النسختين)
```
...
NODE_ENV=production
PORT=3000
ALLOWED_ORIGINS=https://muhkampro.com
```
...
```bash
# انسخ الـ config
sudo cp /var/www/muhkam-erp/deploy/nginx.conf /etc/nginx/sites-available/muhkampro

# عدّل اسم النطاق (استبدل muhkampro.com بنطاقك الفعلي)
sudo nano /etc/nginx/sites-available/muhkampro
...
sudo certbot --nginx -d muhkampro.com -d www.muhkampro.com
```
...
# تحقق من ADVANCED
curl -I https://muhkampro.com/muhkam-advanced/
# يجب أن يرجع: HTTP/2 200

# تحقق من API
curl https://muhkampro.com/api/healthz
...
| `ALLOWED_ORIGINS` | مستحسن | `https://muhkampro.com` |
...
1. ادخل على `https://muhkampro.com` بحساب **superadmin**
...
**الرابط الموحّد للعملاء:** يمكنك إعطاء جميع العملاء رابطاً واحداً: `https://muhkampro.com`
