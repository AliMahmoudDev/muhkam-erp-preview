...
import { Clock, Plus, X, Sun, AlarmClock, LogIn, LogOut, Pencil, Trash2, Timer, Settings, Calculator, CheckCircle2, AlertCircle } from "lucide-react";
...
export default function Attendance() {
  ...
  const dedSettings = useQuery({ queryKey: ["att-ded-settings"], queryFn: () => f("/api/attendance-deductions/settings") });
  const dedTiers    = useQuery({ queryKey: ["att-ded-tiers"],    queryFn: () => f("/api/attendance-deductions/tiers") });
    ...
    <div className="p-4 space-y-4" dir="rtl">
      ...
      <div className="flex items-center justify-between flex-wrap gap-3">
        ...
        {canManage && (
          <div className="flex gap-2 flex-wrap">
            ...
            <button onClick={() => setShowDedCalc(true)} className="erp-btn erp-btn-secondary flex items-center gap-1 text-sm" title="احتساب خصومات الشهر تلقائياً">
              <Calculator size={14} /> احتساب خصومات
            </button>
            <button onClick={() => setShowDedSettings(true)} className="erp-btn erp-btn-ghost flex items-center gap-1 text-sm" title="إعدادات شرائح الخصم">
      ...
      {showDedCalc && canManage && (
        <DeductionCalcModal
          employees={empList}
          today={today}
          onClose={() => { setShowDedCalc(false); previewDed.reset(); }}
          ...
          onApply={(items) => applyDed.mutate(items, {
            onSuccess: () => { setShowDedCalc(false); previewDed.reset(); qc.invalidateQueries({ queryKey: ["employee-deductions"] }); },
          })}
...
function DeductionSettingsModal({
  ...
  const [weeklyOff, setWeeklyOff] = useState<string[]>(
    String(settings?.["weekly_off_days"] ?? "5").split(",").filter(Boolean),
  );
  ...
  const addTier = (applies_to: "late" | "early") =>
    setEditTiers(p => [...p, { applies_to, min_minutes: 0, max_minutes: null, amount: 0, is_active: true }]);
  const updTier = (idx: number, k: string, v: unknown) =>
    setEditTiers(p => p.map((t, i) => i === idx ? { ...t, [k]: v } : t));
  const delTier = (idx: number) =>
    setEditTiers(p => p.filter((_, i) => i !== idx));
  ...
  const renderTierRow = (t: AnyRec, idx: number) => (
    <div key={idx} className="grid grid-cols-[1fr_1fr_1fr_auto_auto] gap-2 items-center">
      <input type="number" min="0" className="erp-input text-xs" placeholder="من (دقيقة)"
        value={String(t["min_minutes"] ?? "")}
        onChange={e => updTier(editTiers.indexOf(t), "min_minutes", e.target.value === "" ? 0 : Number(e.target.value))} />
      <input type="number" min="0" className="erp-input text-xs" placeholder="إلى (فارغ = مفتوح)"
        ...
        <div className="p-5 space-y-5 max-h-[75vh] overflow-y-auto">
          ...
          <section className="space-y-3">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
              <Field label="فترة السماح (دقائق) — تأخير مسموح بدون خصم">
                <input type="number" min="0" className="erp-input w-full" value={grace}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
              <Field label="خصم يوم غياب كامل">
                <input type="number" min="0" step="0.01" className="erp-input w-full" value={absFull}
                  onChange={e => setAbsFull(e.target.value)} placeholder="0 = لا يحتسب تلقائياً" />
              </Field>
              <Field label="خصم نصف يوم غياب">
                <input type="number" min="0" step="0.01" className="erp-input w-full" value={absHalf}
                  onChange={e => setAbsHalf(e.target.value)} placeholder="0 = لا يحتسب تلقائياً" />
              </Field>
            </div>
          ...
          <section className="space-y-3">
            ...
            <div className="text-[11px] text-white/40">
              مثال: من 1 إلى 15 دقيقة → 25 جنيه. اترك الحد الأعلى فارغاً للشريحة المفتوحة (مثلاً 60+ دقيقة).
            </div>
            ...
            <button onClick={() => addTier("late")} className="erp-btn erp-btn-ghost text-xs flex items-center gap-1">
            ...
            {applyEarly && (
              <>
                ...
                <button onClick={() => addTier("early")} className="erp-btn erp-btn-ghost text-xs flex items-center gap-1">
...
function DeductionCalcModal({
  employees, today, onClose, onPreview, previewData, previewing, onApply, applying,
}: {
  employees: AnyRec[];
  today: string;
  onClose: () => void;
      ...
      <div className="erp-modal rounded-xl max-w-5xl w-full my-4">
        ...
        <div className="p-5 space-y-4 max-h-[75vh] overflow-y-auto">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-3 items-end">
            <Field label="الشهر">
              <input type="month" className="erp-input w-full" value={month}
            ...
            <button
              onClick={() => { setExcluded(new Set()); onPreview(month, empId ? Number(empId) : null); }}
              disabled={previewing}
          ...
          {previewData && (
            <div className="grid grid-cols-2 md:grid-cols-4 gap-2 p-3 bg-white/5 rounded-lg border border-white/10">
              <div>
                ...
                <div className="text-lg font-bold text-white">{Number(summary["total_items"] ?? 0)}</div>
              </div>
              <div>
                <div className="text-[10px] text-white/40">جديدة</div>
                <div className="text-lg font-bold text-emerald-300">{Number(summary["new_items"] ?? 0)}</div>
              </div>
              <div>
                <div className="text-[10px] text-white/40">سبق احتسابها</div>
                <div className="text-lg font-bold text-white/40">{Number(summary["already_applied"] ?? 0)}</div>
              </div>
              <div>
                    ...
                    return (
                      <tr key={k} className={`border-t border-white/5 ${applied ? "opacity-40" : ""}`}>
                        <td className="p-2 text-center">
                          {applied ? (
                            <span title="سبق احتسابه"><CheckCircle2 size={14} className="text-emerald-400 inline" /></span>
                          ) : (
                        ...
                        <td className="p-2">
                          ...
                          {it["employee_code"] ? <div className="text-[10px] text-white/40">{String(it["employee_code"])}</div> : null}
                        </td>
