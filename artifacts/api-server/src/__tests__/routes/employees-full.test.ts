...
// selfEmployeeId returns the user's employee_id, or -1 if not an employee
vi.mock('../../lib/employee-self', () => ({
  selfEmployeeId: vi.fn().mockReturnValue(-1),
}));
...
// salesperson role explicitly has can_view_employees:false and can_manage_employees:false
const limitedUser: AuthUser = {
  id: 4, name: 'Salesperson', username: 'salesperson',
  role: 'salesperson', permissions: '{}', active: true,
...
// ── Employee fixture ──────────────────────────────────────────────────────────
const mockEmployee = {
  id: 10, company_id: 1, employee_code: 'EMP0001',
  first_name_ar: 'أحمد', last_name_ar: 'محمد',
  first_name_en: 'Ahmed', last_name_en: 'Mohammed',
  email: 'ahmed@test.com', phone: null, personal_phone: null,
  national_id: null, national_id_image: null,
  job_title_id: null, department_id: null, branch_id: null,
  hire_date: '2024-01-01', employment_status: 'active',
  salary: '5000', currency: 'EGP', salary_type: 'fixed',
  commission_rate: null, commission_basis: null,
  commission_scope_dept_id: null, bank_account: null,
  address_ar: null, address_en: null, city: null, country: 'مصر',
  notes: null, created_by: 1, updated_by: 1, deleted_at: null,
...
describe('GET /api/employees', () => {
  ...
  it('يجب أن يرجع 200 ومصفوفة الموظفين', async () => {
  ...
  it('يجب أن يرجع 403 للمستخدم بدون company_id (requireTenant يقاطع الطلب)', async () => {
    vi.mocked(authenticate).mockImplementationOnce(
      (req: Request, _res: Response, next: NextFunction) => {
  ...
  it('يجب أن يعزل بيانات الشركة — company B لا ترى موظفي company A', async () => {
    mockChainData.mockResolvedValueOnce([mockEmployee]);
...
describe('POST /api/employees', () => {
  ...
  it('يجب أن يرجع 400 عند إرسال body ناقص (بدون الحقول المطلوبة)', async () => {
    const request = (await import('supertest')).default;
    const app = (await import('../../app')).default;
  ...
  it('يجب أن يرفض المستخدم بدون company_id بـ 403 (requireTenant يقاطع الطلب)', async () => {
    vi.mocked(authenticate).mockImplementationOnce(
      (req: Request, _res: Response, next: NextFunction) => {
        (req as Request & { user: AuthUser }).user = noCompanyUser;
        next();
    ...
    const res = await request(app)
      .post('/api/employees')
      .set('Authorization', 'Bearer test-token')
      .send({ first_name_ar: 'أحمد', last_name_ar: 'محمد', hire_date: '2024-01-01', salary: 5000 });

    expect(res.status).toBe(403);
    expect(res.body).toHaveProperty('error');
  });

  it('يجب أن يرجع 201 عند إنشاء موظف بنجاح', async () => {
    dbMock.returning.mockResolvedValueOnce([mockEmployee]);
    ...
    const res = await request(app)
      ...
      .send({
        first_name_ar: 'أحمد', last_name_ar: 'محمد',
        hire_date: '2024-01-01', salary: 5000,
      });

    expect(res.status).toBe(201);
    expect(res.body).toHaveProperty('id', 10);
    expect(res.body).toHaveProperty('employee_code', 'EMP0001');
...
describe('GET /api/departments', () => {
  ...
  it('يجب أن يرجع 200 ومصفوفة الأقسام', async () => {
    const request = (await import('supertest')).default;
    const app = (await import('../../app')).default;
...
describe('GET /api/job-titles', () => {
  ...
  it('يجب أن يرجع 200 ومصفوفة المسميات الوظيفية', async () => {
    const request = (await import('supertest')).default;
    const app = (await import('../../app')).default;
