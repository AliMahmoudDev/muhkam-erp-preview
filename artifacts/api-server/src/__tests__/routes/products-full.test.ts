...
// noCompanyUser → requireTenant → 403 (before any hasPermission check)
...
// ── Product fixture ───────────────────────────────────────────────────────────
// Fields must satisfy formatProduct() + GetProductsResponse schema
const mockProduct = {
  id: 1,
  name: 'جهاز آيفون',
  sku: 'IP-001',
...
// ─────────────────────────────────────────────────────────────────────────────
// products.ts has NO route-level authenticate → ONE authenticate call per request.
// categories.ts has router.use(authenticate) → TWO authenticate calls per request.
// ─────────────────────────────────────────────────────────────────────────────
...
describe('GET /api/products', () => {
  ...
  it('يجب أن يرجع 200 ومصفوفة المنتجات', async () => {
  ...
  it('يجب أن يعزل بيانات الشركة — company B لا ترى منتجات company A', async () => {
    // Company A has one product; chain returns it on the first call
    mockChainData.mockResolvedValueOnce([mockProduct]);

    const request = (await import('supertest')).default;
    const app = (await import('../../app')).default;

    // Request A — default authenticate → adminUserA (company 1)
    ...
    // Request B — adminUserB (company 2): chain falls back to [] (default)
    vi.mocked(authenticate).mockImplementationOnce(
      (req: Request, _res: Response, next: NextFunction) => {
...
describe('POST /api/products', () => {
  ...
  it('يجب أن يرجع 403 للمستخدم بدون company_id (requireTenant يقاطع)', async () => {
    ...
    const res = await request(app)
      .post('/api/products')
      .set('Authorization', 'Bearer test-token')
      .send({ name: 'منتج', cost_price: 100, sale_price: 120, quantity: 0 });

    expect(res.status).toBe(403);
  });

  it('يجب أن يرجع 201 عند إنشاء منتج بنجاح (quantity=0، بدون حركة مخزون)', async () => {
    // quantity=0 → no stock movement, no category → single insert call
    dbMock.returning.mockResolvedValueOnce([mockProduct]);
    ...
    const res = await request(app)
      .post('/api/products')
      .set('Authorization', 'Bearer test-token')
      .send({ name: 'جهاز آيفون', cost_price: 1000, sale_price: 1200, quantity: 0 });

    expect(res.status).toBe(201);
    expect(res.body).toHaveProperty('id', 1);
    expect(res.body).toHaveProperty('name', 'جهاز آيفون');
  });
});

// ─────────────────────────────────────────────────────────────────────────────
// GET /api/categories — categories.ts mounts router.use(authenticate) → TWO
// authenticate calls (global + router-level). For 401: ONE Once entry suffices
// because the global rejection short-circuits before the router auth runs.
// ─────────────────────────────────────────────────────────────────────────────
...
describe('GET /api/categories', () => {
  ...
  it('يجب أن يرجع 200 ومصفوفة التصنيفات', async () => {
    const request = (await import('supertest')).default;
    const app = (await import('../../app')).default;
  ...
  it('يجب أن يرفض الطلب بدون token بـ 401', async () => {
    // ONE Once entry: global rejects → categories router auth never runs
    vi.mocked(authenticate).mockImplementationOnce(
      (_req: Request, res: Response) => {
        res.status(401).json({ error: 'غير مصرح' });
      },
