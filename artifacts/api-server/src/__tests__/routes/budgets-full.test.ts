...
// ── Budget fixture ────────────────────────────────────────────────────────────
const mockBudget = {
  id: 1, name: 'ميزانية 2024', fiscal_year: 2024,
  date_from: '2024-01-01', date_to: '2024-12-31',
  notes: null, status: 'draft', company_id: 1,
...
describe('GET /api/budgets', () => {
  ...
  it('يجب أن يرجع 200 ومصفوفة الميزانيات', async () => {
...
describe('POST /api/budgets', () => {
  ...
  it('يجب أن يرجع 400 عند إرسال body بدون الحقول الأساسية', async () => {
    const request = (await import('supertest')).default;
    const app = (await import('../../app')).default;
  ...
  it('يجب أن يرجع 403 للمستخدم بدون company_id (requireTenant)', async () => {
    ...
    const res = await request(app)
      ...
      .set('Authorization', 'Bearer test-token')
      .send({
        name: 'ميزانية 2024', fiscal_year: 2024,
        date_from: '2024-01-01', date_to: '2024-12-31',
      });
  ...
  it('يجب أن يرجع 201 عند إنشاء ميزانية بنجاح', async () => {
    dbMock.returning.mockResolvedValueOnce([mockBudget]);
    ...
    expect(res.body).toHaveProperty('name', 'ميزانية 2024');
