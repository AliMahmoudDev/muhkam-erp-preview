...
const approvedTransfer = { ...mockTransfer, status: 'approved', approved_by: 1, approved_at: new Date() };
const shippedTransfer  = { ...mockTransfer, status: 'shipped',  shipped_by: 1, shipped_at: new Date() };
const receivedTransfer = { ...mockTransfer, status: 'received', received_by: 1, received_at: new Date() };
...
describe('POST /api/transfers/request', () => {
  ...
  it('يجب أن يرجع 400 عند عدم كفاية المخزون في فرع الإرسال', async () => {
    ...
    (db as unknown as { execute: ReturnType<typeof vi.fn> }).execute
      .mockResolvedValueOnce({ rows: [{ stock: '2' }] });
...
describe('POST /api/transfers/confirm/:id', () => {
  ...
  it('يجب أن يؤكد الاستلام برمز التحقق الصحيح ويرجع 200', async () => {
    ...
    mockChainData
      .mockResolvedValueOnce([shippedWithCode])
