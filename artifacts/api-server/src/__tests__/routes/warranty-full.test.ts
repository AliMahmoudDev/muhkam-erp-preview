...
// ── Warranty record fixture ───────────────────────────────────────────────────
const mockWarranty = {
  id: 1, sale_id: null, product_id: 1,
  product_name: 'iPhone 14', customer_id: null,
  customer_name: 'عميل اختبار', customer_phone: null,
  serial_number: null, device_model: null, warranty_months: 12,
  warranty_start: '2024-01-01', warranty_end: '2025-01-01',
  status: 'active', notes: null, company_id: 1,
  created_at: new Date('2024-01-01T10:00:00.000Z'),
};

// ─────────────────────────────────────────────────────────────────────────────

describe('GET /api/warranty', () => {
  beforeEach(() => {
  ...
  it('يجب أن يرجع 200 ومصفوفة سجلات الضمان', async () => {
  ...
  it('يجب أن يرفض الطلب بدون token بـ 401', async () => {
    ...
    const res = await request(app).get('/api/warranty');

    expect(res.status).toBe(401);
  });

  it('يجب أن يرفض المستخدم بدون company_id بـ 403 (requireTenant)', async () => {
...
describe('POST /api/warranty', () => {
  ...
  it('يجب أن يرجع 400 عند غياب اسم المنتج', async () => {
    const request = (await import('supertest')).default;
    const app = (await import('../../app')).default;
    ...
    const res = await request(app)
      .post('/api/warranty')
      .set('Authorization', 'Bearer test-token')
      .send({ warranty_start: '2024-01-01' });
  ...
  it('يجب أن يرفض المستخدم بدون company_id بـ 403 (requireTenant)', async () => {
    ...
    const res = await request(app)
      .post('/api/warranty')
      .set('Authorization', 'Bearer test-token')
      .send({ product_name: 'iPhone 14', warranty_start: '2024-01-01' });

    expect(res.status).toBe(403);
  });

  it('يجب أن يرجع 201 عند إنشاء سجل ضمان بنجاح', async () => {
    // Seed db.returning to return the warranty record
    dbMock.returning.mockResolvedValueOnce([mockWarranty]);
    ...
    expect(res.body).toHaveProperty('product_name', 'iPhone 14');
