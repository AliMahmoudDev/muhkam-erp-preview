...
vi.mock('@workspace/db', () => {
  ...
  return {
    ...
    receiptVouchersSchema:             {} as Record<string, never>,
...
describe('GET /api/receipt-vouchers', () => {
  beforeEach(() => {
  ...
  it('يجب أن يرجع 200 ومصفوفة سندات القبض', async () => {
  ...
  it('يجب أن يرفض الطلب بدون token بـ 401', async () => {
    ...
    const res = await request(app).get('/api/receipt-vouchers');
...
describe('POST /api/receipt-vouchers', () => {
  beforeEach(() => {
    mockChainData.mockResolvedValue([]);
    vi.mocked(authenticate).mockImplementation(
      (req: Request, _res: Response, next: NextFunction) => {
  ...
  it('يجب أن يرجع 400 عند إرسال body ناقص (بدون customer_name أو amount)', async () => {
    const request = (await import('supertest')).default;
    const app = (await import('../../app')).default;
  ...
  it('يجب أن يرفض المستخدم بدون صلاحية can_add_receipt_voucher بـ 403', async () => {
    vi.mocked(authenticate).mockImplementationOnce(
      (req: Request, _res: Response, next: NextFunction) => {
  ...
  it('يجب أن يرجع 201 عند إنشاء سند قبض بنجاح', async () => {
...
describe('GET /api/payment-vouchers', () => {
  ...
  it('يجب أن يرجع 200 ومصفوفة سندات الصرف', async () => {
    const request = (await import('supertest')).default;
    const app = (await import('../../app')).default;
...
describe('POST /api/payment-vouchers', () => {
  ...
  it('يجب أن يرفض المستخدم بدون صلاحية can_add_payment_voucher بـ 403', async () => {
    vi.mocked(authenticate).mockImplementationOnce(
      (req: Request, _res: Response, next: NextFunction) => {
        (req as Request & { user: AuthUser }).user = employeeUser;
        next();
    ...
    const res = await request(app)
      .post('/api/payment-vouchers')
      .set('Authorization', 'Bearer test-token')
      .send({ customer_name: 'عميل', safe_id: 1, amount: 300 });

    expect(res.status).toBe(403);
    expect(res.body).toHaveProperty('error');
  });

  it('يجب أن يرجع 201 عند إنشاء سند صرف بنجاح', async () => {
