...
const mockBankAccount = {
  id: 1, name: 'حساب بنكي رئيسي', account_number: '1234567890',
  bank_name: 'البنك الأهلي', currency: 'EGP',
  safe_id: null, opening_balance: '0.00', is_active: true,
  company_id: 1,
...
describe('GET /api/bank-accounts', () => {
  ...
  it('يجب أن يرجع 200 ومصفوفة الحسابات البنكية', async () => {
...
describe('POST /api/bank-accounts', () => {
  ...
  it('يجب أن يرجع 400 عند غياب اسم الحساب أو اسم البنك', async () => {
    const request = (await import('supertest')).default;
    const app = (await import('../../app')).default;
  ...
  it('يجب أن يرجع 403 للمستخدم بدون company_id (requireTenant)', async () => {
    ...
    const res = await request(app)
      .post('/api/bank-accounts')
      .set('Authorization', 'Bearer test-token')
      .send({ name: 'حساب اختبار', bank_name: 'بنك الاختبار' });

    expect(res.status).toBe(403);
  });

  it('يجب أن يرجع 201 عند إنشاء حساب بنكي بنجاح', async () => {
    // Seed db.returning to return the bank account
    dbMock.returning.mockResolvedValueOnce([mockBankAccount]);
    ...
    expect(res.body).toHaveProperty('name', 'حساب بنكي رئيسي');
    expect(res.body).toHaveProperty('bank_name', 'البنك الأهلي');
