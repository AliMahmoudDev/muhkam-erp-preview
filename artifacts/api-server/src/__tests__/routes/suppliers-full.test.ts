...
// ─────────────────────────────────────────────────────────────────────────────
// suppliers.ts has NO route-level authenticate → ONE authenticate call per request.
// GET /suppliers: chain → then → [] (default) → 200 with empty array.
// POST /suppliers flow:
//   1. hasPermission check (admin has can_manage_customers → passes)
//   2. Zod parse (name required)
//   3. chain (getNextCustomerCode) → [] → newCode = 1001
//   4. db.insert(customersTable).values().returning() → [mockSupplier]
//   5. getOrCreateCustomerAccount() → { id: 21 } (mocked)
//   6. res.status(201).json(supplier)
// ─────────────────────────────────────────────────────────────────────────────
...
describe('GET /api/suppliers', () => {
  ...
  it('يجب أن يرجع 200 ومصفوفة الموردين', async () => {
  ...
  it('يجب أن يعزل بيانات الشركة — company B لا ترى موردي company A', async () => {
    // Company A has one supplier
    mockChainData.mockResolvedValueOnce([mockSupplier]);
    ...
    expect(resA.body[0]).toHaveProperty('id', 10);

    // Company B — default chain returns []
    vi.mocked(authenticate).mockImplementationOnce(
      (req: Request, _res: Response, next: NextFunction) => {
...
describe('POST /api/suppliers', () => {
  ...
  it('يجب أن يرجع 201 عند إنشاء مورد بنجاح', async () => {
    // Chain for getNextCustomerCode → [] → newCode=1001
    // insert → returning[0] = mockSupplier
    // getOrCreateCustomerAccount → { id: 21 } (mocked)
    dbMock.returning.mockResolvedValueOnce([mockSupplier]);
    ...
    const res = await request(app)
      .post('/api/suppliers')
      .set('Authorization', 'Bearer test-token')
      .send({ name: 'مورد التجاري' });

    expect(res.status).toBe(201);
    expect(res.body).toHaveProperty('id', 10);
    expect(res.body).toHaveProperty('name', 'مورد التجاري');
  });
});
