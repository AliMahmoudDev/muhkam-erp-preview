...
describe('POST /api/shifts', () => {
  ...
  it('يجب أن يرفض 403 بدون صلاحية can_manage_employees', async () => {
    ...
    const res = await request(app)
      ...
      .send({ name_ar: 'وردية صباحية', start_time: '08:00', end_time: '16:00' });
  ...
  it('يجب أن يرجع 400 عند بيانات غير صحيحة', async () => {
    ...
    const res = await request(app)
      .post('/api/shifts')
      .set('Authorization', 'Bearer test-token')
      .send({ name_ar: '', start_time: 'invalid', end_time: '16:00' });
...
describe('POST /api/attendance/check-in', () => {
  ...
  it('يجب أن يرجع 409 إذا تم تسجيل الحضور مسبقاً', async () => {
    ...
    const res = await request(app)
      .post('/api/attendance/check-in')
      .set('Authorization', 'Bearer test-token')
      .send({ employee_id: 10, attendance_date: '2024-06-01' });
  ...
  it('يجب أن يُسجل الحضور بنجاح ويرجع 201', async () => {
    ...
    const res = await request(app)
      .post('/api/attendance/check-in')
      .set('Authorization', 'Bearer test-token')
      .send({ employee_id: 10, attendance_date: '2024-06-01', check_in_time: '08:00' });
    expect(res.status).toBe(201);
...
describe('POST /api/attendance/check-out', () => {
  ...
  it('يجب أن يرجع 409 إذا تم تسجيل الانصراف مسبقاً', async () => {
    mockChainData
      ...
      .mockResolvedValueOnce([{ id: 1, check_in_time: '08:00', check_out_time: '16:00' }]);
    const request = (await import('supertest')).default;
    const app = (await import('../../app')).default;
    const res = await request(app)
      .post('/api/attendance/check-out')
...
describe('PUT /api/attendance/records/:id', () => {
  ...
  it('يجب أن يرجع 404 إذا كان السجل غير موجود', async () => {
    ...
    const app = (await import('../../app')).default;
    const res = await request(app)
      .put('/api/attendance/records/999')
      .set('Authorization', 'Bearer test-token')
      .send({ status: 'present' });
    expect(res.status).toBe(404);
  ...
  /**
   * Regression — attendance record cross-tenant update (fix: innerJoin ownership check)
   *
   * Admin of company 1 attempts PUT /attendance/records/99 where record 99 belongs
   * to an employee of company 2. The ownership innerJoin finds no matching row
   * (mockChainData → []) so the route must return 404 before calling db.update.
   */
  it('يمنع تعديل سجل حضور لموظف تابع لشركة أخرى — عزل المستأجر', async () => {
    mockChainData.mockResolvedValue([]); // innerJoin(employeesTable, company_id=1) finds nothing
    const request = (await import('supertest')).default;
    const app = (await import('../../app')).default;

    const res = await request(app)
      .put('/api/attendance/records/99')
      .set('Authorization', 'Bearer test-token')
      .send({ status: 'absent', notes: 'محاولة عزل' });

    expect(res.status).toBe(404);
    expect(res.body.error).toBe('السجل غير موجود');

    // Guard fires before any write — db.update must not be called
    const { db: dbImport } = await import('@workspace/db');
    expect(
      vi.mocked(dbImport as unknown as { update: ReturnType<typeof vi.fn> }).update
...
describe('POST /api/attendance/overtime', () => {
  ...
  it('يجب أن يرفض 403 بدون صلاحية can_manage_employees', async () => {
    ...
    const res = await request(app)
      ...
      .set('Authorization', 'Bearer test-token')
      .send({ employee_id: 10, date: '2024-06-01', hours: 2 });
    expect(res.status).toBe(403);
  });
  ...
  it('يجب أن يرجع 400 عند بيانات غير صحيحة', async () => {
    ...
    const res = await request(app)
      .post('/api/attendance/overtime')
      .set('Authorization', 'Bearer test-token')
      .send({ employee_id: 10, date: 'invalid', hours: 2 });
    expect(res.status).toBe(400);
  });
