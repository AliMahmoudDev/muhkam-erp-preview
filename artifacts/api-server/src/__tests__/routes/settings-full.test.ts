...
describe('GET /api/settings/system', () => {
  ...
  it('يجب أن يرجع 200 وكائن الإعدادات للمستخدم المصرح له', async () => {
...
describe('POST /api/settings/system', () => {
  beforeEach(() => {
    mockChainData.mockResolvedValue([]);
    vi.mocked(authenticate).mockImplementation(
      (req: Request, _res: Response, next: NextFunction) => {
  ...
  it('يجب أن يرجع 200 عند حفظ إعداد بنجاح من قِبل مسؤول النظام', async () => {
    const request = (await import('supertest')).default;
    const app = (await import('../../app')).default;
    ...
    const res = await request(app)
      ...
      .set('Authorization', 'Bearer test-token')
      .send({ key: 'company_name', value: 'شركة الاختبار' });

    expect(res.status).toBe(200);
    expect(res.body).toHaveProperty('success', true);
    expect(res.body).toHaveProperty('key', 'company_name');
  });

  it('يجب أن يرجع 403 للمستخدم الذي ليس مسؤول نظام', async () => {
    // Apply employeeUser for both the global and the route-level authenticate calls.
