...
// mockChainData drives every `await db.select()...` resolution in this file.
// Setting it once per test avoids repeated chain-mock setup.
...
vi.mock('@workspace/db', () => {
  const txMock: Record<string, ReturnType<typeof vi.fn>> = {
    ...
    returning: vi.fn().mockResolvedValue([{
      id: 99, invoice_no: 'PUR-2024-0001', supplier_name: 'مورد اختبار',
      customer_id: null, customer_name: null, payment_type: 'credit',
      total_amount: '500.00', paid_amount: '0.00', remaining_amount: '500.00',
      status: 'unpaid', date: '2024-01-15', notes: null,
      currency: 'EGP', exchange_rate: '1', shipping_cost: '0',
      is_consignment: false, consignment_warehouse_id: null,
      company_id: 1, tax_amount: '0', posting_status: 'draft',
  ...
  // Thenable chain: every terminal `await` resolves via mockChainData()
...
// ── Lib mocks (helpers called by purchases route) ────────────────────────────
...
// ── Fixture ───────────────────────────────────────────────────────────────────
const mockPurchaseA = {
  id: 42, invoice_no: 'PUR-2024-0042', supplier_name: 'مورد أ',
  customer_id: null, customer_name: null, payment_type: 'credit',
  total_amount: '500.00', paid_amount: '0.00', remaining_amount: '500.00',
  status: 'unpaid', posting_status: 'draft', date: '2024-01-15',
  notes: null, currency: 'EGP', exchange_rate: '1.00',
  shipping_cost: '0.00', is_consignment: false,
  consignment_warehouse_id: null, company_id: 1,
  created_at: new Date('2024-01-15T10:00:00.000Z'),
  tax_amount: '0.00', warehouse_id: 1,
};
...
describe('GET /api/purchases', () => {
  ...
  it('يجب أن يرفض المستخدم بدون صلاحية can_view_purchases بـ 403', async () => {
    vi.mocked(authenticate).mockImplementationOnce(
      (req: Request, _res: Response, next: NextFunction) => {
  ...
  it('يجب أن يعزل بيانات الشركة — company B لا ترى مشتريات company A', async () => {
    // Company A sees its purchase
    mockChainData.mockResolvedValueOnce([mockPurchaseA]);
    ...
    // Company B — mockChainData falls back to default ([])
    vi.mocked(authenticate).mockImplementationOnce(
      (req: Request, _res: Response, next: NextFunction) => {
...
describe('POST /api/purchases', () => {
  ...
  it('يجب أن يرجع 403 للمستخدم بدون صلاحية can_create_purchase', async () => {
    vi.mocked(authenticate).mockImplementationOnce(
      (req: Request, _res: Response, next: NextFunction) => {
        (req as Request & { user: AuthUser }).user = employeeUser;
        next();
    ...
    const res = await request(app)
      .post('/api/purchases')
      .set('Authorization', 'Bearer test-token')
      .send({ payment_type: 'credit', total_amount: 100, paid_amount: 0, items: [] });

    expect(res.status).toBe(403);
    expect(res.body).toHaveProperty('error');
  });

  it('يجب أن يرجع 201 عند إنشاء فاتورة شراء بنجاح', async () => {
    // Mock the transaction to return a fully-formed purchase directly,
    // bypassing inner DB operations (product lookups, inserts etc.)
    ...
    const res = await request(app)
      ...
      .send({
        ...
        warehouse_id: 1,
        items: [
          { product_id: 1, product_name: 'قطعة اختبار', quantity: 2, unit_price: 250, total_price: 500 },
        ],
      });
    ...
    expect(res.body.payment_type).toBe('credit');
  });
});
...
describe('GET /api/purchases/:id', () => {
  ...
  it('يجب أن يرجع 404 لمعرّف مشتريات غير موجود', async () => {
    const request = (await import('supertest')).default;
    const app = (await import('../../app')).default;
  ...
  it('يجب أن يرجع 200 وبيانات الفاتورة عند وجودها', async () => {
    // First chain call returns the purchase; second call (items) returns []
    mockChainData.mockResolvedValueOnce([mockPurchaseA]).mockResolvedValueOnce([]);
    ...
    expect(res.body).toHaveProperty('invoice_no', 'PUR-2024-0042');
  ...
  it('يجب أن يرفض الطلب بدون token بـ 401', async () => {
    ...
    const res = await request(app).get('/api/purchases/42');
