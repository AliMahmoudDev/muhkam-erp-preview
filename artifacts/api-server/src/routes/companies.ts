...
router.get("/subscription/status", authenticate, wrap(async (req, res) => {
  const [user] = await db
    .select()
    .from(erpUsersTable)
  ...
  if (!company) { res.json({ hasSubscription: false }); return; }
  ...
  res.json({
    ...
    edition: company.edition ?? "ultimate",
...
router.get("/companies", ...adminOnly, wrap(async (_req, res) => {
...
router.get("/companies/:id", ...adminOnly, wrap(async (req, res) => {
  const [company] = await db
    .select()
    .from(companiesTable)
...
router.post("/companies", ...adminOnly, wrap(async (req, res) => {
  ...
  if (!["trial","basic","pro"].includes(plan_type)) {
    res.status(400).json({ error: "نوع الخطة يجب أن يكون: trial أو basic أو pro" }); return;
  }
...
router.put("/companies/:id", ...adminOnly, wrap(async (req, res) => {
...
router.put("/companies/:id/assign-user/:userId", ...adminOnly, wrap(async (req, res) => {
...
router.delete("/companies/:id", ...adminOnly, wrap(async (req, res) => {
