...
async function getPrimaryWarehouseId(branchId: number, companyId: number): Promise<number> {
  const [wh] = await db
    ...
    .where(and(
      eq(warehousesTable.branch_id, branchId),
...
// ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
router.post("/transfers/request", wrap(async (req, res) => {
  ...
  if (availableStock < qty) {
    throw httpError(400, `المخزون غير كافٍ في "${fromBranch.name}": المتاح ${availableStock.toFixed(3)}، المطلوب ${qty}`);
  }
...
// ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
router.post("/transfers/ship/:id", wrap(async (req, res) => {
  if (!hasPermission(req.user, "can_view_inventory")) {
  ...
  if (transfer.status !== "approved") {
    throw httpError(409, `لا يمكن الشحن — الحالة الحالية: ${transfer.status} (يجب أن تكون "approved")`);
  }
...
// ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
router.post("/transfers/confirm/:id", wrap(async (req, res) => {
  if (!hasPermission(req.user, "can_view_inventory")) {
  ...
  if (transfer.status !== "shipped") {
    throw httpError(409, `لا يمكن التأكيد — الحالة الحالية: ${transfer.status} (يجب أن تكون "shipped")`);
  }
...
// ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
router.post("/transfers/cancel/:id", wrap(async (req, res) => {
  const companyId  = getTenant(req);
  const transferId = Number(req.params.id);
...
router.get("/transfers", wrap(async (req, res) => {
  ...
  if (branch_id) {
    ...
    conditions.push(
      or(
        eq(stockTransfersTable.from_branch_id, bid),
        eq(stockTransfersTable.to_branch_id,   bid),
      )!
    );
  }
...
router.get("/transfers/:id", wrap(async (req, res) => {
  if (!hasPermission(req.user, "can_view_inventory")) {
