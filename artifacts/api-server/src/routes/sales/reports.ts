...
import {
  db, salesTable, saleItemsTable, customersTable, transactionsTable,
  safesTable, stockMovementsTable, accountsTable, salesReturnsTable,
  receiptVouchersTable, journalEntriesTable, journalEntryLinesTable, customerLedgerTable,
} from "@workspace/db";
...
void [transactionsTable, safesTable, stockMovementsTable, accountsTable,
      salesReturnsTable, receiptVouchersTable, journalEntriesTable,
      journalEntryLinesTable, customerLedgerTable];
...
router.get("/sales/:id", wrap(async (req, res) => {
  ...
  const companyId = getTenant(req);
  const [sale] = await db.select().from(salesTable).where(and(eq(salesTable.id, params.data.id), eq(salesTable.company_id, companyId)));
...
router.post("/sales/:id/post", wrap(async (req, res) => {
  ...
  const saleItems = await db.select({ product_id: saleItemsTable.product_id }).from(saleItemsTable).where(eq(saleItemsTable.sale_id, id));
  void runAllChecks({ customerId: updated.customer_id ?? undefined });
  for (const item of saleItems) { if (item.product_id) void runAllChecks({ productId: item.product_id }); }
