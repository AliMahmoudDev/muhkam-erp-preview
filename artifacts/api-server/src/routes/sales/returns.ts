...
import {
  ...
  transactionsTable, safesTable, stockMovementsTable,
  salesReturnsTable, receiptVouchersTable,
  journalEntriesTable, journalEntryLinesTable, customerLedgerTable,
} from "@workspace/db";
...
router.post("/sales/:id/cancel", wrap(async (req, res) => {
  ...
  const [sale] = await db.select().from(salesTable).where(and(eq(salesTable.id, id), eq(salesTable.company_id, companyId)));
  ...
  const existingReturns = await db.select({ id: salesReturnsTable.id }).from(salesReturnsTable).where(eq(salesReturnsTable.sale_id, id));
  ...
  if (sale.customer_id && Number(sale.remaining_amount) > 0) {
    ...
    if (laterRVs.length > 0) {
      ...
      throw httpError(400, `لا يمكن الإلغاء: توجد ${laterRVs.length} سند(ات) قبض مُسجَّلة على هذا العميل بعد تاريخ الفاتورة (${nos}) — قد تكون مقيّدة على هذه الذمة`);
    }
  }
    ...
    if (saleJE) {
      ...
      if (accountIds.length > 0) {
        ...
        if (laterLines.length > 0) {
          throw httpError(400, "لا يمكن العكس: توجد قيود محاسبية لاحقة مبنية على نفس حسابات هذه الفاتورة — راجع دفتر الأستاذ قبل الإلغاء");
        }
      }
    }
  }
  ...
  await db.transaction(async (tx) => {
    ...
    const saleItems = await tx.select().from(saleItemsTable).where(eq(saleItemsTable.sale_id, sale.id));
    ...
    if (paidAmt > 0 && sale.safe_id) {
      ...
      await tx.insert(transactionsTable).values({
        type: "sale_cancel", reference_type: "sale_cancel",
        reference_id: sale.id,
        safe_id: sale.safe_id, safe_name: sale.safe_name ?? "",
        customer_id: sale.customer_id ?? null, customer_name: sale.customer_name ?? null,
        amount: String(paidAmt), direction: "out",
        description: `إلغاء فاتورة مبيعات ${sale.invoice_no}`,
        date: today, company_id: companyId,
      });
    }
    ...
    if (sale.customer_id) {
      ...
      if (totalAmt > 0) {
        await tx.insert(customerLedgerTable).values({
          customer_id: sale.customer_id, type: "sale_cancel", amount: String(-totalAmt),
          reference_type: "sale", reference_id: sale.id, reference_no: sale.invoice_no,
          description: `إلغاء فاتورة مبيعات ${sale.invoice_no}`,
          date: today, company_id: companyId,
        });
      }
      if (paidAmt > 0) {
        await tx.insert(customerLedgerTable).values({
          customer_id: sale.customer_id, type: "sale_cancel", amount: String(paidAmt),
          reference_type: "sale", reference_id: sale.id, reference_no: sale.invoice_no,
