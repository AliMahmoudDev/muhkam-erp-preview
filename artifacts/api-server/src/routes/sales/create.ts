...
router.post(
  ...
  wrap(async (req, res) => {
    if (!hasPermission(req.user, 'can_create_sale')) {
      res.status(403).json({ error: 'غير مصرح بإنشاء فواتير بيع' });
      return;
    }
    ...
    if (pt === 'cash' && !hasPermission(req.user, 'can_cash_sale')) {
      res.status(403).json({ error: 'غير مصرح بالبيع النقدي' });
      return;
    }
    if (pt === 'partial' && !hasPermission(req.user, 'can_partial_sale')) {
      res.status(403).json({ error: 'غير مصرح بالبيع الجزئي' });
      return;
    }
    if (pt === 'credit' && !hasPermission(req.user, 'can_credit_sale')) {
      res.status(403).json({ error: 'غير مصرح بالبيع الآجل' });
      return;
    }
    ...
    if (paymentsInput && paymentsInput.length > 0) {
      ...
      if (Math.abs(totalAllocated - total_amount) > 0.05) {
        return res
          .status(400)
          .json({
            error: `مجموع الدفعات (${totalAllocated.toFixed(2)}) لا يساوي إجمالي الفاتورة (${total_amount.toFixed(2)})`,
          });
      }
    } else {
      if (payment_type === 'credit') {
        ...
        effectivePayments = [];
        if (paid_amount > 0)
          effectivePayments.push({ type: 'cash', safe_id: safe_id ?? null, amount: paid_amount });
    ...
    const computedPaymentType: 'cash' | 'credit' | 'partial' =
      effectiveCreditAmount === 0 ? 'cash' : effectivePaidAmount === 0 ? 'credit' : 'partial';
    const primarySafeId = cashPayments.length > 0 ? cashPayments[0].safe_id : null;
    const remaining = effectiveCreditAmount;

    const status =
      effectivePaidAmount === 0 && effectiveCreditAmount > 0
        ? 'unpaid'
    ...
    if (role === 'cashier' || role === 'salesperson') {
      if (!userSafeId && cashPayments.length > 0) {
        res.status(403).json({ error: 'المستخدم غير مرتبط بخزينة — راجع الإعدادات' });
        return;
      }
      for (const p of cashPayments) {
        if (p.safe_id && p.safe_id !== userSafeId) {
          res.status(403).json({ error: 'لا يمكنك استخدام خزنة غير المخصصة لك' });
          return;
        }
        if (!p.safe_id) p.safe_id = userSafeId;
      }
    ...
    const sale = await db.transaction(async (tx) => {
      ...
      const uniqueSafeIds = [
        ...new Set(cashPayments.map((p) => p.safe_id).filter((id): id is number => id != null)),
      ];
      if (uniqueSafeIds.length > 0) {
        const safes = await tx
          .select()
          .from(safesTable)
          .where(and(inArray(safesTable.id, uniqueSafeIds), eq(safesTable.company_id, companyId)));
      ...
      if (warehouse_id) {
        const [w] = await tx
          ...
          .where(
            and(eq(warehousesTable.id, warehouse_id), eq(warehousesTable.company_id, companyId))
          );
      ...
      if (salesperson_id) {
        const [u] = await tx
          ...
          .from(erpUsersTable)
          .where(
            and(eq(erpUsersTable.id, salesperson_id), eq(erpUsersTable.company_id, companyId))
          );
      ...
      if (effectiveCreditAmount > 0 && customer_id) {
        ...
        if (cust) {
          await tx
            ...
            .where(
              and(eq(customersTable.id, customer_id), eq(customersTable.company_id, companyId))
            );
        }
      }
    ...
    if (role === 'cashier') {
      ...
      void runAllChecks({ companyId, customerId: sale.customer_id ?? undefined });
