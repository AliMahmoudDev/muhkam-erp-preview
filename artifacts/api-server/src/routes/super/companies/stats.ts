...
router.get(
  ...
  wrap(async (req, res) => {
    ...
    const [salesRow, purchasesRow] = await Promise.all([
      db.execute(
        sql`SELECT COUNT(*) AS cnt, COALESCE(SUM(total),0)::numeric AS total FROM sales WHERE company_id=${id}`
      ),
...
router.get(
  ...
  wrap(async (_req, res) => {
    ...
    const expiringSoon = companies.filter(
      (c) => c.is_active && c.end_date >= nowStr && c.end_date <= in7Str
    );
    ...
    const stats = {
      ...
      trial: companies.filter((c) => c.plan_type === 'trial' && c.is_active && c.end_date >= nowStr)
        .length,
      paid: companies.filter((c) => c.plan_type === 'paid' && c.is_active && c.end_date >= nowStr)
        .length,
