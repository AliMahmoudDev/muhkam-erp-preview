...
router.get(
  ...
  wrap(async (req, res) => {
    ...
    const [companies, userCounts] = await Promise.all([
      db.select().from(companiesTable).orderBy(desc(companiesTable.created_at)),
      db
        .select({
...
router.put(
  ...
  wrap(async (req, res) => {
    ...
    if (edition !== undefined && ['advanced', 'ultimate'].includes(edition))
...
router.post(
  ...
  wrap(async (req, res) => {
    ...
    await writeAuditLog({
      action: 'create',
      record_type: 'company',
      record_id: result.company.id,
      old_value: null,
      new_value: { name, plan_type, admin: result.admin.username },
      user: req.user,
      company_id: null,
  ...
  wrap(async (req, res) => {
    ...
    try {
      ...
      if (isPgFk) {
        res.status(409).json({
          error:
            'فشل حذف الشركة — يوجد بيانات مرتبطة لم يتم حذفها بالكامل. يرجى التواصل مع الدعم الفني.',
    ...
    await writeAuditLog({
      ...
      record_id: id,
      old_value: co,
      new_value: { deleted_user_count: usersInCompany.length, force: !!force },
      user: req.user,
      company_id: req.user?.company_id ?? null,
