...
/* Full cascade delete for a company — disables FK checks temporarily via
   session_replication_role so Postgres skips constraint validation, deletes
   the company row (and all related data is cleaned by existing CASCADE
   constraints), then re-enables FK checks. Wrapped in a transaction so
   the role reset is guaranteed even on error. */
