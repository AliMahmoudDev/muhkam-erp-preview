...
router.get("/reports/cash-flow-indirect", wrap(async (req, res) => {
  ...
  let netIncome = 0;
  for (const r of incomeData.rows as Array<{ type: string; total_debit: number; total_credit: number }>) {
    if (r.type === "revenue") netIncome += Number(r.total_credit) - Number(r.total_debit);
    if (r.type === "expense") netIncome -= Number(r.total_debit) - Number(r.total_credit);
  }
  ...
  // استخدام حركة الحسابات خلال الفترة من قيود اليومية

  // مدينون تجاريون (أصل متداول - ارتفاعه يستهلك نقدية)
  const arData = await db.execute(sql`
    ...
    WHERE je.company_id = ${companyId}
      ...
      AND a.type = 'asset'
  `);
  const arChange = Number((arData.rows[0] as { net: number })?.net ?? 0); // زيادة AR = سلبي للنقدية

  // دائنون تجاريون (التزام - ارتفاعه يحسن النقدية)
  ...
  // المخزون (أصل - ارتفاعه يستهلك نقدية)
  ...
  `);
  const financingNet = (finData.rows as Array<{ net: number }>).reduce((s, r) => s + Number(r.net), 0);
