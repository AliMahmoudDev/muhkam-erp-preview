...
function numOrNull(v: string | null | undefined) {
...
router.post(
  '/salary-structures',
  wrap(async (req, res) => {
    if (!hasPermission(req.user, 'can_manage_payroll')) {
    ...
    if (!parsed.success) {
      res
        .status(400)
        .json({
          error: parsed.error.errors[0]?.message ?? 'بيانات الهيكل غير صحيحة',
...
router.put(
  ...
  wrap(async (req, res) => {
    ...
    const [row] = await db
      ...
      .returning();
    if (!row) {
      res.status(404).json({ error: 'الهيكل غير موجود' });
      return;
    }
    res.json({
      ...row,
...
router.delete(
  ...
  wrap(async (req, res) => {
    ...
    await db
      .delete(salaryStructuresTable)
      .where(
        and(eq(salaryStructuresTable.id, id), eq(salaryStructuresTable.company_id, companyId))
      );
    res.json({ ok: true });
  })
);
...
async function assertStructureOwnership(structureId: number, companyId: number): Promise<boolean> {
  const [row] = await db
    .select({ id: salaryStructuresTable.id })
    .from(salaryStructuresTable)
    .where(
      and(
        eq(salaryStructuresTable.id, structureId),
        eq(salaryStructuresTable.company_id, companyId)
      )
    )
    .limit(1);
  return !!row;
...
router.get(
  ...
  wrap(async (req, res) => {
    const companyId = getTenant(req);
    const id = parseInt(String(req.params['id']), 10);
    if (!(await assertStructureOwnership(id, companyId))) {
      res.status(404).json({ error: 'هيكل الراتب غير موجود' });
      return;
    }
    const rows = await db
      .select()
    ...
    res.json(
      rows.map((r) => ({
        ...
        percentage_of_base: numOrNull(r.percentage_of_base),
        created_at: fmt(r.created_at),
      }))
    );
  })
  ...
  wrap(async (req, res) => {
    ...
    if (!parsed.success) {
      res
        .status(400)
        .json({
          error: parsed.error.errors[0]?.message ?? 'بيانات العنصر غير صحيحة',
          details: parsed.error.errors,
        });
      return;
    }
    ...
    res
      ...
      .json({
        ...
        percentage_of_base: numOrNull(row.percentage_of_base),
        created_at: fmt(row.created_at),
      });
  })
);
...
router.delete(
  '/salary-structures/:structId/components/:id',
  wrap(async (req, res) => {
    if (!hasPermission(req.user, 'can_manage_payroll')) {
      res.status(403).json({ error: 'غير مصرح' });
      return;
    ...
    const companyId = getTenant(req);
    const structId = parseInt(String(req.params['structId']), 10);
    const id = parseInt(String(req.params['id']), 10);
    if (!(await assertStructureOwnership(structId, companyId))) {
      res.status(404).json({ error: 'هيكل الراتب غير موجود' });
      return;
    }
    await db
      ...
      .where(
        and(
          eq(salaryComponentsTable.id, id),
          eq(salaryComponentsTable.salary_structure_id, structId)
        )
      );
    res.json({ ok: true });
  })
...
router.get(
  ...
  wrap(async (req, res) => {
    ...
    const rows = await db
      .select()
      .from(taxBracketsTable)
      .where(
        and(eq(taxBracketsTable.company_id, companyId), eq(taxBracketsTable.fiscal_year, year))
      )
    ...
    res.json(
      rows.map((r) => ({
        ...
        max_salary: numOrNull(r.max_salary),
  ...
  wrap(async (req, res) => {
    ...
    if (!parsed.success) {
      res
        .status(400)
        .json({
          error: parsed.error.errors[0]?.message ?? 'بيانات الشريحة الضريبية غير صحيحة',
          details: parsed.error.errors,
        });
      return;
    }
    ...
    const [row] = await db
      .insert(taxBracketsTable)
      .values({
        company_id: companyId,
        fiscal_year,
        min_salary: String(min_salary),
        max_salary: max_salary != null ? String(max_salary) : null,
    ...
    res
      ...
      .json({
        ...
        max_salary: numOrNull(row.max_salary),
...
router.put(
  ...
  wrap(async (req, res) => {
    ...
    const [row] = await db
      ...
      .returning();
    if (!row) {
      res.status(404).json({ error: 'الشريحة الضريبية غير موجودة' });
      return;
    }
    res.json({
      ...row,
...
router.get(
  ...
  wrap(async (req, res) => {
    ...
    const rows = await db
      .select()
      .from(statutoryContributionsTable)
      .where(eq(statutoryContributionsTable.company_id, companyId))
      .orderBy(statutoryContributionsTable.name_ar);
    res.json(
      rows.map((r) => ({
        ...r,
        employee_percentage: Number(r.employee_percentage),
        employer_percentage: Number(r.employer_percentage),
        created_at: fmt(r.created_at),
      }))
    );
  })
  ...
  wrap(async (req, res) => {
    ...
    if (!parsed.success) {
      res
        .status(400)
        .json({
          error: parsed.error.errors[0]?.message ?? 'بيانات الاشتراك غير صحيحة',
          details: parsed.error.errors,
        });
      return;
    }
    ...
    const [row] = await db
      .insert(statutoryContributionsTable)
      .values({
        company_id: companyId,
        contribution_type,
        name_ar,
    ...
    res
      ...
      .json({
        ...row,
        employee_percentage: Number(row.employee_percentage),
        employer_percentage: Number(row.employer_percentage),
        created_at: fmt(row.created_at),
      });
  })
);
...
router.put(
  ...
  wrap(async (req, res) => {
    ...
    const { name_ar, name_en, employee_percentage, employer_percentage, is_mandatory, is_active } =
      parsed.data;
    const [row] = await db
      .update(statutoryContributionsTable)
      .set({
        name_ar,
        name_en: name_en ?? name_ar,
        employee_percentage: String(employee_percentage ?? 0),
      ...
      .where(
        and(
          eq(statutoryContributionsTable.id, id),
          eq(statutoryContributionsTable.company_id, companyId)
        )
      )
      .returning();
    if (!row) {
...
router.delete(
  ...
  wrap(async (req, res) => {
    ...
    const id = parseInt(String(req.params['id']), 10);
    await db
      .delete(statutoryContributionsTable)
      .where(
        and(
          eq(statutoryContributionsTable.id, id),
          eq(statutoryContributionsTable.company_id, companyId)
...
router.get(
  ...
  wrap(async (req, res) => {
    if (!hasPermission(req.user, 'can_view_payroll')) {
      res.status(403).json({ error: 'غير مصرح' });
      return;
    }
...
router.post(
  ...
  wrap(async (req, res) => {
    ...
    const { employee_id, salary_amount, currency, effective_date, reason } = req.body as Record<
      string,
      unknown
    >;
    if (!employee_id || !salary_amount || !effective_date) {
      res.status(400).json({ error: 'بيانات التاريخ الوظيفي غير مكتملة' });
      return;
    }
    const [row] = await db
      .insert(salaryHistoryTable)
      .values({
        employee_id: Number(employee_id),
        salary_amount: String(Number(salary_amount)),
    ...
    res
      .status(201)
      .json({ ...row, salary_amount: Number(row.salary_amount), created_at: fmt(row.created_at) });
  })
);
