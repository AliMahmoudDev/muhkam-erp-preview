...
router.get(
  ...
  wrap(async (req, res) => {
    if (!hasPermission(req.user, 'can_view_products')) {
      res.status(403).json({ error: 'غير مصرح بعرض الأصناف' });
      return;
    }
    const companyId = getTenant(req);
    const warehouseIdParam = req.query.warehouse_id
  ...
  wrap(async (req, res) => {
    ...
    if (resolvedCategoryId) {
      const [cat] = await db
        .select()
        .from(categoriesTable)
        .where(eq(categoriesTable.id, resolvedCategoryId));
...
router.put(
  ...
  wrap(async (req, res) => {
    if (!hasPermission(req.user, 'can_manage_products')) {
      res.status(403).json({ error: 'غير مصرح بتعديل المنتجات' });
      return;
    }
    ...
    const [product] = await db
      ...
      .set({
        ...
        cost_price:
          parsed.data.cost_price !== undefined ? String(parsed.data.cost_price) : undefined,
        sale_price:
          parsed.data.sale_price !== undefined ? String(parsed.data.sale_price) : undefined,
        low_stock_threshold: parsed.data.low_stock_threshold ?? null,
      })
      .where(and(eq(productsTable.id, params.data.id), eq(productsTable.company_id, companyId)))
      .returning();
    ...
    await deleteCache(`products:${companyId}`);
    res.json(
      UpdateProductResponse.parse(formatProduct({ ...product, category_name: categoryName }))
    );
  })
);
...
router.post(
  '/products/bulk-margin-update',
  wrap(async (req, res) => {
    if (!hasPermission(req.user, 'can_manage_products')) {
      res.status(403).json({ error: 'غير مصرح' });
      return;
    }
    const { margin_percent, category_id } = req.body as {
      margin_percent: number;
      category_id?: number;
    };
    if (typeof margin_percent !== 'number' || margin_percent < 0) {
      res.status(400).json({ error: 'نسبة الهامش غير صالحة' });
      return;
    }
    const companyId = getTenant(req);
...
router.delete(
  ...
  wrap(async (req, res) => {
    if (!hasPermission(req.user, 'can_manage_products')) {
      res.status(403).json({ error: 'غير مصرح بحذف المنتجات' });
      return;
    }
