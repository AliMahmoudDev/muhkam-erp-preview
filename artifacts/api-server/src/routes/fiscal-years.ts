  ...
  wrap(async (req, res) => {
    ...
    if (!parsed.success) {
      res
        .status(400)
        .json({
          error: 'بيانات السنة المالية غير صحيحة',
...
router.patch(
  ...
  wrap(async (req, res) => {
    ...
    if (!fy.is_open) {
      res.status(400).json({ error: 'لا يمكن تعيين سنة مقفلة كالسنة الحالية' });
      return;
    }
...
router.patch(
  ...
  wrap(async (req, res) => {
    ...
    void writeAuditLog({
      ...
      record_type: 'fiscal_year',
      record_id: id,
      old_value: { is_open: true },
      new_value: { is_open: false, closed_at: updated.closed_at },
      user: { id: userId, username: requireUser(req).username },
      company_id: companyId,
    });
...
router.patch(
  ...
  wrap(async (req, res) => {
    ...
    if (role !== 'admin' && role !== 'super_admin') {
      res.status(403).json({ error: 'إعادة فتح السنة المالية للمسؤول فقط' });
      return;
    }
...
router.delete(
  ...
  wrap(async (req, res) => {
    ...
    const role = req.user?.role;
    if (role !== 'admin' && role !== 'super_admin') {
      res.status(403).json({ error: 'حذف السنة المالية للمسؤول فقط' });
      return;
    }
    ...
    if (!fy.is_open) {
      res.status(400).json({ error: 'لا يمكن حذف سنة مالية مقفلة' });
      return;
    }
    if (fy.is_current) {
      res.status(400).json({ error: 'لا يمكن حذف السنة المالية الحالية' });
      return;
    }
...
router.post(
  '/fiscal-years/:id/closing-entries',
  requireTenant,
  wrap(async (req, res) => {
    const companyId = getTenant(req);
    const role = req.user?.role;
  ...
  `);
    ...
    if (rows.length === 0)
      throw httpError(400, 'لا توجد حسابات إيرادات أو مصروفات لإقفالها في هذه الفترة');
    ...
    const retainedEarnings = await getOrCreateAccount(
      { code: 'EQUITY-RETAINED', name: 'الأرباح المحتجزة', type: 'equity' },
      companyId
    );
    ...
    for (const l of lines) {
      ...
      await db
        ...
        .where(and(eq(accountsTable.id, l.accountId), eq(accountsTable.company_id, companyId)));
    }
