...
function parseStatuses(raw: string | string[] | unknown): string[] {
  ...
  if (typeof raw === 'string') {
    try {
      ...
      logger.warn(
        { err, raw },
        '[repair-dashboard-cards] Invalid statuses JSON — returning empty list'
      );
      return [];
    }
  }
...
async function ensureDefaults(company_id: number) {
  const existing = await db
    .select({ id: repairDashboardCardsTable.id })
...
router.post(
  ...
  wrap(async (req, res) => {
    ...
    const [maxRow] = await db
      .select({ m: sql<number>`coalesce(max(${repairDashboardCardsTable.sort_order}), -1)` })
      .from(repairDashboardCardsTable)
      .where(eq(repairDashboardCardsTable.company_id, company_id));
    const nextOrder = Number(maxRow?.m ?? -1) + 1;

    const [row] = await db
      .insert(repairDashboardCardsTable)
      .values({
        company_id,
        name,
...
router.patch(
  ...
  wrap(async (req, res) => {
    ...
    const [existing] = await db
      ...
      .where(
        and(
          eq(repairDashboardCardsTable.id, id),
          eq(repairDashboardCardsTable.company_id, company_id)
        )
      );
    ...
    if (!parsed.success) {
      return res
        .status(400)
        .json({
          error: 'بيانات تحديث الكارت غير صحيحة',
          details: parsed.error.issues.map((i) => i.message),
        });
    }
    ...
    const [row] = await db
      .update(repairDashboardCardsTable)
      .set(patch)
      .where(
        and(
          eq(repairDashboardCardsTable.id, id),
...
router.delete(
  ...
  wrap(async (req, res) => {
    ...
    await db
      .delete(repairDashboardCardsTable)
      .where(
        and(
          eq(repairDashboardCardsTable.id, id),
          eq(repairDashboardCardsTable.company_id, company_id)
...
/* ── Reorder (admin only) ────────────────────────────────────────── */
router.post(
  '/repair-dashboard-cards/reorder',
  requireRole('admin', 'super_admin'),
  wrap(async (req, res) => {
    const { company_id } = ctx(req);
    ...
    if (!parsed.success) {
      return res
        .status(400)
        .json({
          error: 'بيانات إعادة الترتيب غير صحيحة',
          details: parsed.error.issues.map((i) => i.message),
        });
    }
