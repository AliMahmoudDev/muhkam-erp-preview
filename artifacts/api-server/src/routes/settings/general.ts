...
async function upsertSetting(key: string, value: string | null, companyId: number) {
  if (!companyId || companyId <= 0)
    throw Object.assign(new Error('upsertSetting: companyId required'), { status: 403 });
  if (value === null) {
    await db
...
router.post(
  ...
  wrap(async (req, res) => {
    ...
    if (!parsedReset.success) {
      res.status(400).json({ error: firstZodError(parsedReset.error) });
      return;
    }
    ...
    await db.transaction(async (tx) => {
      ...
      await tx.execute(
        sql`DELETE FROM refresh_tokens          WHERE user_id IN (SELECT id FROM erp_users WHERE company_id = ${cid} AND id != ${currentUserId})`
      );
      await tx.execute(
      ...
      await tx.execute(
        sql`DELETE FROM erp_users WHERE company_id = ${cid} AND id != ${currentUserId}`
      );
    });

    res.json({
      success: true,
      message: 'تم إعادة تعيين قاعدة البيانات بالكامل — تم حذف جميع البيانات',
    });
  })
);
...
router.get(
  ...
  wrap(async (req, res) => {
    ...
    const linkedPurchases = await db
      ...
      .from(purchasesTable)
      .where(
        and(eq(purchasesTable.customer_id, customerId), eq(purchasesTable.company_id, companyId))
      )
      .orderBy(desc(purchasesTable.created_at));

    const purchasesWithItems = await Promise.all(
      linkedPurchases.map(async (pur) => {
        const items = await db
          .select()
          .from(purchaseItemsTable)
          .where(eq(purchaseItemsTable.purchase_id, pur.id));
        return { ...pur, items };
      })
    );

    const salesReturns = await db
      .select()
      .from(salesReturnsTable)
      .where(
        and(
          eq(salesReturnsTable.customer_id, customerId),
          eq(salesReturnsTable.company_id, companyId)
        )
      )
      .orderBy(desc(salesReturnsTable.created_at));

    const receiptVouchers = await db
      .select()
      .from(receiptVouchersTable)
      .where(
        and(
          eq(receiptVouchersTable.customer_id, customerId),
          eq(receiptVouchersTable.company_id, companyId)
        )
      )
      .orderBy(desc(receiptVouchersTable.created_at));

    const depositVouchers = await db
      .select()
      .from(depositVouchersTable)
      .where(
        and(
          eq(depositVouchersTable.customer_id, customerId),
          eq(depositVouchersTable.company_id, companyId)
        )
      )
    ...
    const paymentVouchers = await db
      ...
      .where(
        and(
          eq(paymentVouchersTable.customer_id, customerId),
          eq(paymentVouchersTable.company_id, companyId)
        )
      )
...
router.get(
  ...
  wrap(async (req, res) => {
    ...
    const companyId = role === 'super_admin' ? Number(req.query.company_id) : req.user?.company_id;
    if (!companyId || !Number.isFinite(companyId)) {
      res
        .status(role === 'super_admin' ? 400 : 403)
        .json({
          error: role === 'super_admin' ? 'company_id query param required' : 'Tenant not resolved',
        });
      return;
    }
...
router.post(
  ...
  wrap(async (req, res) => {
    ...
    if (!parsedSys.success) {
      res.status(400).json({ error: firstZodError(parsedSys.error) });
      return;
    }
