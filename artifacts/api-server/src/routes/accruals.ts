...
const createAccrualSchema = z.object({
  ...
  total_amount: z
    .number({ error: 'المبلغ الإجمالي يجب أن يكون رقماً' })
    .positive('المبلغ يجب أن يكون أكبر من صفر'),
  months_total: z
    .number({ error: 'عدد الأشهر يجب أن يكون رقماً' })
    .int()
    .min(1, 'عدد الأشهر يجب أن يكون شهراً على الأقل'),
...
async function getAccrualAccounts(
  type: string,
  category: string,
  _description: string,
  cid: number
  ...
  if (type === 'prepayment' && category === 'expense') {
    const prepaid = await getOrCreateAccount(
      { code: 'ASSET-PREPAID-EXP', name: 'مصروفات مدفوعة مقدماً', type: 'asset' },
      cid
    );
    const expense = await getOrCreateAccount(
      { code: 'EXP-GENERAL', name: 'مصروفات عمومية وإدارية', type: 'expense' },
      cid
    );
    return {
  ...
  if (type === 'prepayment' && category === 'revenue') {
    const prepaid = await getOrCreateAccount(
      { code: 'LIAB-DEFERRED-REV', name: 'إيرادات مؤجلة', type: 'liability' },
      cid
    );
    const expense = await getOrCreateAccount(
      { code: 'REV-SALES', name: 'إيرادات المبيعات', type: 'revenue' },
      cid
    );
    return {
      prepaid_account_id: prepaid.id,
  ...
  if (type === 'accrual' && category === 'expense') {
    const prepaid = await getOrCreateAccount(
      { code: 'LIAB-ACCRUED-EXP', name: 'مصروفات مستحقة', type: 'liability' },
      cid
    );
    const expense = await getOrCreateAccount(
      { code: 'EXP-GENERAL', name: 'مصروفات عمومية وإدارية', type: 'expense' },
  ...
  // accrual + revenue
  const prepaid = await getOrCreateAccount(
    { code: 'ASSET-ACCRUED-REV', name: 'إيرادات مستحقة التحصيل', type: 'asset' },
    cid
  );
  const expense = await getOrCreateAccount(
    { code: 'REV-MISC', name: 'إيرادات متنوعة', type: 'revenue' },
    cid
  );
  return {
    prepaid_account_id: prepaid.id,
  ...
  wrap(async (req, res) => {
    ...
    if (!parsed.success) {
      res
        .status(400)
        .json({
          error: 'بيانات الاستحقاق غير صالحة',
          details: parsed.error.issues.map((i) => i.message),
        });
      return;
    }
    const { type, category, description, total_amount, months_total, start_date, end_date } =
      parsed.data;

    const accounts = await getAccrualAccounts(type, category, description, cid);

    const [row] = await db
      .insert(accrualsTable)
      .values({
        type,
        category,
        description,
  ...
  wrap(async (req, res) => {
    ...
    if (!recParsed.success) {
      res
        .status(400)
        .json({
          error: 'الفترة مطلوبة بصيغة YYYY-MM',
          details: recParsed.error.issues.map((i) => i.message),
        });
      return;
    }
    ...
    const monthly =
      accrual.months_total > 0 ? Number(accrual.total_amount) / accrual.months_total : 0;
    ...
    const expenseAcc = await db
      ...
      .from(accountsTable)
      .where(
        and(eq(accountsTable.id, accrual.expense_account_id), eq(accountsTable.company_id, cid))
      )
      .then((r) => r[0]);
    const prepaidAcc = await db
      .select()
      .from(accountsTable)
      .where(
        and(eq(accountsTable.id, accrual.prepaid_account_id), eq(accountsTable.company_id, cid))
      )
      .then((r) => r[0]);

    // القيد: من مصروف / إلى مدفوع مقدماً (أو العكس)
    let debitAcc = expenseAcc,
      creditAcc = prepaidAcc;
    if (accrual.type === 'prepayment' && accrual.category === 'revenue') {
      debitAcc = prepaidAcc;
      creditAcc = expenseAcc;
    }
    ...
    const newStatus =
      newRecognized >= Number(accrual.total_amount) - 0.001 ? 'completed' : 'active';
    ...
    const run = await db.transaction(async (tx) => {
      ...
      for (const [acc, sign] of [
        [debitAcc, 1],
        [creditAcc, -1],
      ] as [typeof expenseAcc, number][]) {
        await tx
          .update(accountsTable)
          .set({ current_balance: sql`current_balance + ${String(amount * sign)}::numeric` })
          .where(and(eq(accountsTable.id, acc.id), eq(accountsTable.company_id, cid)));
      }
      ...
      const [accrualRun] = await tx
        .insert(accrualRunsTable)
        .values({
...
router.delete(
  ...
  wrap(async (req, res) => {
    ...
    if (Number(row.amount_recognized) > 0)
      throw httpError(400, 'لا يمكن حذف سجل تم استحقاق جزء منه');
    await db
      .delete(accrualsTable)
      .where(and(eq(accrualsTable.id, Number(req.params.id)), eq(accrualsTable.company_id, cid)));
