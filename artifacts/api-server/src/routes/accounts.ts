...
router.post(
  ...
  wrap(async (req, res) => {
    ...
    void writeAuditLog({
      ...
      record_type: 'account',
      record_id: acc.id,
      new_value: { code: acc.code, name: acc.name, type: acc.type },
      user: req.user,
      company_id: cid,
    });
...
router.put(
  ...
  wrap(async (req, res) => {
    ...
    const [acc] = await db
      ...
      .where(and(eq(accountsTable.id, id), eq(accountsTable.company_id, cid)))
      .returning();
    if (!acc) {
    ...
    const accountRows =
      accountIds.length > 0
        ? await db
            .select()
            .from(accountsTable)
            .where(and(inArray(accountsTable.id, accountIds), eq(accountsTable.company_id, cid)))
        : [];
...
router.delete(
  ...
  wrap(async (req, res) => {
    ...
    if (entry.status === 'posted') {
      res.status(400).json({ error: 'لا يمكن حذف قيد منشور — استخدم عكس القيد' });
      return;
    }
...
router.post(
  ...
  wrap(async (req, res) => {
    ...
    if (original.status !== 'posted') {
      res.status(400).json({ error: 'يمكن عكس القيود المنشورة فقط' });
      return;
    }
    ...
    const origLines = await db
      ...
      .where(eq(journalEntryLinesTable.entry_id, id));
    if (!origLines.length) {
      res.status(400).json({ error: 'القيد لا يحتوي على بنود' });
      return;
    }
    ...
    void writeAuditLog({
      ...
      record_type: 'journal_entry',
      record_id: id,
      new_value: { reversal_entry_id: reversalEntry.id, entry_no: reversalEntry.entry_no },
      user: req.user,
      company_id: cid,
    });
