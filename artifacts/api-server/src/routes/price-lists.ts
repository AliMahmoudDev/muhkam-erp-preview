  ...
  wrap(async (req, res) => {
    ...
    if (!parsed.success) {
      res
        .status(400)
        .json({
          error: 'بيانات قائمة الأسعار غير صحيحة',
...
router.delete(
  ...
  wrap(async (req, res) => {
    ...
    await db
      ...
      .where(and(eq(customersTable.price_list_id, id), eq(customersTable.company_id, companyId)));
...
router.get(
  '/price-lists/customer-price/:customer_id/:product_id',
  wrap(async (req, res) => {
    const companyId = getTenant(req);
    ...
    const markupPercent =
      customer.price_list_markup != null
        ...
        : listItem.markup_percent != null
          ? Number(listItem.markup_percent)
          : null;
