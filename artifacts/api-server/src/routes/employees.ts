...
router.post("/departments", wrap(async (req, res) => {
  ...
  const [dep] = await db.insert(departmentsTable).values({
    company_id: companyId, name_en: name_en?.trim() ?? name_ar.trim(),
    name_ar: name_ar.trim(), description_en: description_en ?? null, description_ar: description_ar ?? null,
  }).returning();
...
router.delete("/departments/:id", wrap(async (req, res) => {
  ...
  const active = await db
    ...
    .from(employeesTable)
    .where(and(eq(employeesTable.department_id, id), eq(employeesTable.company_id, companyId),
      isNull(employeesTable.deleted_at), eq(employeesTable.employment_status, "active")));
  if ((active[0]?.count ?? 0) > 0) {
    res.status(422).json({ error: "لا يمكن حذف قسم يحتوي على موظفين نشطين" }); return;
  }
...
router.post("/job-titles", wrap(async (req, res) => {
  ...
  const [jt] = await db.insert(jobTitlesTable).values({
    company_id: companyId, name_en: name_en?.trim() ?? name_ar.trim(), name_ar: name_ar.trim(),
  }).returning();
...
router.delete("/job-titles/:id", wrap(async (req, res) => {
  ...
  const active = await db
    .select({ count: sql<number>`COUNT(*)::int` })
    .from(employeesTable)
    .where(and(eq(employeesTable.job_title_id, id), eq(employeesTable.company_id, companyId),
      isNull(employeesTable.deleted_at)));
  if ((active[0]?.count ?? 0) > 0) {
    res.status(422).json({ error: "لا يمكن حذف مسمى وظيفي مرتبط بموظفين" }); return;
  }
...
router.post("/employees", wrap(async (req, res) => {
  ...
  if (!parsed.success) {
    res.status(400).json({ error: parsed.error.issues[0]?.message ?? "بيانات غير صحيحة" }); return;
  }
  ...
  if (data.national_id) {
    ...
    if (dupNid.length > 0) {
      res.status(409).json({ error: "الرقم القومي مسجّل لموظف آخر" }); return;
    }
  }
  ...
  if (dupCode.length > 0) {
    res.status(409).json({ error: "كود الموظف مستخدم بالفعل" }); return;
  }
  ...
  await db.insert(employeeStatusHistoryTable).values({
    employee_id: emp.id, old_status: null, new_status: "active",
    reason: "تسجيل موظف جديد", changed_by: userId,
  });

  await writeAuditLog({
    action: "create", record_type: "employee", record_id: emp.id,
...
router.put("/employees/:id", wrap(async (req, res) => {
  ...
  await writeAuditLog({
    action: "update", record_type: "employee", record_id: id,
    new_value: updates,
    user: { id: userId ?? undefined, username: req.user?.username },
  });
...
router.patch("/employees/:id/status", wrap(async (req, res) => {
  ...
  if (!validStatuses.includes(new_status)) {
    res.status(400).json({ error: `حالة الموظف يجب أن تكون: ${validStatuses.join(" | ")}` }); return;
  }

  const [existing] = await db.select({ employment_status: employeesTable.employment_status })
    .from(employeesTable)
    .where(and(eq(employeesTable.id, id), eq(employeesTable.company_id, companyId), isNull(employeesTable.deleted_at)));
  ...
  await db.insert(employeeStatusHistoryTable).values({
    employee_id: id, old_status: existing.employment_status,
    new_status, reason: reason ?? null, changed_by: userId,
  });
...
router.delete("/employees/:id", wrap(async (req, res) => {
  ...
  const [emp] = await db.select({ id: employeesTable.id, first_name_ar: employeesTable.first_name_ar, last_name_ar: employeesTable.last_name_ar })
    .from(employeesTable)
    .where(and(eq(employeesTable.id, id), eq(employeesTable.company_id, companyId), isNull(employeesTable.deleted_at)));
  ...
  await db.insert(employeeStatusHistoryTable).values({
    employee_id: id, old_status: "active", new_status: "terminated",
    reason: "حذف من النظام", changed_by: userId,
  });

  await writeAuditLog({
    action: "delete", record_type: "employee", record_id: id,
...
router.post("/employees/:id/documents", wrap(async (req, res) => {
  ...
  if (!document_type || !file_name) {
    res.status(400).json({ error: "نوع المستند واسم الملف مطلوبان" }); return;
  }
  const emp = await db.select({ id: employeesTable.id })
    .from(employeesTable)
    .where(and(eq(employeesTable.id, id), eq(employeesTable.company_id, companyId), isNull(employeesTable.deleted_at)));
  ...
  const [doc] = await db.insert(employeeDocumentsTable).values({
    employee_id: id, document_type, file_name, file_path: file_path ?? null,
    expiry_date: expiry_date ?? null, notes: notes ?? null,
  }).returning();
...
router.post("/employees/:id/contacts", wrap(async (req, res) => {
  ...
  const [contact] = await db.insert(employeeContactsTable).values({
    employee_id: id, contact_type: contact_type ?? "emergency",
    name: name.trim(), relationship: relationship ?? null, phone: phone ?? null, email: email ?? null,
  }).returning();
