 ...
 */
router.get("/inventory/reorder-suggestions", wrap(async (req, res) => {
  if (!hasPermission(req.user, "can_view_inventory")) {
    res.status(403).json({ error: "ليس لديك صلاحية عرض المخزون" }); return;
  }
  const companyId = getTenant(req);
  ...
  const suggestions = (rows.rows as Record<string, unknown>[])
    .map((r) => {
      ...
      const minStock = r.min_stock !== null && r.min_stock !== undefined ? Number(r.min_stock) : null;
      ...
      const targetByMin = lowFlag && minStock !== null ? Math.max(minStock * 2 - current, minStock - current) : 0;
      const suggestedQty = Math.max(targetByVelocity - current, targetByMin, 0);
      const cost = Number(r.cost_price ?? 0);
      const reason = lowFlag && trendingOut
