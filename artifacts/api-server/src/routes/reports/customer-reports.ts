...
router.get(
  ...
  wrap(async (req, res) => {
    ...
    SELECT c.id, c.name, c.customer_code,
           COALESCE(SUM(CASE WHEN je.id IS NOT NULL THEN CAST(jel.debit AS FLOAT8) ELSE 0 END), 0)
         - COALESCE(SUM(CASE WHEN je.id IS NOT NULL THEN CAST(jel.credit AS FLOAT8) ELSE 0 END), 0) AS balance
    FROM customers c
  ...
  `);
    for (const r of openRows.rows as Record<string, unknown>[]) {
      rows.push({
        date: String(r.date ?? '1900-01-01'),
        type: 'opening_balance',
        description: String(r.description ?? 'رصيد أول المدة'),
        debit: 0,
  ...
  `);
    for (const r of salesRows.rows as Record<string, unknown>[]) {
      rows.push({
        date: String(r.date),
        type: 'sale',
        description: `فاتورة مبيعات ${r.invoice_no}`,
        debit: Number(r.total_amount),
        credit: 0,
        reference_no: r.invoice_no ? String(r.invoice_no) : undefined,
      });
    }
  ...
  `);
    for (const r of rvRows.rows as Record<string, unknown>[]) {
      rows.push({
        date: String(r.date),
        type: 'receipt',
        description: `سند قبض ${r.voucher_no}`,
        debit: 0,
        credit: Number(r.amount),
        reference_no: r.voucher_no ? String(r.voucher_no) : undefined,
      });
    }
  ...
  `);
    for (const r of retRows.rows as Record<string, unknown>[]) {
      rows.push({
        ...
        reference_no: r.return_no ? String(r.return_no) : undefined,
      });
    }
    ...
    const periodCredits = periodRows
      .filter((r) => r.type !== 'sale' && r.type !== 'opening_balance')
      .reduce((s, r) => s + r.credit, 0);
    const openingRowCredit = periodRows
  ...
  `);
    for (const r of purRows.rows as Record<string, unknown>[]) {
      rows.push({
        date: String(r.date),
        type: 'purchase',
        description: `فاتورة شراء ${r.invoice_no}`,
        debit: 0,
        credit: Number(r.total_amount),
        reference_no: r.invoice_no ? String(r.invoice_no) : undefined,
      });
  ...
  `);
    for (const r of pvRows.rows as Record<string, unknown>[]) {
      rows.push({
        date: String(r.date),
        type: 'payment',
        description: `سند دفع ${r.voucher_no}`,
  ...
  `);
    for (const r of spRows.rows as Record<string, unknown>[]) {
      rows.push({
        ...
        description: String(r.description ?? 'سداد للمورد'),
        debit: Number(r.amount),
        credit: 0,
      });
    }
    ...
    const supPeriodPurchases = periodRows
      .filter((r) => r.type === 'purchase')
      .reduce((s, r) => s + r.credit, 0);
    const supPeriodPayments = periodRows
      .filter((r) => r.type === 'payment' || r.type === 'supplier_payment')
      .reduce((s, r) => s + r.debit, 0);
    const supPeriodReturns = periodRows
      .filter((r) => r.type === 'purchase_return')
      .reduce((s, r) => s + r.debit, 0);
    const supOpeningCredit = periodRows
      .filter((r) => r.type === 'opening_balance')
      .reduce((s, r) => s + r.credit, 0);
    ...
    const supplierValidation = buildValidation([
      {
        ...
        expected:
          r2(openingBalance) +
          r2(supOpeningCredit) +
          r2(supPeriodPurchases) -
          r2(supPeriodPayments) -
          r2(supPeriodReturns),
        actual: r2(runningBalance),
      },
    ]);
...
router.get(
  ...
  wrap(async (req, res) => {
    ...
    const items: AgingItem[] = rows.map((row) => {
      ...
      if (days <= 30) {
        bucket = '0-30';
        buckets['0-30'] += row.remaining;
      } else if (days <= 60) {
        bucket = '31-60';
        buckets['31-60'] += row.remaining;
      } else if (days <= 90) {
        bucket = '61-90';
        buckets['61-90'] += row.remaining;
      } else {
        bucket = '90+';
        buckets['90+'] += row.remaining;
      }
