...
router.get(
  ...
  wrap(async (req, res) => {
    if (!hasPermission(req.user, 'can_view_accounts')) {
      res.status(403).json({ error: 'غير مصرح بعرض التقارير المالية' });
      return;
    }
    ...
    for (const r of custRows.rows as Record<string, unknown>[]) {
      ...
      issues.push({
        id: nextId(),
        group: 'customer_issues',
        type: 'customer_balance',
        severity: sev,
        color: colorOf(sev),
        message: `فارق في رصيد AR — ${r.name}`,
        action:
          'رصيد حساب AR في دفتر الأستاذ لا يطابق مجموع الفواتير المرحّلة — راجع القيود المحاسبية',
        details: {
    ...
    /* ── فحص الربحية ── */
    {
      ...
      issues.push({
        id: nextId(),
        group: 'accounting_issues',
        type: 'profit_equation',
        severity: sev,
        color: colorOf(sev),
        message:
          sev === 'OK'
    ...
    void checkHealthCritical(criticalCount > 0, companyId);
