  ...
  wrap(async (req, res) => {
    ...
    if (!parsed.success) {
      res
        .status(400)
        .json({
          error: 'بيانات التحقق من الجهاز غير صحيحة',
    ...
    if (!companyId) {
      res.status(403).json({ error: 'لا يمكن التحقق من الجهاز بدون سياق شركة واضح' });
      return;
    }
...
router.post(
  ...
  wrap(async (req, res) => {
    ...
    if (!user || !['admin', 'manager', 'super_admin'].includes(user.role ?? '')) {
      res.status(403).json({ error: 'غير مصرح — المدراء فقط' });
      return;
    }
    ...
    if (!parsed.success) {
      res
        .status(400)
        .json({
          error: 'بيانات الموافقة على الجهاز غير صحيحة',
          details: parsed.error.issues.map((i) => i.message),
        });
      return;
    }
    ...
    if (!companyId) {
      res.status(403).json({ error: 'لا يمكن اعتماد الجهاز بدون سياق شركة واضح' });
      return;
    }
    ...
    await db
      .update(erpUsersTable)
      .set({ trusted_device_id: device_id })
      .where(and(eq(erpUsersTable.id, target_user_id), eq(erpUsersTable.company_id, companyId)));
