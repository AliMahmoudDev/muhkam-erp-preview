...
import { db, repairJobsTable, repairJobPartsTable, repairStatusHistoryTable } from '@workspace/db';
...
router.get(
  ...
  wrap(async (req, res) => {
    ...
    if (technician_id && technician_id !== 'all') {
      ...
      conds.push(
        sql`(${repairJobsTable.technician_id} = ${tid} OR ${repairJobsTable.technician_2_id} = ${tid})`
      );
    }
    ...
    if (search?.trim()) {
      ...
      if (/^\d+$/.test(s)) {
        filtered = jobs.filter(
          (j) => j.job_no.endsWith(`-${s}`) || j.job_no.endsWith(`-${s.padStart(4, '0')}`)
        );
      } else {
        filtered = jobs.filter(
          (j) =>
...
router.get(
  ...
  wrap(async (req, res) => {
    ...
    const parts = await db
      ...
      .where(
        and(eq(repairJobPartsTable.job_id, id), eq(repairJobPartsTable.company_id, company_id))
      );
    ...
    return res.json({
      ...
      parts,
      history,
      tracking_token: computeTrackingToken(job.company_id, job.job_no),
    });
  })
);
...
router.post(
  ...
  wrap(async (req, res) => {
    ...
    if (!vj.success)
      return res.status(400).json({ error: vj.error.errors[0]?.message ?? 'بيانات غير صالحة' });
    ...
    const job = await db.transaction(async (tx) => {
      ...
      const [createdJob] = await tx
        ...
        .values({
          ...
          external_workshop_name: b.external_workshop_name
            ? String(b.external_workshop_name)
            : null,
...
router.patch(
  ...
  wrap(async (req, res) => {
    ...
    if ('status' in b && String(b.status) !== existing.status) {
      ...
      if (String(b.status) === 'repaired' || String(b.status) === 'final_quality_check') {
        ...
        const [partsRows, inRepairEntry] = await Promise.all([
          ...
          db
            .select({ created_at: repairStatusHistoryTable.created_at })
            .from(repairStatusHistoryTable)
            .where(
              and(
                eq(repairStatusHistoryTable.job_id, id),
    ...
    if ('responsible_technician_id' in b)
      updates.responsible_technician_id = b.responsible_technician_id
        ? Number(b.responsible_technician_id)
        : null;
    ...
    for (const slot of [
      {
        ...
        prevId: existing.technician_id,
        prevName: existing.technician_name,
        label: 'فني أساسي',
      },
      {
        idKey: 'technician_2_id',
        nameKey: 'technician_2_name',
        prevId: existing.technician_2_id,
        prevName: existing.technician_2_name,
        label: 'فني ثاني',
      },
    ] as const) {
      if (slot.idKey in b) {
        const newId = (b as Record<string, unknown>)[slot.idKey]
          ? Number((b as Record<string, unknown>)[slot.idKey])
          : null;
...
router.delete(
  ...
  wrap(async (req, res) => {
    ...
    const id = Number(req.params.id);
    const [job] = await db
      .select({ id: repairJobsTable.id, job_no: repairJobsTable.job_no })
      .from(repairJobsTable)
      .where(and(eq(repairJobsTable.id, id), eq(repairJobsTable.company_id, company_id)))
      .limit(1);
...
/* ══════════════════════════════════════════════════════════════
   ...
   ══════════════════════════════════════════════════════════════ */
router.get(
  '/repair-jobs/:id/engineer-reports',
  wrap(async (req, res) => {
    if (!hasPermission(req.user, 'can_view_repairs')) {
...
router.post(
  ...
  wrap(async (req, res) => {
    ...
    if (note.length > MAX_ENGINEER_REPORT_LEN)
      return res
        .status(400)
        .json({ error: `نص التقرير لا يجب أن يتجاوز ${MAX_ENGINEER_REPORT_LEN} حرف` });
    ...
    const [row] = await db
      .insert(repairStatusHistoryTable)
      .values({
        job_id,
        company_id,
...
router.delete(
  '/repair-jobs/:id/engineer-reports/:rid',
  wrap(async (req, res) => {
    if (!hasPermission(req.user, 'can_manage_repairs')) {
    ...
    const [existing] = await db
      .select({ id: repairStatusHistoryTable.id })
      .from(repairStatusHistoryTable)
      .where(
        and(
    ...
    await db
      .delete(repairStatusHistoryTable)
      .where(
        and(
          eq(repairStatusHistoryTable.id, rid),
          eq(repairStatusHistoryTable.company_id, company_id),
...
router.post(
  ...
  `);
    if ((dupName.rows as unknown[]).length > 0) {
      ...
      return res
        .status(400)
        .json({ error: `يوجد عميل بنفس الاسم بالفعل: "${d.name}"`, existing: d });
    }
  ...
  `);
    if ((dupPhone.rows as unknown[]).length > 0) {
      ...
      return res
        .status(400)
        .json({ error: `رقم الهاتف مستخدم بالفعل للعميل: "${d.name}"`, existing: d });
    }
    ...
    while (attempts < 5 && !inserted) {
      attempts++;
      const nextCode = await getNextCustomerCode(company_id);
      try {
