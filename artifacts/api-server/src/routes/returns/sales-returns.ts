/**
 * Sales returns — create, list, get, and cancel sale return records.
 */
...
import {
  ...
  saleItemsTable, customerLedgerTable, salesTable,
} from "@workspace/db";
import { nextSaleReturnNo } from "../../lib/invoice-no";
...
router.post("/sales-returns", wrap(async (req, res) => {
  ...
  if (customer_id) {
    ...
    if (alreadyReturnedForCustomer + total > totalSalesForCustomer + 0.01) {
      res.status(400).json({ error: "لا يمكن إرجاع كمية أكبر من المباعة للعميل" }); return;
    }
  }
  ...
  const ret = await db.transaction(async (tx) => {
    ...
    for (const item of items) {
      ...
      const origSaleItemId: number | null = item.original_sale_item_id ? parseInt(item.original_sale_item_id) : null;
      ...
      if (origSaleItemId) {
        ...
        const origItems = await tx.select().from(saleItemsTable)
          .where(and(eq(saleItemsTable.sale_id, parseInt(sale_id)), eq(saleItemsTable.product_id, item.product_id)));
