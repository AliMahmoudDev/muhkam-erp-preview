...
const egyptianPhone = z
  ...
  .trim()
  .regex(/^01[0125]\d{8}$/, 'رقم الهاتف يجب أن يكون 11 رقم مصري صحيح');

/* ─── Auth ───────────────────────────────────────────────────────────────── */
export const loginSchema = z
  .object({
    userId: z
      .number({ invalid_type_error: 'userId يجب أن يكون رقماً' })
      .int()
      .positive('userId يجب أن يكون رقماً موجباً')
      .optional(),
    username: z.string().min(1).max(100).optional(),
    pin: z
      .string({ required_error: 'الرقم السري مطلوب' })
  ...
  .refine(
    (data) =>
      data.userId !== undefined || (data.username !== undefined && data.username.trim().length > 0),
...
export const createSafeSchema = z.object({
  name: z
    .string({ required_error: 'اسم الخزينة مطلوب' })
...
export const stockTransferConfirmSchema = z.object({
  verification_code: z.string({ required_error: 'رمز التحقق مطلوب' }).min(1, 'رمز التحقق مطلوب'),
});
