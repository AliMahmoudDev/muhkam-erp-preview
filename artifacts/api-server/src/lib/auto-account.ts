...
export async function getOrCreateAccount(
  ...
  const selectAccount = async (): Promise<AccountRef | undefined> => {
  ...
  const existing = await selectAccount();
  ...
  const [created] = await db
    ...
    .onConflictDoNothing({ target: [accountsTable.code, accountsTable.company_id] })
    .returning({ id: accountsTable.id, code: accountsTable.code, name: accountsTable.name });

  if (created) return created;

  const createdByConcurrentRequest = await selectAccount();
  if (createdByConcurrentRequest) return createdByConcurrentRequest;

  throw new Error(
    `auto-account.getOrCreateAccount: failed to create or find account ${spec.code} for company ${companyId}`
  );
}
