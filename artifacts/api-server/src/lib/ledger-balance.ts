...
/* ── رصيد عميل واحد (AR) ──────────────────────────────────────────────────── */
export async function getCustomerLedgerBalance(
  ...
  const result = await db.execute(sql`
    ...
    JOIN journal_entries je ON je.id = jel.entry_id AND je.status = 'posted' AND je.company_id = ${companyId}
    JOIN accounts a ON a.id = jel.account_id AND a.company_id = ${companyId}
...
/* ── رصيد مورد واحد (AP) ──────────────────────────────────────────────────── */
export async function getSupplierLedgerBalance(
  accountId: number | null | undefined,
  companyId: number
): Promise<number> {
  if (!accountId) return 0;
