  ...
  await db.insert(journalEntryLinesTable).values([
    {
      ...
      account_code: `${P}-DR`,
      account_name: 'test DR',
      debit: String(debitAmt),
      credit: '0',
    },
    {
      entry_id: entry.id,
...
describe('1 — Journal Entry Balance Enforcement', () => {
  ...
  it('يقبل قيداً متوازناً ويُحدِّث أرصدة الحسابات', async () => {
    ...
    const [dr] = await db
      .select({ bal: accountsTable.current_balance })
      .from(accountsTable)
      .where(eq(accountsTable.id, drId));
    const [cr] = await db
      .select({ bal: accountsTable.current_balance })
      .from(accountsTable)
      .where(eq(accountsTable.id, crId));
...
describe('2 — Account Balance Drift Detection & Repair', () => {
  ...
  it('setup: ينشئ حساباً وقيداً متوازناً', async () => {
    ...
    const [entry] = await db
      .select({ id: journalEntriesTable.id })
      .from(journalEntriesTable)
      .where(eq(journalEntriesTable.reference, `${P}-DRIFT`));
    if (entry) createdJournalEntryIds.push(entry.id);
  ...
  it('يكشف الانحراف بعد تعديل الرصيد المُخزَّن يدوياً', async () => {
    ...
    const check = await checkAccountBalanceDrift(1);
  ...
  it('يُصلح الانحراف بعد تشغيل repairAccountBalances', async () => {
    const result = await repairAccountBalances(1);
...
describe('3 — Customer Balance Drift Detection & Repair', () => {
  ...
  it('يكشف الانحراف: customers.balance ≠ SUM(customer_ledger.amount)', async () => {
    ...
    const check = await checkCustomerBalanceDrift(1);
  ...
  it('يُصلح الانحراف: يعيد حساب balance من customer_ledger', async () => {
    const result = await repairCustomerBalances(1);
    assert.ok(result.repaired >= 1, 'يجب إصلاح عميل واحد على الأقل');

    const [row] = await db
      .select({ bal: customersTable.balance })
      .from(customersTable)
      .where(eq(customersTable.id, custId));
    assert.strictEqual(
      Number(row.bal),
      0,
      'يجب أن يُصبح الرصيد 0 بعد الإصلاح (لا توجد سجلات دفتر أستاذ)'
    );
  });
  ...
  it('لا يوجد انحراف بعد إضافة سجل customer_ledger مطابق', async () => {
    ...
    const check2 = await checkCustomerBalanceDrift(1);
...
describe('4 — Inventory Drift Detection', () => {
  ...
  it('يكشف الانحراف: products.quantity (10) ≠ SUM(stock_movements) (0)', async () => {
    ...
    const check = await checkInventoryDrift(1);
...
describe('5 — Expense creates correct journal entry', () => {
  ...
  it('إنشاء قيد مصروفات يدوياً والتحقق من التوجيه المحاسبي الصحيح', async () => {
    ...
    const [entry] = await db
      .select({ id: journalEntriesTable.id })
      .from(journalEntriesTable)
      .where(eq(journalEntriesTable.reference, expRef));
    assert.ok(entry, 'يجب وجود قيد يومية بالمرجع المحدد');
    createdJournalEntryIds.push(entry.id);

    /* التحقق من اتجاه القيد */
    const [expBal] = await db
      .select({ bal: accountsTable.current_balance })
      .from(accountsTable)
      .where(eq(accountsTable.id, expAcctId));
    const [safeBal] = await db
      .select({ bal: accountsTable.current_balance })
      .from(accountsTable)
      .where(eq(accountsTable.id, safeAcctId));
...
describe('6 — Reversal creates opposite accounting effect', () => {
  ...
  it('القيد الأصلي والعكسي يصفّيان بعضهما تماماً', async () => {
    ...
    const [orig] = await db
      .select({ id: journalEntriesTable.id })
      .from(journalEntriesTable)
      .where(eq(journalEntriesTable.reference, origRef));
    ...
    const [rev] = await db
      .select({ id: journalEntriesTable.id })
      .from(journalEntriesTable)
      .where(eq(journalEntriesTable.reference, revRef));
    ...
    const [bal1] = await db
      .select({ bal: accountsTable.current_balance })
      .from(accountsTable)
      .where(eq(accountsTable.id, a1Id));
    const [bal2] = await db
      .select({ bal: accountsTable.current_balance })
      .from(accountsTable)
      .where(eq(accountsTable.id, a2Id));
...
describe('7 — Detect manually-inserted imbalanced journal entries', () => {
  ...
  it('يكشف قيداً مُدرجاً يدوياً بـ debit ≠ credit في الجدول', async () => {
    ...
    const check = await checkJournalEntryBalance(1);
...
describe('8 — COGS reversal JE on sale_return nets to zero', () => {
  ...
  it('قيد بيع COGS + قيد عكسي مرتجع يُصفّيان الحسابين إلى صفر', async () => {
    ...
    const [saleJE] = await db
      .select({ id: journalEntriesTable.id })
      .from(journalEntriesTable)
      .where(eq(journalEntriesTable.reference, saleRef));
    if (saleJE) createdJournalEntryIds.push(saleJE.id);

    /* التحقق المرحلي: بعد البيع، COGS مدين والمخزون دائن */
    const [invMid] = await db
      .select({ bal: accountsTable.current_balance })
      .from(accountsTable)
      .where(eq(accountsTable.id, invAcctId));
    const [cogsMid] = await db
      .select({ bal: accountsTable.current_balance })
      .from(accountsTable)
      .where(eq(accountsTable.id, cogsAcctId));
    assert.strictEqual(
      Number(invMid.bal),
      -COGS_AMT,
      'ASSET-INVENTORY يجب أن يكون سالباً بعد قيد البيع'
    );
    ...
    const [retJE] = await db
      .select({ id: journalEntriesTable.id })
      .from(journalEntriesTable)
      .where(eq(journalEntriesTable.reference, retRef));
    if (retJE) createdJournalEntryIds.push(retJE.id);

    /* التحقق النهائي: كلا الحسابين = 0 بعد البيع + المرتجع */
    const [invFinal] = await db
      .select({ bal: accountsTable.current_balance })
      .from(accountsTable)
      .where(eq(accountsTable.id, invAcctId));
    const [cogsFinal] = await db
      .select({ bal: accountsTable.current_balance })
      .from(accountsTable)
      .where(eq(accountsTable.id, cogsAcctId));
    ...
    const drift = await checkAccountBalanceDrift(1);
...
describe('9 — Stock Count: creates correct adjustments and passes integrity', () => {
  ...
  it('checkInventoryDrift لا يكتشف انحرافاً بعد التطبيق', async () => {
    const drift = await checkInventoryDrift(1);
...
describe('10 — Stock Transfer: correct movements, integrity, and validations', () => {
  ...
  it('setup: ينشئ منتجاً بكمية 20 مع حركة مخزنية في WH1', async () => {
    ...
    await db.insert(stockMovementsTable).values({
      ...
      quantity: String(INIT_QTY),
      quantity_before: '0',
      quantity_after: String(INIT_QTY),
      unit_cost: '30',
      date: '2025-01-01',
      warehouse_id: WH_SRC,
      company_id: 1,
  ...
  it('التحويل يُنشئ حركتَي transfer_out (WH1) و transfer_in (WH2)', async () => {
    ...
    await db.insert(stockMovementsTable).values({
      ...
      quantity: String(-XFER_QTY),
      quantity_before: String(INIT_QTY),
      quantity_after: String(INIT_QTY),
      unit_cost: '30',
  ...
  it('الكمية الإجمالية للمنتج لا تتغير بعد التحويل (transfer_in + transfer_out = صفر)', async () => {
    const [result] = await db
      .select({ net: sql<string>`SUM(${stockMovementsTable.quantity})` })
      .from(stockMovementsTable)
      .where(eq(stockMovementsTable.product_id, prodId2));
  ...
  it('لا يجوز التحويل إذا كانت الكمية المطلوبة أكبر من المتاح', async () => {
    ...
    const [prod] = await db
      .select({ qty: productsTable.quantity })
      .from(productsTable)
      .where(eq(productsTable.id, prodId2));
...
after(async () => {
  ...
  if (createdAccountIds.length > 0) {
    await db.delete(accountsTable).where(inArray(accountsTable.id, createdAccountIds));
  }
  /* حذف سجلات دفتر أستاذ العملاء */
  if (createdCustomerIds.length > 0) {
    await db
      .delete(customerLedgerTable)
  ...
  if (createdTransferIds.length > 0) {
    ...
    await db.delete(stockTransfersTable).where(inArray(stockTransfersTable.id, createdTransferIds));
  }
