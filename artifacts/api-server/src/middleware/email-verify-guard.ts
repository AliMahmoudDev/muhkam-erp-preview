 ...
 *
 * ── What it does ─────────────────────────────────────────────────────────────
 *  For trial companies where verification_status = 'expired':
...
async function getVerificationStatus(companyId: number): Promise<{ emailVerified: boolean; verificationStatus: string }> {
  const now    = Date.now();
...
export async function emailVerifyGuard(req: Request, res: Response, next: NextFunction): Promise<void> {
  ...
  try {
    const { emailVerified, verificationStatus } = await getVerificationStatus(user.company_id);

    if (!emailVerified && verificationStatus === "expired") {
      logger.warn(
        { companyId: user.company_id, method: req.method, url: req.url },
        "[EmailVerifyGuard] Write blocked — email verification expired"
      );

      res.status(403).json({
        error: "يجب التحقق من البريد الإلكتروني قبل إجراء أي تعديلات — يرجى التواصل مع الدعم لإعادة إرسال رابط التحقق",
    ...
    logger.error({ err, companyId: user.company_id }, "[EmailVerifyGuard] Error checking verification — failing open");
    next();
  }
}
