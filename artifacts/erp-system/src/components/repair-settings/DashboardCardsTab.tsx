        ...
        <div className="flex items-start gap-3 flex-1 min-w-0">
          <div className="flex items-start gap-3 p-3 rounded-xl bg-amber-500/8 border border-amber-500/20 flex-1">
            ...
            <p className="text-[12px] text-amber-300/70 leading-relaxed">
              كل كارت يضمّ حالة واحدة أو أكثر، يُعرض أعلى صفحة الصيانة بحجم نسبي حسب عدد البطاقات.
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-1">
                    ...
                    {c.is_system && (
                      <span className="text-[9px] px-1.5 py-0.5 rounded-full bg-surface text-ink/40 font-medium">
                        افتراضي
                      </span>
                    )}
                  ...
                  <div className="flex flex-wrap gap-1">
                    {c.statuses.slice(0, 5).map((s) => (
                      <span
                        key={s}
                        className="text-[10px] px-1.5 py-0.5 rounded-md bg-surface text-ink/45 font-mono"
                      >
                        {s}
                      </span>
                    ))}
                    {c.statuses.length > 5 && (
                      <span className="text-[10px] px-1.5 py-0.5 rounded-md bg-surface text-ink/35">
                        +{c.statuses.length - 5}
                      </span>
                    )}
                  </div>
                ...
                <div className="flex items-center gap-1 shrink-0">
                  ...
                  <button
                    onClick={() => remove(c.id)}
                    disabled={busyId === c.id || cards.length <= 1}
...
function DashboardCardEditor({
  ...
  const [alertThreshold, setAlertThreshold] = useState<string>(
    initial?.alert_threshold?.toString() ?? ''
  );
  ...
  const createStatus = async () => {
    ...
    if (!label) {
      toast({ title: 'اكتب اسم الحالة', variant: 'destructive' });
      return;
    }
    ...
    try {
      ...
      toast({
        title: e instanceof Error ? e.message : 'تعذر إضافة الحالة',
        variant: 'destructive',
      });
    } finally {
  ...
  const save = async () => {
    ...
    if (statuses.length === 0) {
      toast({ title: 'اختَر حالة واحدة على الأقل', variant: 'destructive' });
      return;
    }
    setSaving(true);
    try {
  ...
  return createPortal(
    <div
      className="fixed inset-0 z-[70] flex items-start justify-center pt-8 pb-8 bg-black/70 backdrop-blur-md"
      dir="rtl"
      onClick={(e) => {
    ...
    >
      <div
        className="glass-panel rounded-2xl border border-line w-full mx-4 overflow-hidden flex flex-col"
      ...
      >
        ...
        <div className="flex-1 overflow-y-auto p-5 space-y-5">
          {/* Live preview */}
          <div
            className="rounded-2xl p-3 border bg-gradient-to-br to-transparent flex flex-col gap-1"
            style={{
                  ...
                  {companyStatuses.map((s) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        {s.label_ar}
                        {!s.is_system && (
                          <span className="text-[8px] px-1 rounded bg-amber-500/15 text-amber-300/80 font-bold">
                            مخصص
                          </span>
                        )}
          ...
          <div>
            ...
            <input
              ...
              onChange={(e) => setAlertThreshold(e.target.value.replace(/[^\d]/g, ''))}
              placeholder="مثال: 5"
              inputMode="numeric"
              className="erp-input w-full text-sm"
            />
            <p className="text-[10px] text-ink/30 mt-1">
              يتغيّر شكل الكارت لتنبيه بصري عند بلوغ هذا الحد
            </p>
          </div>
        </div>
