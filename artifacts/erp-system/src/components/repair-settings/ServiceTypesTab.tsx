...
const COMMISSION_TYPE_LABELS: Record<string, { label: string; hint: string; suffix: string }> = {
  profit_based: {
    label: '% من الربح الصافي',
    hint: 'نسبة من (مبلغ الخدمة − تكلفة قطعها)',
    suffix: '%',
  },
...
const EMPTY_FORM = {
  name_ar: '',
  commission_type: 'profit_based' as ServiceType['commission_type'],
  commission_value: '0',
  is_active: true,
};
...
export default function ServiceTypesTab() {
  ...
  const { data: types = [], isLoading } = useQuery<ServiceType[]>({
    ...
    queryFn: async () => {
      const r = await authFetch(api('/api/repair-service-types'));
      const data: unknown = await r.json();
      return Array.isArray(data) ? (data as ServiceType[]) : [];
    },
    staleTime: 10_000,
  });
  ...
  const deleteMutation = useMutation({
    mutationFn: (id: number) =>
      authFetch(api(`/api/repair-service-types/${id}`), { method: 'DELETE' }).then(async (r) => {
        if (!r.ok) throw new Error((await r.json()).error);
      }),
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ['/api/repair-service-types'] });
          ...
          <div className="grid grid-cols-3 gap-1.5">
            {(
              Object.entries(COMMISSION_TYPE_LABELS) as [
                ServiceType['commission_type'],
                (typeof COMMISSION_TYPE_LABELS)[string],
              ][]
            ).map(([key, meta]) => (
              <button
                key={key}
  ...
  return (
    <div className="flex flex-col h-full overflow-hidden" dir="rtl">
      ...
      <div className="px-6 pt-5 pb-4 shrink-0 border-b border-line">
        ...
        <div className="mt-3 flex items-start gap-2 px-3 py-2 rounded-xl bg-amber-500/8 border border-amber-500/15">
          ...
          <p className="text-[10px] text-amber-300/60 leading-relaxed">
            قيم الكوميشن تظهر في هذه الإعدادات فقط — لا تظهر للفنيين. الكوميشن يُحسب ويُجمَّد
            تلقائياً عند تسليم البطاقة (المرحلة الثانية).
          </p>
        </div>
      </div>
      ...
      <div className="flex-1 overflow-y-auto px-6 py-4 space-y-2">
        ...
        {/* قائمة الأنواع */}
        {isLoading && <div className="text-center py-10 text-ink/30 text-sm">جارٍ التحميل...</div>}
