      ...
      <div className="p-5 space-y-5">
        <div className="flex items-start gap-3 p-4 rounded-xl bg-green-500/8 border border-green-500/20">
          ...
          <div>
            ...
            <p className="text-[12px] text-green-300/50 leading-relaxed">
              عدّل نص الرسائل التي تُرسل للعميل من بطاقة الصيانة. استخدم المتغيّرات أدناه وستُستبدل
              تلقائياً.
            </p>
          </div>
        </div>
        ...
        <div className="rounded-xl border border-line overflow-hidden bg-surface">
          <div className="px-4 py-2 border-b border-line">
            <span className="text-[11px] font-bold text-ink/40 tracking-widest uppercase">
              المتغيّرات المتاحة
            </span>
          </div>
        ...
        <div className="rounded-xl border border-line overflow-hidden">
          <div className="px-4 py-3 bg-surface border-b border-line flex items-center justify-between">
            <span className="text-[12px] font-semibold text-ink/50">
              رسالة "الجهاز جاهز للاستلام"
            </span>
            <button
              onClick={() => resetTo('ready')}
          ...
          <textarea
            value={readyBuf}
            onChange={(e) => setReadyBuf(e.target.value)}
            rows={6}
            dir="rtl"
        ...
        <div className="rounded-xl border border-line overflow-hidden">
          ...
          <textarea
            value={progressBuf}
            onChange={(e) => setProgressBuf(e.target.value)}
            rows={6}
            dir="rtl"
            className="erp-input w-full text-sm py-2.5 leading-relaxed font-sans border-0 rounded-none bg-transparent"
          />
