...
export function OnboardingPanel() {
  ...
  const { data: salesRaw, isLoading: loadingSales } = useQuery<unknown[]>({
    queryKey: ['/api/sales'],
    queryFn: () =>
      authFetch(api('/api/sales')).then((r) => {
  ...
  const addWarehouse = useMutation({
    mutationFn: () =>
      authFetch(api('/api/settings/warehouses'), {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
  ...
  return (
    <>
          ...
          <div className="glass-panel rounded-3xl p-7 w-full max-w-xs border border-line shadow-2xl space-y-5">
            ...
            <div>
              ...
              <input
                ...
                onKeyDown={(e) =>
                  e.key === 'Enter' && warehouseName.trim() && addWarehouse.mutate()
                }
              />
            </div>
            <div className="flex gap-3">
              <button
                onClick={() => addWarehouse.mutate()}
      ...
      <div
        ...
        style={{
          background:
            'linear-gradient(135deg, rgba(99,102,241,0.08), rgba(168,85,247,0.06), rgba(255,255,255,0.02))',
        }}
      >
        <button
                  ...
                  <div className="flex items-center gap-2.5 mb-3">
                    <div
                      className={`w-8 h-8 rounded-xl flex items-center justify-center shrink-0 ${
                        step.done ? 'bg-emerald-500/20' : 'bg-surface'
                      }`}
                    >
                      {step.done ? (
                    ...
                    <div className="flex-1 min-w-0">
                      <span
                        className={`text-xs font-bold ${step.done ? 'text-emerald-300' : 'text-ink/70'}`}
                      >
                        الخطوة {step.num}
                      </span>
                      <p
                        className={`text-sm font-black leading-tight ${step.done ? 'text-emerald-200 line-through opacity-60' : 'text-ink'}`}
                      >
                        {step.label}
                      </p>
                    </div>
