...
export default function ReadyForDeliveryModal({
  ...
  useEffect(() => {
    ...
    authFetch(api(`/api/repair-checklist-items?device_type=${encodeURIComponent(deviceType)}`))
      ...
      .catch(() => {
        /* نبقى على القالب الافتراضي المحدد مسبقاً */
      })
  ...
  async function handleQcApprove() {
    if (items.length === 0) {
      setQcErrors(['لا توجد بنود فحص']);
      return;
    }
    if (!allDecided) {
      setQcErrors([`متبقي ${pendingCount} بند`]);
      return;
    }
    if (failCount > 0) {
      setQcErrors([`لا يمكن القبول ووجود ${failCount} بند مرفوض`]);
      return;
    }
    setQcLoading(true);
    setQcErrors([]);
  ...
  async function handleQcReject() {
    if (rejectReason.trim().length < 3) {
      setQcErrors(['يجب كتابة سبب رفض (3 أحرف على الأقل)']);
      return;
    }
    setQcLoading(true);
    setQcErrors([]);
    try {
      ...
      toast({
        title: '⚠ رُفض الفحص',
        description: 'البطاقة أُعيدت لـ "جارٍ الإصلاح"',
        variant: 'destructive',
      });
      onRejected();
    } catch {
  ...
  const [selectedProduct, setSelectedProduct] = useState<
    import('./ready-for-delivery/types').Product | null
  >(null);
          ...
          <div className="flex items-center gap-2.5">
            <div
              ...
              style={{
                ...
                border:
                  phase === 'qc'
                    ? '1px solid rgba(196,132,252,0.3)'
              ...
              <div className="flex items-center gap-2">
                ...
                {!qcOnly && (
                  <span className="flex items-center gap-1 text-[10px] text-ink/40">
                    ...
                    <span
                      className={`px-1.5 py-0.5 rounded-full text-[9px] font-bold ${phase === 'billing' ? 'bg-lime-500/20 text-lime-300' : 'bg-surface text-ink/40'}`}
                    >
                      ٢
                    </span>
                  </span>
