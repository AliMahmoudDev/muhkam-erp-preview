...
export default function RepairReturnModal({
  ...
  async function handleSubmit(e: React.FormEvent) {
    e.preventDefault();
    if (!problemDesc.trim()) {
      setError('يرجى وصف سبب الإرجاع');
      return;
    }
    const amount = Number(refundAmount);
    if (isNaN(amount) || amount < 0) {
      setError('المبلغ المسترد غير صحيح');
      return;
    }
    if (amount > 0 && !safeId) {
      setError('يرجى اختيار الخزنة لخصم المبلغ المسترد منها');
      return;
    }
          ...
          <div className="flex items-center gap-2.5">
            ...
            <div>
              ...
              <p className="text-[10px] text-ink/40">
                بطاقة صيانة {jobNo} · {customerName}
              </p>
            </div>
          </div>
          {!done && (
            ...
            <div className="overflow-y-auto p-5 space-y-4">
              ...
              <div>
                <label className="block text-[11px] font-bold text-ink/60 mb-1.5">
                  المبلغ المُسترد <span className="text-red-400">*</span>
                </label>
              ...
              <div>
                <label className="block text-[11px] font-bold text-ink/60 mb-1.5">
                  سبب الإرجاع / وصف المشكلة <span className="text-red-400">*</span>
                </label>
                <textarea
                  value={problemDesc}
                        ...
                        >
                          <div className="flex items-start justify-between gap-2 mb-2.5">
                            <div>
                              ...
                              <p className="text-[10px] text-ink/40">
                                الكمية: {Number(part.quantity)} · السعر:{' '}
                                {Number(part.unit_price).toFixed(2)} ج.م
                              </p>
                            </div>
                          </div>
                          ...
                          {!part.warehouse_id && disp === 'stock' && (
                            <p className="text-[9px] text-amber-400/80 mt-1">
                              ⚠ هذه القطعة بدون مخزن محدد — لن تُعاد للمخزن
                            </p>
                          )}
                        </div>
                      );
              ...
              {error && (
                <div
                  className="flex items-center gap-1.5 p-2.5 rounded-xl"
                  style={{
            ...
            <div className="flex gap-2 p-5 pt-0 shrink-0">
              ...
              >
                {loading ? (
                  ...
                  <>
                    <RotateCcw className="w-3.5 h-3.5" /> تسجيل المرتجع
                  </>
                )}
              </button>
              <button
