  ...
  function handlePrint() {
    ...
    const esc = (v: unknown): string =>
      String(v ?? '')
        ...
        .replace(/>/g, '&gt;');
    const deviceLine =
      [g.receiptData.device_brand, g.receiptData.device_model].filter(Boolean).map(esc).join(' ') ||
      '—';
    const allParts = [
      ...g.preSavedParts.map((p) => ({
        name: p.product_name,
        qty: p.quantity,
        price: p.unit_price,
        net: p.total,
...
<script>window.addEventListener('load',()=>{window.print();setTimeout(()=>window.close(),1000);});</script>
  ...
  function handleWhatsapp() {
    if (!g.receiptData?.customer_phone) {
    ...
    if (hasParts) {
      ...
      for (const p of g.preSavedParts) {
        partsBlock.push(
          `• ${p.product_name} — ${p.quantity} × ${fmt(p.unit_price)} = ${fmt(p.total)}`
        );
      }
    ...
    const lines = [
      ...
      `رقم البطاقة: ${g.receiptData.job_no}`,
      ...
      `المدفوع مقدماً: ${fmt(g.dep)}`,
      `*المتبقي: ${fmt(g.totalRem)} ج.م*`,
      ``,
      `شكراً لتعاملكم معنا 🙏`,
    ]
      .filter(Boolean)
  ...
  return createPortal(
    ...
    >
      ...
      >
        ...
        {g.draftRestored && (
          <div
            className="mx-5 mt-3 p-2.5 rounded-xl flex items-center justify-between gap-2"
            style={{
          ...
          >
            <span className="text-[11px] text-blue-200">
              تم استعادة بيانات فاتورة محفوظة محلياً لهذه البطاقة.
            </span>
            <button
              type="button"
        ...
        {/* Loading / Error */}
        {g.fetchLoading && (
          <div className="flex items-center justify-center gap-2 text-ink/60 text-xs py-8">
        ...
        {!g.fetchLoading && !g.fetchErr && (
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-0 divide-y lg:divide-y-0 lg:divide-x lg:divide-x-reverse divide-white/5">
            ...
            <DeliveryGateForm
              ...
              selectedWarehouseId={g.selectedWarehouseId}
              setSelectedWarehouseId={g.setSelectedWarehouseId}
              filteredProducts={g.filteredProducts}
              productSearch={g.productSearch}
              setProductSearch={g.setProductSearch}
              showProductDrop={g.showProductDrop}
              setShowProductDrop={g.setShowProductDrop}
              productSearchRef={g.productSearchRef}
              addQty={g.addQty}
              setAddQty={g.setAddQty}
              addPrice={g.addPrice}
              setAddPrice={g.setAddPrice}
              ...
              setPartLines={g.setPartLines}
              selectProduct={g.selectProduct}
              addPartLine={g.addPartLine}
              updateLineDiscount={g.updateLineDiscount}
              showExtForm={g.showExtForm}
              setShowExtForm={g.setShowExtForm}
              extVendor={g.extVendor}
              setExtVendor={g.setExtVendor}
              extDesc={g.extDesc}
              setExtDesc={g.setExtDesc}
              extPrice={g.extPrice}
              setExtPrice={g.setExtPrice}
              extVendorCost={g.extVendorCost}
              setExtVendorCost={g.setExtVendorCost}
              extVendorPayType={g.extVendorPayType}
              setExtVendorPayType={g.setExtVendorPayType}
              extVendorSafeId={g.extVendorSafeId}
              setExtVendorSafeId={g.setExtVendorSafeId}
              addExternalLine={g.addExternalLine}
              safes={g.safes}
              payRows={g.payRows}
              setPayRows={g.setPayRows}
              payType={g.payType}
              setPayType={g.setPayType}
              paySafe={g.paySafe}
              setPaySafe={g.setPaySafe}
              payAmount={g.payAmount}
              setPayAmount={g.setPayAmount}
              paidSoFar={g.paidSoFar}
              grandTotal={g.grandTotal}
              remaining={g.remaining}
              payIsDone={g.payIsDone}
              addPayRow={g.addPayRow}
              fillAll={g.fillAll}
              brokerName={g.brokerName}
              setBrokerName={g.setBrokerName}
              brokerComm={g.brokerComm}
              setBrokerComm={g.setBrokerComm}
            />
            ...
            <div className="overflow-y-auto max-h-[65vh] flex flex-col">
                ...
                {g.receiptData && (
                  <div className="space-y-1 text-[11px]">
                    {[
                      ...
                      {
                        label: 'الجهاز',
                        val: [g.receiptData.device_brand, g.receiptData.device_model]
                          .filter(Boolean)
                          .join(' '),
                      },
                ...
                <div className="grid grid-cols-2 gap-3">
                  ...
                  <div>
                    ...
                    <input
                      ...
                      step="any"
                      value={g.cost}
                      onChange={(e) => g.setCost(e.target.value)}
                      placeholder="0.00"
                      dir="ltr"
              ...
              <div className="px-5 py-4 space-y-2 flex-1">
                  ...
                  {g.preSavedPartsTotal > 0 && (
                    <div className="flex justify-between items-center">
                      ...
                      <span className="font-bold text-amber-300">
                        {fmtCurrency(g.preSavedPartsTotal)}
                      </span>
                    </div>
                  )}
                  {g.partsTotal > 0 && (
                ...
                <div
                  ...
                  style={{
                    background: g.totalRem > 0 ? 'rgba(245,158,11,0.10)' : 'rgba(16,185,129,0.10)',
                    border: `1px solid ${g.totalRem > 0 ? 'rgba(245,158,11,0.3)' : 'rgba(52,211,153,0.3)'}`,
                  }}
                >
                  <span className="text-[12px] font-black text-ink">المتبقي على العميل:</span>
                  <span
                    className={`text-[15px] font-black ${g.totalRem > 0 ? 'text-amber-300' : 'text-emerald-300'}`}
                  >
        ...
        <div className="px-5 py-4 border-t border-line space-y-2">
          <div className="grid grid-cols-2 gap-2">
            <button
              ...
              disabled={!g.receiptData || !g.receiptData.customer_phone}
          ...
          <div className="flex gap-2">
            <button
              ...
              disabled={g.saving || !g.receiptData}
            ...
            >
              {g.saving ? (
                ...
                <>
                  <Save className="w-3.5 h-3.5" /> حفظ مؤقت
                </>
              )}
            </button>
            <button
              onClick={() => void g.handleConfirm()}
              disabled={g.saving || !g.receiptData}
              className="flex-1 py-2.5 rounded-xl text-ink text-xs font-bold transition-all disabled:opacity-40 flex items-center justify-center gap-1.5"
              style={{
