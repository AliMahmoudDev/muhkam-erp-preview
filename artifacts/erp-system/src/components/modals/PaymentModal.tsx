...
export default function PaymentModal({ onClose }: Props) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (!form.customer_id) {
      toast({ title: 'اختر العميل / المورد', variant: 'destructive' });
      return;
    }
    if (!form.safe_id) {
    ...
    if (!amount || amount <= 0) {
      toast({ title: 'أدخل مبلغاً أكبر من صفر', variant: 'destructive' });
      return;
    }
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/70 backdrop-blur-sm">
      <form
        ...
        className="relative w-full max-w-sm rounded-3xl p-7 space-y-4 shadow-2xl border border-orange-500/30 bg-[var(--erp-bg-card)]"
      >
        <button
          type="button"
          onClick={onClose}
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-ink/30 text-xs">صرف مبلغ من الخزينة</p>
          </div>
        </div>

        <div>
          <label className="block text-ink/50 text-xs mb-1.5 font-medium">العميل / المورد *</label>
          <SearchableSelect
            items={customerItems}
            value={form.customer_id}
            onChange={(v) => setForm((f) => ({ ...f, customer_id: v }))}
            placeholder="ابحث باسم أو كود..."
            emptyLabel="-- اختر العميل / المورد --"
            clearable={false}
          />
        </div>
