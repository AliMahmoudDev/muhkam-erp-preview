...
export default function PreDeliveryModal({ job, parts: partsProp, onClose, onSaved }: Props) {
  ...
  async function handleSave() {
    ...
    try {
      ...
      const settled = isExternal
        ? 0
        : internalUnreturned.filter((p) => decisions[p.id] !== 'defer').length;
      toast({
        title: 'تمت مراجعة ما قبل التسليم',
        description: isExternal
          ? `ورشة خارجية — تكلفة ${wcost.toFixed(2)}`
          : settled > 0
            ? `تم التعامل مع ${settled} قطعة`
            : 'تم تأكيد المراجعة',
      });
      onSaved();
    } catch (e) {
    ...
    >
      ...
      >
          ...
          <div className="flex items-center gap-2.5">
            ...
            <div>
              ...
              <p className="text-[11px] text-ink/50">
                البطاقة <span className="text-ink font-bold">{job.job_no}</span> — قبل الانتقال إلى
                "جاهز للتسليم"
              </p>
            </div>
          </div>
        ...
        <div className="px-5 py-4 max-h-[60vh] overflow-y-auto space-y-4">
                ...
                <div className="flex-1 min-w-0">
                  <p
                    className={[
                      ...
                      !isExternal ? 'text-emerald-200' : 'text-ink/70',
                    ].join(' ')}
                  >
                    ورشة داخلية
                  </p>
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  <p
                    className={[
                      'text-[12px] font-black',
                      isExternal ? 'text-cyan-200' : 'text-ink/70',
                    ].join(' ')}
                  >
                    ورشة خارجية
                  </p>
          ...
          {isExternal && (
            <section
              className="rounded-xl border border-cyan-400/25 p-3"
                        ...
                        {(['stock', 'scrap', 'defer'] as Decision[]).map((d) => {
                          ...
                          return (
                            <button
                              ...
                              title={
                                disabled ? 'هذه القطعة غير مرتبطة بصنف/مخزن — لا يمكن إرجاعها' : ''
                              }
                              className={[
          ...
          <section
            className="rounded-xl border border-amber-400/20 p-3"
          ...
          >
            ...
            <p className="text-[10px] text-amber-200/70 mt-2">
              عمولة الوسيط تُخصم من الإيراد الصافي للفنّي عند احتساب الراتب — لا تُسجَّل كمصروف
              منفصل.
            </p>
          </section>
        </div>
