...
const rowTypeConfig: Record<
  string,
  { label: string; icon: string; credit_color: string; debit_color: string }
> = {
  opening_balance: {
    label: 'رصيد أول المدة',
    icon: '⚖',
    credit_color: 'text-amber-400',
    debit_color: 'text-amber-400',
  },
  sale: {
    label: 'فاتورة مبيعات',
    icon: '🛍',
    credit_color: 'text-emerald-400',
    debit_color: 'text-white/50',
  },
  sale_return: {
    label: 'مرتجع مبيعات',
    icon: '↩',
    credit_color: 'text-white/50',
    debit_color: 'text-orange-400',
  },
  purchase: {
    label: 'فاتورة شراء',
    icon: '📦',
    credit_color: 'text-red-400',
    debit_color: 'text-white/50',
  },
  purchase_return: {
    label: 'مرتجع مشتريات',
    icon: '↪',
    credit_color: 'text-white/50',
    debit_color: 'text-blue-400',
  },
  receipt_voucher: {
    label: 'سند قبض',
    icon: '💰',
    credit_color: 'text-white/50',
    debit_color: 'text-emerald-400',
  },
  payment_voucher: {
    label: 'سند صرف',
    icon: '💸',
    credit_color: 'text-orange-400',
    debit_color: 'text-white/50',
  },
  supplier_payment: {
    label: 'تسديد دفعة',
  ...
  const { data, isLoading, isError } = useQuery<FullStatementResponse>({
    ...
    queryFn: async () => {
      const res = await authFetch(
        api(`/api/contacts/${contactId}/full-statement?type=${contactType}`),
        {
  ...
  const handlePrint = () => {
    ...
    const rows = filtered
      .map((r, i) => {
        ...
        <td style="padding:6px;border:1px solid #dee2e6;text-align:center;color:#28a745">${r.debit > 0 ? r.debit.toLocaleString('ar-EG-u-nu-latn', { minimumFractionDigits: 2 }) : '—'}</td>
        <td style="padding:6px;border:1px solid #dee2e6;text-align:center;color:#dc3545">${r.credit > 0 ? r.credit.toLocaleString('ar-EG-u-nu-latn', { minimumFractionDigits: 2 }) : '—'}</td>
        <td style="padding:6px;border:1px solid #dee2e6;text-align:center;font-weight:bold;color:${r.balance >= 0 ? '#856404' : '#0c5460'}">
          ${r.balance !== 0 ? `${Math.abs(r.balance).toLocaleString('ar-EG-u-nu-latn', { minimumFractionDigits: 2 })} ${r.balance > 0 ? 'عليه' : 'له'}` : 'صفر'}
        </td>
      </tr>`;
      })
      .join('');
    ...
    doc.write(`<!DOCTYPE html><html dir="rtl"><head><meta charset="utf-8">
      ...
      <p>الرصيد النهائي: <strong>${Math.abs(closingBalance).toLocaleString('ar-EG-u-nu-latn', { minimumFractionDigits: 2 })} ${closingBalance >= 0 ? 'عليه لنا' : 'له علينا'}</strong></p>
      ...
      <div className="glass-panel rounded-3xl w-full max-w-5xl max-h-[92vh] flex flex-col border border-white/10 shadow-2xl overflow-hidden">
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-white/10 bg-white/5 shrink-0">
          <div>
            ...
            {contact && (
              <p className="text-white/50 text-sm mt-0.5">
                ...
                <span
                  className={`mr-2 text-xs px-2 py-0.5 rounded-full border font-bold
                  ${
                    contact.type === 'both'
          ...
          <div className="flex items-center gap-2">
            <button
              onClick={handlePrint}
              className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-white/10 hover:bg-white/20 text-white/70 text-sm transition-colors"
            >
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 px-6 py-4 shrink-0 border-b border-white/10">
              <div className="glass-panel rounded-2xl px-4 py-3 border border-emerald-500/20 bg-emerald-500/5">
                <div className="flex items-center gap-2 text-emerald-400 text-xs font-bold mb-1">
                  <TrendingUp className="w-3.5 h-3.5" /> إجمالي المبيعات
                </div>
              ...
              <div className="glass-panel rounded-2xl px-4 py-3 border border-red-500/20 bg-red-500/5">
                <div className="flex items-center gap-2 text-red-400 text-xs font-bold mb-1">
                  <TrendingDown className="w-3.5 h-3.5" /> إجمالي المشتريات
                </div>
                <div className="text-white font-black text-lg">
              ...
              <div className="glass-panel rounded-2xl px-4 py-3 border border-blue-500/20 bg-blue-500/5">
                <div className="flex items-center gap-2 text-blue-400 text-xs font-bold mb-1">
                  <ArrowDownToLine className="w-3.5 h-3.5" /> إجمالي المدفوعات
                </div>
                <div className="text-white font-black text-lg">
                  {formatCurrency(summary!.total_receipts + summary!.total_payments)}
                </div>
              </div>
              <div
                className={`glass-panel rounded-2xl px-4 py-3 border ${closingBalance >= 0 ? 'border-yellow-500/30 bg-yellow-500/5' : 'border-violet-500/30 bg-violet-500/5'}`}
              >
                <div
                  className={`text-xs font-bold mb-1 ${closingBalance >= 0 ? 'text-yellow-400' : 'text-violet-400'}`}
                >
                  ⚖ صافي الرصيد
                </div>
                <div
                  className={`font-black text-lg ${closingBalance >= 0 ? 'text-yellow-400' : 'text-violet-400'}`}
                >
                  {formatCurrency(Math.abs(closingBalance))}
                  <span className="text-xs font-normal mr-1">
                    {closingBalance >= 0 ? 'عليه لنا' : 'له علينا'}
                  </span>
                </div>
              </div>
            </div>
              ...
              <table className="w-full text-right text-white/80 whitespace-nowrap">
                <thead className="bg-white/5 border-b border-white/10 sticky top-0">
                  <tr>
                    ...
                    <th className="p-3 font-semibold text-center text-emerald-400 text-sm">
                      مدين (له)
                    </th>
                    <th className="p-3 font-semibold text-center text-red-400 text-sm">
                      دائن (عليه)
                    </th>
                ...
                <tbody>
                  {filtered.length === 0 ? (
                    ...
                    filtered.map((row, i) => {
                      const cfg = rowTypeConfig[row.type] ?? {
                        label: row.type,
                        icon: '•',
                        credit_color: 'text-red-400',
                        debit_color: 'text-emerald-400',
                      ...
                      return (
                        <tr key={i} className="border-b border-white/5 erp-table-row">
                          <td className="p-3 text-white/50 tabular-nums text-sm">
                          ...
                          <td className="p-3 text-white/30 font-mono text-xs">
                            {row.reference_no ?? '—'}
                          </td>
                          <td
                            className={`p-3 text-center font-bold text-sm ${row.debit > 0 ? cfg.debit_color : 'text-white/20'}`}
                          >
                            {row.debit > 0 ? formatCurrency(row.debit) : '—'}
                          </td>
                          <td
                            className={`p-3 text-center font-bold text-sm ${row.credit > 0 ? cfg.credit_color : 'text-white/20'}`}
                          >
                            {row.credit > 0 ? formatCurrency(row.credit) : '—'}
                          </td>
                          <td
                            className={`p-3 text-center font-black text-sm ${row.balance > 0 ? 'text-yellow-400' : row.balance < 0 ? 'text-violet-400' : 'text-white/30'}`}
                          >
                            {row.balance !== 0
                ...
                {filtered.length > 0 && (
                  <tfoot className="bg-white/5 border-t border-white/10">
                    <tr>
                      <td colSpan={3} className="p-3 text-white/60 font-bold text-sm">
                        الرصيد النهائي
                      </td>
                      <td className="p-3 text-center font-black text-emerald-400">
                        {formatCurrency(filtered.reduce((s, r) => s + r.debit, 0))}
                      </td>
                      <td className="p-3 text-center font-black text-red-400">
                        {formatCurrency(filtered.reduce((s, r) => s + r.credit, 0))}
                      </td>
                      <td
                        className={`p-3 text-center font-black ${closingBalance >= 0 ? 'text-yellow-400' : 'text-violet-400'}`}
                      >
                        {formatCurrency(Math.abs(closingBalance))}{' '}
                        {closingBalance >= 0 ? 'عليه' : 'له'}
                      </td>
                    </tr>
                  </tfoot>
                )}
