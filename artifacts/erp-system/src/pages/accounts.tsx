  ...
  return (
    <>
      <tr className="border-b border-line erp-table-row">
          ...
          <div className="flex items-center gap-2" style={{ paddingRight: `${depth * 20}px` }}>
            {hasChildren ? (
              <button
                ...
                className="p-0.5 rounded text-ink/40 hover:text-ink/80 transition-colors"
              >
            ...
            <span
              className={`font-mono text-xs ${depth === 0 ? 'text-amber-400 font-bold' : 'text-ink/50'}`}
            >
              {account.code}
            </span>
          </div>
        ...
        <td className="p-3">
          <span
            className={`font-${depth === 0 ? 'black' : depth === 1 ? 'bold' : 'medium'} text-ink`}
          >
            {account.name}
          </span>
        </td>
        <td className="p-3">
          <span
            className={`px-2 py-0.5 rounded-lg text-xs font-bold border ${TYPE_COLORS[account.type] || ''}`}
          >
        ...
        <td className="p-3 text-right">
          <span
            className={`font-bold ${account.current_balance >= 0 ? 'text-emerald-400' : 'text-red-400'}`}
          >
        ...
        <td className="p-3">
          {account.is_posting ? (
            <span className="px-2 py-0.5 rounded text-xs text-emerald-400 bg-emerald-500/10 border border-emerald-500/20">
              ترحيل
            </span>
          ) : (
            <span className="px-2 py-0.5 rounded text-xs text-ink/30 bg-surface border border-line">
              إجمالي
            </span>
          )}
        </td>
...
export default function Accounts() {
  ...
  const seedMutation = useMutation({
    mutationFn: async () => {
      for (const acc of DEFAULT_ACCOUNTS) {
        await authFetch(api('/api/accounts'), {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ ...acc, opening_balance: 0 }),
        });
      }
    },
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ['/api/accounts'] });
      toast({ title: '✅ تم تحميل الحسابات الافتراضية' });
    },
  });
  ...
  const totals = useMemo(() => {
    const sum = (type: string) =>
      accounts
        .filter((a) => a.type === type && a.is_posting)
    ...
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-5 gap-3">
        {Object.entries(totals).map(([type, val]) => (
          <div
            key={type}
            className={`glass-panel rounded-2xl p-3 border ${TYPE_COLORS[type].split(' ').slice(1).join(' ')}`}
          >
            <p className={`text-xs font-bold mb-1 ${TYPE_COLORS[type].split(' ')[0]}`}>
              {TYPE_LABELS[type]}
            </p>
          ...
          <div className="glass-panel rounded-3xl p-8 w-full max-w-md border border-line shadow-2xl modal-panel">
            ...
            <div className="space-y-3">
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="text-ink/60 text-xs mb-1 block">كود الحساب *</label>
                  <input
                    required
                    type="text"
              ...
              <div>
                <label className="text-ink/60 text-xs mb-1 block">اسم الحساب *</label>
                <input
                  required
                  type="text"
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="text-ink/60 text-xs mb-1 block">رصيد افتتاحي</label>
                  <input
                    type="number"
                    step="0.01"
                    className="glass-input"
                ...
                <div className="flex items-end pb-1">
                  <label className="flex items-center gap-2 cursor-pointer">
                    <input
                      type="checkbox"
                      checked={form.is_posting}
                      onChange={(e) => setForm((f) => ({ ...f, is_posting: e.target.checked }))}
                ...
                <tr>
                  <td colSpan={5} className="p-12 text-center">
                    <div className="text-ink/30 space-y-3">
                      ...
                      <button
                        onClick={() => seedMutation.mutate()}
                        className="btn-primary px-6 py-2 text-sm mx-auto"
                      >
