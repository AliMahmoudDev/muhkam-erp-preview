...
function EntryDetailModal({ entryId, onClose }: { entryId: number; onClose: () => void }) {
  ...
  const { data: entry, isLoading } = useQuery<JournalEntryDetail>({
    ...
    queryFn: () =>
      authFetch(api(`/api/journal-entries/${entryId}`)).then((r) => {
  ...
  const postMutation = useMutation({
    mutationFn: () =>
      authFetch(api(`/api/journal-entries/${entryId}/post`), { method: 'PATCH' }).then((r) => {
        if (!r.ok) throw new Error('خطأ في جلب البيانات');
        return r.json();
      }),
    onSuccess: () => {
  ...
  const deleteMutation = useMutation({
    mutationFn: () =>
      authFetch(api(`/api/journal-entries/${entryId}`), { method: 'DELETE' }).then(async (r) => {
  ...
  const reversalMutation = useMutation({
    mutationFn: () =>
      authFetch(api(`/api/journal-entries/${entryId}/reverse`), { method: 'POST' }).then(
        async (r) => {
          const j = await r.json();
          if (!r.ok) throw new Error(j.error);
          return j;
      ...
      <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/70 backdrop-blur-sm modal-overlay">
        <div className="glass-panel rounded-3xl p-0 w-full max-w-3xl border border-line shadow-2xl max-h-[90vh] overflow-hidden flex flex-col">
          <div className="flex justify-between items-center p-6 border-b border-line bg-surface">
            ...
            <div className="flex gap-2">
              {entry?.status === 'draft' && (
                <>
                  <button
                    ...
                    className="px-4 py-2 rounded-xl bg-emerald-500/20 text-emerald-400 hover:bg-emerald-500/30 text-sm font-bold border border-emerald-500/30 transition-colors"
                  >
              ...
              {entry?.status === 'posted' && (
                <button
                  onClick={() => {
                    if (confirm('هل تريد عكس هذا القيد المحاسبي؟ سيتم إنشاء قيد معاكس جديد.'))
          ...
          <div className="overflow-y-auto p-6 space-y-5">
            {isLoading ? (
              <div className="text-center py-12 text-ink/40">جاري التحميل...</div>
            ) : !entry ? null : (
              <>
                <div className="grid grid-cols-2 gap-4 p-4 bg-surface rounded-2xl border border-line">
                  <div>
                    <p className="text-ink/50 text-xs">رقم القيد</p>
                    <p className="text-amber-400 font-bold font-mono">{entry.entry_no}</p>
                  </div>
                  <div>
                    <p className="text-ink/50 text-xs">التاريخ</p>
                    <p className="text-ink">{entry.date}</p>
                  </div>
                  <div>
                    <p className="text-ink/50 text-xs">البيان</p>
                    <p className="text-ink font-semibold">{entry.description}</p>
                  </div>
                  <div>
                    <p className="text-ink/50 text-xs">الحالة</p>
                    <div className="mt-1">
                  ...
                  {entry.reference && (
                    <div>
                      <p className="text-ink/50 text-xs">المرجع</p>
                      <p className="text-ink/70">{entry.reference}</p>
                    </div>
                  )}
                </div>
                <div className="rounded-2xl overflow-hidden border border-line">
                  <table className="w-full text-right text-sm">
                    ...
                    <tfoot className="bg-surface border-t border-line">
                      <tr>
                        <td colSpan={2} className="p-3 text-ink/60 font-bold">
                          الإجمالي
                        </td>
...
function NewEntryModal({ onClose }: { onClose: () => void }) {
  ...
  const { data: accounts = [] } = useQuery<Account[]>({
    ...
    queryFn: () =>
      authFetch(api('/api/accounts')).then((r) => {
        if (!r.ok) throw new Error('خطأ في جلب البيانات');
        return r.json();
      }),
  });
  ...
  const createMutation = useMutation({
    mutationFn: (data: object) =>
      authFetch(api('/api/journal-entries'), {
        method: 'POST',
  ...
  const handleSave = (status: 'draft' | 'posted') => {
    ...
    if (mappedLines.length < 2) {
      toast({ title: 'أضف سطرين على الأقل', variant: 'destructive' });
      return;
    }
      ...
      <div className="glass-panel rounded-3xl p-0 w-full max-w-4xl border border-line shadow-2xl max-h-[95vh] overflow-hidden flex flex-col">
                  ...
                  <td colSpan={2} className="p-3">
                    {isBalanced ? (
                      <span className="text-emerald-400 text-xs flex items-center gap-1">
                        <CheckCircle className="w-3 h-3" /> متوازن
                      </span>
                    ) : (
                      totalDebit > 0 && (
                        <span className="text-red-400 text-xs flex items-center gap-1">
                          <AlertCircle className="w-3 h-3" /> فرق:{' '}
        ...
        <div className="p-6 border-t border-line bg-black/30 flex gap-3">
          <button
            ...
            disabled={!isBalanced || !form.description || createMutation.isPending}
...
export default function JournalEntries() {
  ...
  const { data: entries = [], isLoading } = useQuery<JournalEntry[]>({
    ...
    queryFn: () =>
      authFetch(api('/api/journal-entries')).then((r) => {
        if (!r.ok) throw new Error('خطأ في جلب البيانات');
        return r.json();
      }),
  });
            ...
            <tbody>
              {isLoading ? (
                ...
                filtered.map((entry) => (
                  <tr
                    key={entry.id}
                    className="border-b border-line erp-table-row cursor-pointer"
