...
export default function Tasks() {
  ...
  const { data: safes = [] } = useQuery<Safe[]>({
    ...
    queryFn: () =>
      authFetch(api('/api/settings/safes')).then((r) => {
  ...
  const navActions = allNavActions.filter(
    (a) => a.permission === null || hasPermission(user, a.permission) === true
  );
      ...
      {stats && (
        <div className="grid grid-cols-4 gap-2">
          {[
            ...
            {
              label: 'الصافي',
              val: stats.net_profit,
              c: Number(stats.net_profit) >= 0 ? 'emerald' : 'red',
            },
          ].map(({ label, val, c }) => (
            <div key={label} className={`glass-panel rounded-xl p-3 border border-${c}-500/15`}>
              <p className="text-white/30 text-xs">{label}</p>
              <p className={`text-${c}-400 font-black text-sm mt-0.5`}>
  ...
  const { data: txToday = [] } = useQuery<Transaction[]>({
    ...
    queryFn: () =>
      safeId
        ? authFetch(
            api(
              `/api/financial-transactions?safe_id=${safeId}&from=${closingDate}&to=${closingDate}`
            )
          ).then((r) => {
            if (!r.ok) throw new Error('خطأ في جلب البيانات');
            return r.json();
  ...
  return (
    <div>
      ...
      {txToday.length === 0 && safeId && (
        <div className="text-center py-4 text-white/25 text-sm mb-4">
          لا توجد حركات لهذه الخزينة في هذا اليوم
        </div>
      )}
      ...
      <div className="glass-panel rounded-2xl p-4 border border-amber-500/20 bg-amber-500/5 space-y-3">
        ...
        {variance !== null && (
          <div
            className={`flex items-center justify-between rounded-xl px-4 py-3 border ${variance === 0 ? 'bg-emerald-500/10 border-emerald-500/20' : variance > 0 ? 'bg-teal-500/10 border-teal-500/20' : 'bg-red-500/10 border-red-500/20'}`}
          >
            <span className="text-white/60 text-sm">العجز / الزيادة</span>
            <span
              className={`font-black text-lg ${variance === 0 ? 'text-emerald-400' : variance > 0 ? 'text-teal-400' : 'text-red-400'}`}
            >
              {variance > 0 ? '+' : ''}
      ...
      <div className="flex gap-3 mt-4">
        ...
        <button
          onClick={() =>
            onSuccess(`تم حفظ إقفال خزينة "${selectedSafe?.name}" بتاريخ ${closingDate} ✓`)
          }
