...
export default function ScrapInventory() {
  ...
  const addM = useMutation({
    mutationFn: (data: Record<string, unknown>) =>
      authFetch(api("/api/scrap-items"), { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(data) })
        .then(async r => { const j = await r.json(); if (!r.ok) throw new Error(j.error); return j; }),
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ["/api/scrap-items"] });
      setShowNew(false); setNewName(""); setNewQty("1"); setNewCost("0"); setNewReason("");
      toast({ title: "✅ أُضيفت" });
    },
  ...
  const delM = useMutation({
    ...
    onSuccess: () => { qc.invalidateQueries({ queryKey: ["/api/scrap-items"] }); toast({ title: "✅ حُذفت" }); },
  });
              ...
              {items.map(it => (
                <tr key={it.id} className="border-b border-white/5 text-white/80">
                  ...
                  <td className="px-3 py-2">
                    <button onClick={() => delM.mutate(it.id)} className="btn-icon text-red-400/40 hover:text-red-400 w-7 h-7">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm" onClick={() => setShowNew(false)}>
          <div className="glass-panel rounded-2xl p-5 w-96 border border-white/10 space-y-3" onClick={e => e.stopPropagation()}>
            ...
            <div className="grid grid-cols-2 gap-2">
              <input type="number" value={newQty} onChange={(e) => setNewQty(e.target.value)} placeholder="الكمية" className="erp-input w-full text-sm" />
              <input type="number" value={newCost} onChange={(e) => setNewCost(e.target.value)} placeholder="سعر القطعة" className="erp-input w-full text-sm" />
            </div>
            <input value={newReason} onChange={(e) => setNewReason(e.target.value)} placeholder="السبب (مثل: تلف، كسر...)" className="erp-input w-full text-sm" />
            <button onClick={() => {
              if (!newName.trim()) { toast({ title: "اسم القطعة مطلوب", variant: "destructive" }); return; }
              addM.mutate({ product_name: newName, quantity: newQty, unit_cost: newCost, reason: newReason });
            }} className="w-full py-2 rounded-xl bg-amber-500/20 border border-amber-500/30 text-amber-300 font-bold text-sm">حفظ</button>
          </div>
        </div>
      )}
    </div>
