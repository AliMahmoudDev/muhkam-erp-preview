...
export function DeviceDetails({
  ...
  const doAction = async (action: 'delete' | 'maintenance' | 'available') => {
    try {
      if (action === 'delete') {
        ...
        toast({
          title: action === 'maintenance' ? 'تم إرسال الجهاز للصيانة' : 'تم إرجاع الجهاز كمتاح',
        });
      }
      onRefresh();
      onClose();
  ...
  return (
    <>
      ...
      >
        <div
          className="glass-panel rounded-2xl border border-line w-full max-w-lg mx-4 overflow-hidden flex flex-col"
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <div className="flex items-center gap-2 mt-0.5">
                  ...
                  <button
                    onClick={() => {
                      navigator.clipboard.writeText(device.device_no);
                    }}
          ...
          <div className="overflow-y-auto flex-1 p-4 space-y-4">
              ...
              <>
                ...
                <div className="grid grid-cols-3 gap-2">
                  ...
                  <InfoCard
                    icon={Battery}
                    label="البطارية"
                    value={device.battery_health ? `${device.battery_health}%` : undefined}
                    color={
                      device.battery_health && device.battery_health < 80
                        ? 'text-amber-300'
                        : 'text-ink/70'
                    }
                  />
                  <div className="bg-surface rounded-xl border border-line p-2.5 flex flex-col gap-0.5">
                    <div className="flex items-center gap-1.5 text-ink/30 text-[10px]">
                ...
                <div className="grid grid-cols-2 gap-2">
                  ...
                  {device.imei && (
                    <InfoCard icon={Info} label="IMEI / SN" value={device.imei} copyable />
                  )}
                </div>
                  ...
                  <div className="grid grid-cols-3 gap-2 text-center">
                    <div className="bg-surface rounded-lg p-2">
                      ...
                      <p className="font-bold text-ink/60 text-sm">
                        {parseFloat(device.purchase_price).toLocaleString()}
                      </p>
                      <p className="text-[10px] text-ink/20">ج.م</p>
                    </div>
                    <div className="bg-surface rounded-lg p-2">
                      <p className="text-[10px] text-ink/30 mb-1">
                        {device.status === 'sold' ? 'بيع بـ' : 'سعر البيع'}
                      </p>
                      <p className="font-bold text-ink text-sm">
                        {parseFloat(
                          device.status === 'sold'
                            ? (device.sold_price ?? device.sale_price)
                            : device.sale_price
                        ).toLocaleString()}
                      </p>
                      <p className="text-[10px] text-ink/20">ج.م</p>
                    ...
                    <div className="bg-surface rounded-lg p-2">
                      ...
                      <p
                        className={`font-bold text-sm ${profit > 0 ? 'text-emerald-400' : 'text-red-400'}`}
                      >
                        {profit >= 0 ? '+' : ''}
                        {profit.toLocaleString()}
                      </p>
                      <p
                        className={`text-[10px] ${profit > 0 ? 'text-emerald-400/40' : 'text-red-400/40'}`}
                      >
                ...
                <div className="bg-surface rounded-xl border border-line p-3 space-y-3">
                  ...
                  <div className="grid grid-cols-2 gap-3 text-sm">
                    ...
                    <div>
                      ...
                      <p className="text-ink/80 font-mono text-xs">
                        {device.purchase_invoice_no || '—'}
                      </p>
                    </div>
                    <div>
            ...
            {tab === 3 && (
              <div>
                ...
                (device.condition_notes && device.condition_notes.includes('صيانة')) ? (
                  ...
                  <div className="flex flex-col items-center justify-center py-16 text-center">
                    ...
                    <p className="text-ink/20 text-xs mt-1">
                      هذا الجهاز لم يتم إرساله للصيانة من قبل
                    </p>
                  </div>
                )}
              </div>
                    ...
                    <div className="bg-surface rounded-xl border border-line p-3 grid grid-cols-2 gap-3 text-sm">
                      <div>
                        ...
                        <p className="text-ink/80">
                          {device.sold_at
                            ? new Date(device.sold_at).toLocaleDateString('ar-EG', {
                                year: 'numeric',
                                month: 'long',
                                day: 'numeric',
                              })
                      ...
                      <div>
                        ...
                        <p className="text-emerald-400 font-bold">
                          {parseFloat(device.sold_price ?? '0').toLocaleString()} ج.م
                        </p>
                      </div>
                      <div>
                        <p className="text-[10px] text-ink/30 mb-0.5">صافي الربح</p>
                        <p
                          className={`font-bold ${profit >= 0 ? 'text-emerald-400' : 'text-red-400'}`}
                        >
                          {profit >= 0 ? '+' : ''}
                      ...
                      <div>
                        ...
                        <p className="text-ink/70">
                          {PAY_LABELS[device.payment_method ?? ''] ?? device.payment_method ?? '—'}
                        </p>
                      </div>
                      <div>
                        <p className="text-[10px] text-ink/30 mb-0.5">حالة الدفع</p>
                        <p
                          className={`font-medium text-xs ${device.payment_status === 'paid' ? 'text-emerald-400' : device.payment_status === 'partial' ? 'text-amber-400' : 'text-red-400'}`}
                        >
                          {STATUS_LABELS[device.payment_status ?? ''] ??
          ...
          <div className="px-4 py-3.5 border-t border-line shrink-0">
            {confirming ? (
              <div className="flex items-center justify-between">
                ...
                <div className="flex gap-2">
                  <button
                    ...
                    className="px-3 py-1.5 rounded-xl border border-line text-ink/50 text-xs"
                  >
                    إلغاء
                  </button>
                  <button
              ...
              <div className="flex gap-2">
                ...
                {device.status === 'maintenance' && (
                  <button
                    onClick={() => setConfirming('available')}
