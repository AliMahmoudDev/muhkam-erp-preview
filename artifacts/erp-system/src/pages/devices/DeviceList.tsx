          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-line">
                ...
                <th className="text-right text-[11px] font-bold text-ink/25 px-3 py-2.5 hidden sm:table-cell">المواصفات</th>
                <th className="text-right text-[11px] font-bold text-ink/25 px-3 py-2.5 hidden sm:table-cell">IMEI</th>
                <th className="text-right text-[11px] font-bold text-ink/25 px-3 py-2.5">الأسعار</th>
                <th className="text-center text-[11px] font-bold text-ink/25 px-3 py-2.5">الحالة</th>
                <th className="text-right text-[11px] font-bold text-ink/25 px-3 py-2.5 hidden md:table-cell">المورد / العميل</th>
                <th className="text-center text-[11px] font-bold text-ink/25 px-3 py-2.5 w-16">إجراءات</th>
              </tr>
            </thead>
            <tbody>
              {paginatedDevices.map((d, idx) => (
                <tr key={d.id}
                  ...
                  <td className="px-4 py-3 cursor-pointer" onClick={() => setSelected(d)}>
                    <p className="font-semibold text-ink/90 text-sm">{d.brand} {d.model}</p>
                  ...
                  <td className="px-3 py-3 hidden sm:table-cell cursor-pointer" onClick={() => setSelected(d)}>
                    <div className="flex items-center gap-1.5 flex-wrap">
                      {d.storage && <span className="text-[11px] text-ink/50 bg-surface px-1.5 py-0.5 rounded">{d.storage}</span>}
                      {d.color  && <span className="text-[11px] text-ink/40">{d.color}</span>}
                      {d.battery_health && (
                        <span className={`text-[11px] flex items-center gap-0.5 ${d.battery_health < 80 ? "text-amber-400/70" : "text-ink/35"}`}>
                  ...
                  <td className="px-3 py-3 hidden sm:table-cell cursor-pointer" onClick={() => setSelected(d)}>
                    {d.imei
                      ? <span className="text-[11px] text-ink/35 font-mono tracking-wide">{maskImei(d.imei)}</span>
            ...
            return (
              <div key={d.id}
                ...
                <div className="p-3 cursor-pointer" onClick={() => setSelected(d)}>
                  ...
                  <p className="font-bold text-ink text-sm leading-tight">{d.brand} {d.model}</p>
                  <p className="text-[10px] text-ink/25 font-mono mt-0.5">{d.device_no}</p>

                  <div className="mt-2.5 flex flex-wrap gap-1">
                    {d.storage && <span className="text-[10px] text-ink/45 bg-surface px-1.5 py-0.5 rounded">{d.storage}</span>}
                    {d.color   && <span className="text-[10px] text-ink/35">{d.color}</span>}
                    {d.battery_health && (
                      <span className={`text-[10px] flex items-center gap-0.5 ${d.battery_health < 80 ? "text-amber-400/70" : "text-ink/30"}`}>
                        <Battery className="w-2.5 h-2.5" />{d.battery_health}%
                      </span>
                    )}
                  </div>
                  ...
                  {d.imei && (
                    <p className="text-[10px] text-ink/25 font-mono mt-1.5 tracking-wide">{maskImei(d.imei)}</p>
                  )}
