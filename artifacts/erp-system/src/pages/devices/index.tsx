...
export function StatusBadge({ status }: { status: DeviceStatus }) {
  ...
  return (
    <span className={`text-[11px] font-bold px-2 py-0.5 rounded-full border ${cls}`}>{label}</span>
  );
}
...
export function RowMenu({
  ...
  const [confirming, setConfirming] = useState<
    'delete' | 'maintenance' | 'available' | 'return' | null
  >(null);
  ...
  const handleOpenMenu = (e: React.MouseEvent) => {
    ...
    if (btnRef.current) {
      ...
      const leftPos = Math.max(
        4,
        Math.min(rect.right - dropdownWidth, window.innerWidth - dropdownWidth - 4)
      );
          ...
          <div className="glass-panel rounded-2xl border border-line p-5 w-72 space-y-4">
            ...
            <div className="flex gap-2">
              <button
                ...
                className="flex-1 py-2 rounded-xl border border-line text-ink/50 text-sm"
              >
                إلغاء
              </button>
              <button
                onClick={(e) => {
                  e.stopPropagation();
                  doAction(confirming as 'delete' | 'maintenance' | 'available');
                }}
...
export default function Devices() {
  ...
  return (
    <div className="p-4 space-y-4 max-w-5xl mx-auto" dir="rtl">
        ...
        <>
          ...
          <div className="grid grid-cols-3 gap-2">
            ...
            <div className="glass-panel rounded-xl border border-blue-500/15 p-3">
              ...
              <p
                className={`text-[10px] mt-0.5 font-semibold ${(stats?.sold_profit ?? 0) >= 0 ? 'text-emerald-400/70' : 'text-red-400/70'}`}
              >
      ...
      {selected && (
        <DeviceDetails device={selected} onClose={() => setSelected(null)} onRefresh={refresh} />
      )}
    </div>
  );
}
