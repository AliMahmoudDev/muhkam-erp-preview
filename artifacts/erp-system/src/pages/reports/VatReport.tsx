...
export default function VatReport({ warehouseId }: { warehouseId?: number | null }) {
  ...
  const { data, isLoading, refetch } = useQuery<VatData>({
    ...
    queryFn: () =>
      authFetch(
        api(`/api/reports/vat-report?date_from=${dateFrom}&date_to=${dateTo}${whParam}`)
      ).then(async (r) => {
  ...
  const handleCsvExport = () => {
    ...
    const rows = [
      ...
      [
        'الحالة',
        data.vat_status === 'payable'
          ? 'مستحقة للسداد'
          : data.vat_status === 'refundable'
            ? 'مستحقة الاسترداد'
            : 'متوازنة',
      ],
      [],
    ...
    a.href = url;
    a.download = `vat-report-${dateFrom}-${dateTo}.csv`;
    a.click();
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          ...
          <div className="bg-blue-500/8 border border-blue-500/20 rounded-2xl p-5">
            ...
            <div className="text-2xl font-bold text-blue-300 mb-1">
              {formatCurrency(data.output_vat.tax_amount)}
            </div>
          ...
          <div className="bg-amber-500/8 border border-amber-500/20 rounded-2xl p-5">
            ...
            <div className="text-2xl font-bold text-amber-300 mb-1">
              {formatCurrency(data.input_vat.tax_amount)}
            </div>
            <div className="text-xs text-ink/40">
          ...
          {/* Net VAT */}
          <div
            className={`border rounded-2xl p-5 ${netPositive ? 'bg-red-500/8 border-red-500/20' : 'bg-emerald-500/8 border-emerald-500/20'}`}
          >
            <div className="flex items-center justify-between mb-3">
              <span
                className={`text-xs font-bold ${netPositive ? 'text-red-400' : 'text-emerald-400'}`}
              >
                الضريبة الصافية
              </span>
            ...
            <div
              className={`text-2xl font-bold mb-1 ${netPositive ? 'text-red-300' : 'text-emerald-300'}`}
            >
