    ...
    <div className="rpt-panel rounded-2xl overflow-hidden">
      ...
      >
        <div>
          ...
          <p style={{ fontSize: 10, color: txtHint, marginTop: 2 }}>
            حركة النقد الفعلية · التحويلات بين الخزن مستثناة
          </p>
        </div>
          ...
          <tbody>
            ...
            {openingBalance !== null && (
              <tr style={{ background: 'var(--bg-row-alt)' }}>
                <td style={cell({ color: txtSub, fontSize: 12.5, fontStyle: 'italic' })}>
                  رصيد أول الفترة (الخزينة)
                </td>
                <td style={cellNum({ color: txtSub, fontSize: 12.5, fontStyle: 'italic' })}>
            ...
            <tr>
              ...
              <td style={cellNum({ color: 'var(--status-success)' })}>
                {formatCurrency(cf.customer_receipts)}
              </td>
            </tr>
            {showSubBreak && (
              <>
                <ChildRow
                  label="  · سندات القبض المرحّلة"
                  amount={formatCurrency(cf.receipts_in)}
                  color={txtSub}
                />
                <ChildRow
                  label="  · مبيعات نقدية مباشرة"
                  amount={formatCurrency(cf.cash_sales)}
                  color={txtSub}
                />
              </>
            )}
            ...
            <tr style={{ background: netBg }}>
              <td
                style={cell({
                  ...
                  color: netColor,
                  borderBottom: 'none',
                  borderTop: `2px solid ${netColor}35`,
                  paddingTop: 20,
                  paddingBottom: changePct !== null ? 12 : 20,
                })}
              >
                ...
                {changePct !== null && (
                  <div
                    style={{
                      fontSize: 11,
                      fontWeight: 600,
                      color: isImproved ? 'var(--status-success)' : 'var(--status-danger)',
                      marginTop: 4,
                      opacity: 0.85,
                    }}
                  >
                    {isImproved ? '↑' : '↓'} {Math.abs(changePct).toFixed(1)}%{' '}
                    {isImproved ? 'أعلى' : 'أقل'} من الفترة السابقة
                  </div>
                )}
              </td>
              <td
            ...
            {closingBalance !== null && (
              <>
                ...
                <tr style={{ background: 'var(--bg-row-section)' }}>
                  <td style={cell({ fontWeight: 700, color: txtMain, fontSize: 13 })}>
                    = رصيد آخر الفترة (الخزينة)
                  </td>
                  <td style={cellNum({ fontWeight: 700, color: txtMain, fontSize: 13 })}>
...
export default function CashFlowReport() {
  ...
  const { data, isLoading } = useQuery<CashFlowData>({
    ...
    queryFn: () =>
      authFetch(api(`/api/reports/cash-flow?date_from=${dateFrom}&date_to=${dateTo}`)).then(
        async (r) => {
  ...
  const { data: prevData } = useQuery<CashFlowData>({
    ...
    queryFn: () =>
      authFetch(api(`/api/reports/cash-flow?date_from=${prevFrom}&date_to=${prevTo}`)).then(
        async (r) => {
          if (!r.ok) throw new Error(`API Error: ${r.status}`);
          return r.json();
        }
    ...
    <div className="space-y-4" style={{ fontFamily: "'Tajawal','Cairo',sans-serif" }}>
      ...
      {isLoading ? (
        <div className="grid grid-cols-3 gap-3">
          {[0, 1, 2].map((i) => (
            <div key={i} className="rpt-panel rounded-2xl p-4 h-20 animate-pulse" />
          ))}
        </div>
      ) : (
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
          <CfKPICard
            label="إجمالي الداخل النقدي"
            hint="مقبوضات + مبيعات نقدية + إيداعات"
            value={cf.total_in}
            variant="green"
            icon={TrendingUp}
          />
          <CfKPICard
            label="إجمالي الخارج النقدي"
            hint="مدفوعات للموردين + مصروفات"
            value={cf.total_out}
            variant="red"
            icon={TrendingDown}
          />
      ...
      {isLoading ? (
        ...
        <CashFlowStatement cf={cf} closingBalance={closingBalance} prevNetCf={prevNetCf} />
      )}
    </div>
  );
}
