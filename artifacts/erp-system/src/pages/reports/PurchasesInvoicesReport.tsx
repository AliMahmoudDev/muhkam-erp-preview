...
function exportPurchasesExcel(rows: PurchaseRow[]) {
  ...
  const body = rows.map((p) => [
    ...
    p.paid_amount,
    p.remaining_amount,
    PAY_AR[p.payment_type] || p.payment_type,
  ...
  a.href = url;
  a.download = 'فواتير_المشتريات.csv';
  a.click();
    ...
    <div className="space-y-4" style={{ fontFamily: "'Tajawal','Cairo',sans-serif" }}>
      <div className="grid grid-cols-3 gap-4">
        <div className="glass-panel rounded-2xl p-4 border border-red-500/10">
          ...
          <p className="text-2xl font-black text-ink">{formatCurrency(totalPurchases)}</p>
          <p className="text-ink/30 text-xs">{filtered.length} فاتورة</p>
        </div>
        <div className="glass-panel rounded-2xl p-4 border border-emerald-500/10">
          ...
          <p className="text-2xl font-black text-ink">{formatCurrency(totalPaid)}</p>
        </div>
        <div className="glass-panel rounded-2xl p-4 border border-amber-500/10">
          <p className="text-amber-400 text-xs mb-1">المتبقي للعملاء</p>
          <p className="text-2xl font-black text-ink">{formatCurrency(totalRemaining)}</p>
        </div>
      </div>
      <div className="flex gap-3 flex-wrap items-center">
        ...
        <div className="flex gap-1">
          {[
            ...
            <button
              key={opt.v}
              onClick={() => setPayFilter(opt.v)}
              className={`px-3 py-1.5 rounded-xl text-xs font-bold border transition-all ${payFilter === opt.v ? 'bg-amber-500/20 border-amber-500/40 text-amber-400' : 'glass-panel border-line text-ink/50 hover:text-ink'}`}
            >
              {opt.l}
            </button>
          ))}
        ...
        <div className="flex gap-2 mr-auto">
          <button
            onClick={() => exportPurchasesExcel(filtered)}
          ...
          <button
            onClick={() => printPurchasesReport(filtered)}
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-right text-sm whitespace-nowrap">
            <thead className="bg-surface border-b border-line">
              <tr>
                <th className="p-3 text-ink/50">رقم الفاتورة</th>
                <th className="p-3 text-ink/50">العميل</th>
                <th className="p-3 text-ink/50">الإجمالي</th>
                <th className="p-3 text-ink/50">المدفوع</th>
                <th className="p-3 text-ink/50">المتبقي</th>
                <th className="p-3 text-ink/50">الدفع</th>
                <th className="p-3 text-ink/50">الحالة</th>
                <th className="p-3 text-ink/50">التاريخ</th>
                <th className="p-3 text-ink/50">فاتورة</th>
              </tr>
            </thead>
            <tbody>
              {isLoading ? (
                ...
                filtered.map((p) => (
                  <tr key={p.id} className="border-b border-line erp-table-row">
                    ...
                    <td className="p-3">
                      <InvoicePdfButton type="purchases" id={p.id} />
                    </td>
                  </tr>
                ))
              )}
            ...
            {filtered.length > 0 && (
              <tfoot className="bg-surface border-t border-line">
                <tr>
                  <td colSpan={2} className="p-3 text-ink/50 font-bold">
                    الإجمالي ({filtered.length} فاتورة)
                  </td>
                  <td className="p-3 font-black text-ink">{formatCurrency(totalPurchases)}</td>
