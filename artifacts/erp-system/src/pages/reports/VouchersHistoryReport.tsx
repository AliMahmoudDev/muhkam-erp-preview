...
function unifyVouchers(r: RV[], d: DV[], p: PV[], t: ST[]): UV[] {
  ...
  t.filter((x) => x.direction === 'out' && x.type === 'transfer_out').forEach((x) =>
    rows.push({
      uid: `transfer-${x.id}`,
      kind: 'transfer',
      rawId: x.id,
      voucherNo: `TRF-${x.id}`,
      partyName: x.description || 'تحويل خزنة',
      safeName: x.safe_name || '—',
      amount: x.amount,
      status: null,
      date: x.date || '',
      notes: null,
...
function VSubBadge({ sub }: { sub: string }) {
  const map: Record<string, string> = {
    ...
    تحويل: 'bg-amber-500/15 text-amber-300 border-amber-500/20',
  };
  return (
    <span
  ...
  return (
    <div className="flex items-center justify-center gap-2 pt-2" dir="ltr">
      <button
        onClick={() => onChange(1)}
        disabled={page === 1}
        className="p-1.5 rounded-lg text-ink/40 hover:text-ink disabled:opacity-30 hover:bg-surface transition-all"
      >
      ...
      <span className="text-xs text-ink/50 min-w-[80px] text-center font-mono">
        {page} / {totalPages}
      </span>
      <button
        onClick={() => onChange(page + 1)}
...
export default function VouchersHistoryReport() {
  ...
  const { data: receiptsRaw, isLoading: l1 } = useQuery<RV[]>({
    queryKey: ['/api/receipt-vouchers'],
    queryFn: () =>
      authFetch(api('/api/receipt-vouchers')).then(async (r) => {
        if (!r.ok) throw new Error(`API Error: ${r.status}`);
        return r.json();
      }),
  });
  const { data: depositsRaw, isLoading: l2 } = useQuery<DV[]>({
    queryKey: ['/api/deposit-vouchers'],
    queryFn: () =>
      authFetch(api('/api/deposit-vouchers')).then(async (r) => {
        if (!r.ok) throw new Error(`API Error: ${r.status}`);
        return r.json();
      }),
  });
  const { data: paymentsRaw, isLoading: l3 } = useQuery<PV[]>({
    queryKey: ['/api/payment-vouchers'],
    queryFn: () =>
      authFetch(api('/api/payment-vouchers')).then(async (r) => {
        if (!r.ok) throw new Error(`API Error: ${r.status}`);
        return r.json();
      }),
  });
  const { data: transfersRaw, isLoading: l4 } = useQuery<ST[]>({
    queryKey: ['/api/safe-transfers'],
    queryFn: () =>
      authFetch(api('/api/safe-transfers')).then(async (r) => {
        if (!r.ok) throw new Error(`API Error: ${r.status}`);
        return r.json();
      }),
  });
  ...
  const allRows = useMemo(
    () => unifyVouchers(receipts, deposits, payments, transfers),
    [receipts, deposits, payments, transfers]
  );
  const filtered = useMemo(() => {
    let rows = allRows;
    if (vFilter === 'قبض')
      rows = rows.filter((r) => r.voucherType === 'قبض' && r.kind !== 'transfer');
    if (vFilter === 'صرف')
      rows = rows.filter((r) => r.voucherType === 'صرف' && r.kind !== 'transfer');
    ...
    if (search) {
      ...
      rows = rows.filter(
        (r) =>
          ...
          r.safeName.toLowerCase().includes(q)
      );
    }
    rows = rows.filter(
      (r) => !r.date || (r.date.slice(0, 10) >= dateFrom && r.date.slice(0, 10) <= dateTo)
    );
    return rows;
  ...
  return (
    <div className="space-y-5" dir="rtl" style={{ fontFamily: "'Tajawal','Cairo',sans-serif" }}>
      ...
      <div className="grid grid-cols-3 gap-3">
        ...
        <div
          className={`border rounded-2xl p-4 ${netFlow >= 0 ? 'bg-blue-500/10 border-blue-500/20' : 'bg-red-500/10 border-red-500/20'}`}
        >
          <div className={`text-xs mb-1 ${netFlow >= 0 ? 'text-blue-400/70' : 'text-red-400/70'}`}>
            صافي الحركة النقدية
          </div>
          <div className={`text-lg font-black ${netFlow >= 0 ? 'text-blue-400' : 'text-red-400'}`}>
            {netFlow >= 0 ? '+' : ''}
          ...
          {(['الكل', 'قبض', 'صرف', 'تحويل'] as VFilter[]).map((t) => {
            const count =
              t === 'الكل'
                ? allRows.length
                : t === 'قبض'
                  ? allRows.filter((r) => r.voucherType === 'قبض').length
                  : t === 'صرف'
                    ? allRows.filter((r) => r.voucherType === 'صرف' && r.kind !== 'transfer').length
                    : allRows.filter((r) => r.kind === 'transfer').length;
            return (
              <button
                key={t}
              ...
              >
                ...
                {t}
                <span
                  className={`text-xs px-1 py-0.5 rounded-full ${vFilter === t ? 'bg-black/20' : 'bg-surface'}`}
                >
                  {count}
                </span>
              </button>
        ...
        <div className="flex items-center gap-1.5 mr-auto text-xs text-ink/40">
          ...
          {PER_PAGE_OPTIONS.map((n) => (
            <button
              ...
              className={`px-2 py-1 rounded-lg font-bold border transition-all ${perPage === n ? 'bg-amber-500/20 border-amber-500/40 text-amber-400' : 'border-line text-ink/40 hover:text-ink'}`}
            >
              {n}
            </button>
          ))}
                ...
                <tr>
                  <td colSpan={9} className="p-16 text-center">
                    ...
                    <p className="text-ink/40 text-sm">
                      {search ? 'لا نتائج للبحث' : 'لا توجد سندات في هذه الفترة'}
                    </p>
                    {!search && (
                      <p className="text-ink/25 text-xs mt-1">جرّب تغيير نطاق التاريخ أو الفلتر</p>
                    )}
                  </td>
                </tr>
              ) : (
                  ...
                  >
                    <td className="p-4">
                      <div className="flex flex-col gap-1">
                        <VTypeBadge type={row.voucherType} />
                        <VSubBadge sub={row.subType} />
                      </div>
                    </td>
                    ...
                    <td
                      className="p-4 text-ink/50 text-sm max-w-[140px] truncate"
                      ...
                      <div className="flex items-center gap-1">
                        {row.kind === 'deposit' && row.status === 'draft' && (
                          <button
                            ...
                            title="ترحيل"
                            className="p-1.5 rounded-lg text-emerald-400 hover:bg-emerald-500/15 transition-all disabled:opacity-50"
                          >
                        ...
                        {row.kind === 'deposit' && row.status === 'posted' && (
                          <button
                            ...
                            title="إلغاء"
                            className="p-1.5 rounded-lg text-amber-400 hover:bg-amber-500/15 transition-all disabled:opacity-50"
                          >
        ...
        {filtered.length > 0 && (
          <div className="border-t border-line px-6 py-3 flex items-center justify-between gap-4">
            <span className="text-xs text-ink/30">
              {filtered.length} سند إجمالاً — يعرض{' '}
