...
export default function ProductProfitReport({ warehouseId }: { warehouseId?: number | null }) {
  ...
  const { data, isLoading } = useQuery<ProductProfitData>({
    ...
    queryFn: () =>
      authFetch(
        api(`/api/reports/product-profit?date_from=${dateFrom}&date_to=${dateTo}${wParam}`)
      ).then(async (r) => {
        ...
        {[
          {
            ...
            color: summary.total_revenue >= 0 ? 'text-emerald-400' : 'text-red-400',
          },
          {
            label: 'تكلفة البضاعة',
          ...
          },
          {
            ...
            color: summary.total_profit >= 0 ? 'text-amber-400' : 'text-red-400',
          },
          {
            label: 'هامش الربح الكلي',
            value: `${summary.overall_margin.toFixed(1)}%`,
            color: summary.overall_margin >= 20 ? 'text-emerald-400' : 'text-red-400',
          },
        ].map((c) => (
        ...
        <div className="flex gap-1">
          {(['profit', 'revenue', 'margin', 'qty'] as const).map((s) => (
            <button
              key={s}
              onClick={() => setSort(s)}
              className={`px-3 py-1.5 rounded-xl text-xs font-bold border transition-all ${sort === s ? 'bg-amber-500/20 border-amber-500/40 text-amber-400' : 'glass-panel border-line text-ink/50 hover:text-ink'}`}
            >
              {s === 'profit'
                ? 'الربح'
                : s === 'revenue'
          ...
          <table className="w-full text-right text-sm whitespace-nowrap">
            <thead className="bg-surface border-b border-line">
              <tr>
                ...
                <th className="p-3 text-ink/50">الكمية المباعة</th>
                <th className="p-3 text-ink/50">إجمالي المبيعات</th>
                <th className="p-3 text-ink/50">تكلفة البضاعة</th>
                <th className="p-3 text-ink/50">الربح</th>
                <th className="p-3 text-ink/50">هامش الربح</th>
              </tr>
            </thead>
            <tbody>
              {isLoading ? (
                ...
                products.map((p, i) => (
                  <tr key={p.product_id} className="border-b border-line erp-table-row">
                    ...
                    <td
                      className={`p-3 font-black ${p.profit >= 0 ? 'text-amber-400' : 'text-red-400'}`}
                    >
                      {formatCurrency(p.profit)}
                    </td>
                    <td className="p-3">
                      <span
                        className={`px-2 py-0.5 rounded-full text-xs font-bold border ${p.profit_margin >= 30 ? 'bg-emerald-500/20 border-emerald-500/30 text-emerald-400' : p.profit_margin >= 15 ? 'bg-amber-500/20 border-amber-500/30 text-amber-400' : 'bg-red-500/20 border-red-500/30 text-red-400'}`}
                      >
            ...
            {products.length > 0 && (
              <tfoot className="bg-surface border-t border-line">
                <tr>
                  <td colSpan={3} className="p-3 text-ink/50 font-bold">
                  ...
                  <td
                    className={`p-3 font-black ${summary.total_profit >= 0 ? 'text-amber-400' : 'text-red-400'}`}
                  >
                    {formatCurrency(summary.total_profit)}
                  </td>
                  <td className="p-3 font-bold text-ink/50">
                    {summary.overall_margin.toFixed(1)}%
                  </td>
                </tr>
              </tfoot>
            )}
