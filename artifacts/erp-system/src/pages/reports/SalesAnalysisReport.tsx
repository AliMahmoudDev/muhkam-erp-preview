...
export default function SalesAnalysisReport({ warehouseId }: { warehouseId?: number | null }) {
  ...
  const { data, isLoading } = useQuery<SalesAnalysisData>({
    ...
    queryFn: () =>
      authFetch(
        api(`/api/reports/sales-analysis?date_from=${dateFrom}&date_to=${dateTo}${wParam}`)
      ).then(async (r) => {
    ...
    <div className="space-y-4" style={{ fontFamily: "'Tajawal','Cairo',sans-serif" }}>
      ...
      <div className="flex bg-surface rounded-xl p-1 gap-1 w-fit">
        <button
          ...
          className={`px-4 py-2 rounded-xl text-sm font-bold transition-all flex items-center gap-2 ${view === 'product' ? 'bg-amber-500/20 border border-amber-500/40 text-amber-400' : 'text-ink/50 hover:text-ink'}`}
        >
          <Package className="w-4 h-4" /> حسب المنتج
        </button>
        <button
          onClick={() => setView('customer')}
          className={`px-4 py-2 rounded-xl text-sm font-bold transition-all flex items-center gap-2 ${view === 'customer' ? 'bg-amber-500/20 border border-amber-500/40 text-amber-400' : 'text-ink/50 hover:text-ink'}`}
        >
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-right text-sm whitespace-nowrap">
                <thead className="bg-surface border-b border-line">
                  <tr>
                    <th className="p-3 text-ink/50">#</th>
                    <th className="p-3 text-ink/50">المنتج</th>
                    <th className="p-3 text-ink/50">الكمية</th>
                    <th className="p-3 text-ink/50">متوسط السعر</th>
                    <th className="p-3 text-ink/50">
                      <span
                        title="مرتب حسب الأعلى مبيعًا"
                        className="inline-flex items-center gap-1 cursor-default"
                      >
                        إجمالي المبيعات <span className="text-amber-400">↓</span>
                      </span>
                    </th>
                    <th className="p-3 text-ink/50">% من الإجمالي</th>
                    <th className="p-3 text-ink/50">عدد الفواتير</th>
                  </tr>
                </thead>
                <tbody>
                  {isLoading ? (
                    ...
                    byProductSorted.map((p, i) => (
                      <tr key={p.product_id} className="border-b border-line erp-table-row">
                        ...
                        <td className="p-3">
                          <div className="flex items-center gap-2">
                            <div className="h-1.5 bg-surface rounded-full flex-1 max-w-[80px] overflow-hidden">
                              <div
                                className="h-full bg-amber-400 rounded-full"
                                style={{
                                  width: `${totalByProd > 0 ? (p.total_revenue / totalByProd) * 100 : 0}%`,
                                }}
                              />
                            </div>
                            <span className="text-ink/40 text-xs">
                              {totalByProd > 0
                                ? ((p.total_revenue / totalByProd) * 100).toFixed(1)
                                : 0}
                              %
                            </span>
                          </div>
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-right text-sm whitespace-nowrap">
              <thead className="bg-surface border-b border-line">
                <tr>
                  <th className="p-3 text-ink/50">#</th>
                  <th className="p-3 text-ink/50">العميل</th>
                  <th className="p-3 text-ink/50">إجمالي المبيعات</th>
                  <th className="p-3 text-ink/50">% من الإجمالي</th>
                  <th className="p-3 text-ink/50">عدد الفواتير</th>
                </tr>
              </thead>
                        ...
                        <div className="flex items-center gap-2">
                          <div className="h-1.5 bg-surface rounded-full flex-1 max-w-[80px] overflow-hidden">
                            <div
                              ...
                              style={{
                                width: `${totalByCust > 0 ? (c.total_revenue / totalByCust) * 100 : 0}%`,
                              }}
                            />
                          </div>
                          <span className="text-ink/40 text-xs">
                            {totalByCust > 0
                              ? ((c.total_revenue / totalByCust) * 100).toFixed(1)
                              : 0}
                            %
                          </span>
                        </div>
