  ...
  return (
    <div className="space-y-4" style={{ fontFamily:"'Tajawal','Cairo',sans-serif" }}>
      ...
      <div className="flex bg-surface rounded-xl p-1 gap-1 w-fit">
        <button onClick={()=>setView("product")} className={`px-4 py-2 rounded-xl text-sm font-bold transition-all flex items-center gap-2 ${view==="product"?"bg-amber-500/20 border border-amber-500/40 text-amber-400":"text-ink/50 hover:text-ink"}`}><Package className="w-4 h-4"/> حسب المنتج</button>
        <button onClick={()=>setView("customer")} className={`px-4 py-2 rounded-xl text-sm font-bold transition-all flex items-center gap-2 ${view==="customer"?"bg-amber-500/20 border border-amber-500/40 text-amber-400":"text-ink/50 hover:text-ink"}`}><Users className="w-4 h-4"/> حسب العميل</button>
      </div>
      {view==="product" ? (
        <>
          ...
          <div className="glass-panel rounded-3xl overflow-hidden border border-line">
            <div className="overflow-x-auto">
              <table className="w-full text-right text-sm whitespace-nowrap">
                <thead className="bg-surface border-b border-line"><tr><th className="p-3 text-ink/50">#</th><th className="p-3 text-ink/50">المنتج</th><th className="p-3 text-ink/50">الكمية</th><th className="p-3 text-ink/50">متوسط السعر</th><th className="p-3 text-ink/50"><span title="مرتب حسب الأعلى مبيعًا" className="inline-flex items-center gap-1 cursor-default">إجمالي المبيعات <span className="text-amber-400">↓</span></span></th><th className="p-3 text-ink/50">% من الإجمالي</th><th className="p-3 text-ink/50">عدد الفواتير</th></tr></thead>
                <tbody>
                  {isLoading ? <TableSkeleton cols={7} rows={5}/> :
                   byProductSorted.length===0 ? <tr><td colSpan={7} className="p-12 text-center text-ink/40">لا توجد بيانات</td></tr> :
                   byProductSorted.map((p,i)=>(
                    <tr key={p.product_id} className="border-b border-line erp-table-row">
                      ...
                      <td className="p-3 text-ink/70">{p.total_qty.toFixed(2)}</td>
                      <td className="p-3 text-ink/70">{formatCurrency(p.avg_price)}</td>
                      <td className="p-3 text-emerald-400 font-bold">{formatCurrency(p.total_revenue)}</td>
                      <td className="p-3">
                        <div className="flex items-center gap-2">
                          <div className="h-1.5 bg-surface rounded-full flex-1 max-w-[80px] overflow-hidden"><div className="h-full bg-amber-400 rounded-full" style={{width:`${totalByProd>0?(p.total_revenue/totalByProd*100):0}%`}}/></div>
                          <span className="text-ink/40 text-xs">{totalByProd>0?(p.total_revenue/totalByProd*100).toFixed(1):0}%</span>
                        </div>
                      </td>
                      <td className="p-3 text-ink/50">{p.invoice_count}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
        ...
        <div className="glass-panel rounded-3xl overflow-hidden border border-line">
          <div className="overflow-x-auto">
            <table className="w-full text-right text-sm whitespace-nowrap">
              <thead className="bg-surface border-b border-line"><tr><th className="p-3 text-ink/50">#</th><th className="p-3 text-ink/50">العميل</th><th className="p-3 text-ink/50">إجمالي المبيعات</th><th className="p-3 text-ink/50">% من الإجمالي</th><th className="p-3 text-ink/50">عدد الفواتير</th></tr></thead>
              <tbody>
                {isLoading ? <TableSkeleton cols={5} rows={5}/> :
                 byCustomer.length===0 ? <tr><td colSpan={5} className="p-12 text-center text-ink/40">لا توجد بيانات</td></tr> :
                 byCustomer.map((c,i)=>(
                  <tr key={c.customer_id??i} className="border-b border-line erp-table-row">
                    <td className="p-3 text-ink/30 text-xs">{i+1}</td>
                    <td className="p-3 text-ink font-bold">{c.customer_name}</td>
                    <td className="p-3 text-emerald-400 font-bold">{formatCurrency(c.total_revenue)}</td>
                    <td className="p-3">
                      <div className="flex items-center gap-2">
                        <div className="h-1.5 bg-surface rounded-full flex-1 max-w-[80px] overflow-hidden"><div className="h-full bg-blue-400 rounded-full" style={{width:`${totalByCust>0?(c.total_revenue/totalByCust*100):0}%`}}/></div>
                        <span className="text-ink/40 text-xs">{totalByCust>0?(c.total_revenue/totalByCust*100).toFixed(1):0}%</span>
                      </div>
                    </td>
                    <td className="p-3 text-ink/50">{c.invoice_count}</td>
                  </tr>
                ))}
              </tbody>
            </table>
