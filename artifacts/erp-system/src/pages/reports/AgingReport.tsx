...
export default function AgingReport() {
  ...
  return (
    <div
      dir="rtl"
      style={{ fontFamily: "'Tajawal','Cairo',sans-serif", color: 'var(--text-1)' }}
      className="space-y-4"
    >
      {/* ── Header ── */}
      ...
      {data && (
        ...
        >
          ...
          >
            {formatCurrency(
              bucket ? (data.buckets[bucket as keyof typeof data.buckets] ?? 0) : data.total
            )}
          </span>
        </div>
      )}
      ...
      <div style={cardStyle}>
        ...
        {error && (
          <p style={{ textAlign: 'center', color: 'var(--status-danger)' }}>{String(error)}</p>
        )}
        {data && !isLoading && (
