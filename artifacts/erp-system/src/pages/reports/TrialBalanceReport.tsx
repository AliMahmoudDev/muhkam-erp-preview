...
export default function TrialBalanceReport() {
  ...
  const { data, isLoading, refetch } = useQuery<TrialBalanceData>({
    ...
    queryFn: () =>
      authFetch(api(`/api/reports/trial-balance?date_from=${dateFrom}&date_to=${dateTo}`))
  ...
  return (
    <div className="space-y-4" dir="rtl" style={{ fontFamily: "'Tajawal','Cairo',sans-serif" }}>
      ...
      {/* Summary Bar */}
      {data && (
        <div className={`flex flex-wrap items-center gap-4 p-4 rounded-2xl border ${
          data.summary.is_balanced
            ? "bg-emerald-500/8 border-emerald-500/20"
            : "bg-red-500/8 border-red-500/25"
        }`}>
          <div className={`flex items-center gap-2 font-bold text-sm ${data.summary.is_balanced ? "text-emerald-400" : "text-red-400"}`}>
            {data.summary.is_balanced
          ...
          <table className="w-full text-sm">
            ...
            <tbody>
              ...
              <tr className="bg-amber-500/10 border-t-2 border-amber-500/30">
                <td colSpan={3} className="px-3 py-3 text-sm font-bold text-amber-300">الإجمالي العام</td>
                ...
                <td className={`px-3 py-3 text-left font-mono text-sm font-bold ${
