...
export default function ProfitLossReport() {
  ...
  const { data: plData, isLoading } = useQuery<ProfitsData>({
    ...
    queryFn: () =>
      authFetch(api(buildQS(dateFrom, dateTo, branches))).then(async (r) => {
        if (!r.ok) throw new Error(`API Error: ${r.status}`);
        return r.json();
      }),
    staleTime: 60_000,
  ...
  const { data: prevData } = useQuery<ProfitsData>({
    ...
    queryFn: () =>
      authFetch(api(buildQS(prevFrom, prevTo, branches))).then(async (r) => {
        if (!r.ok) throw new Error(`API Error: ${r.status}`);
        return r.json();
      }),
    staleTime: 60_000,
  ...
  const branchLabel =
    branches.length === 0
      ? 'جميع الفروع'
      : branches.length === 1
        ? (warehouses.find((w) => w.id === branches[0])?.name ?? String(branches[0]))
