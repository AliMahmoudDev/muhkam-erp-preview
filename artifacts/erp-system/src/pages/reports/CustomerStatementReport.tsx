...
export default function CustomerStatementReport() {
  ...
  const { data, isLoading, isFetching } = useQuery<CustomerStatementData>({
    ...
    queryFn: () =>
      authFetch(
        api(
          `/api/reports/customer-statement?customer_id=${customerId}&date_from=${dateFrom}&date_to=${dateTo}`
        )
      ).then(async (r) => {
        if (!r.ok) throw new Error(`API Error: ${r.status}`);
        return r.json();
          ...
          {data && (
            <div className="grid grid-cols-3 gap-3">
              <div className="glass-panel rounded-2xl p-4 border border-line">
                ...
                <p
                  className={`text-lg font-black ${data.opening_balance >= 0 ? 'text-amber-400' : 'text-red-400'}`}
                >
              ...
              <div className="glass-panel rounded-2xl p-4 border border-line">
                ...
                <p
                  className={`text-lg font-black ${data.closing_balance >= 0 ? 'text-emerald-400' : 'text-red-400'}`}
                >
              ...
              <div className="glass-panel rounded-2xl p-4 border border-line">
                ...
                <p
                  className={`text-lg font-black ${data.customer.balance >= 0 ? 'text-blue-400' : 'text-red-400'}`}
                >
              ...
              <table className="w-full text-right text-sm whitespace-nowrap">
                ...
                <tbody>
                  {isLoading || isFetching ? (
                    ...
                    stmt.map((row, i) => {
                      const meta = STMT_TYPE_MAP[row.type] ?? {
                        label: row.type,
                        cls: 'text-ink/50',
                      };
                      return (
