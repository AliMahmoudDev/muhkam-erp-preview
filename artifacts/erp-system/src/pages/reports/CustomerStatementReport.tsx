  ...
  return (
    <div className="space-y-4" style={{ fontFamily:"'Tajawal','Cairo',sans-serif" }}>
      <div className="flex flex-wrap gap-3 items-center">
        <select value={customerId} onChange={e=>setCustomerId(e.target.value)} className="glass-input rounded-xl px-3 py-2 text-sm text-ink min-w-[200px]">
      ...
      {!customerId ? (
        <div className="glass-panel rounded-2xl p-12 text-center border border-line">
          <Users className="w-10 h-10 text-ink/20 mx-auto mb-3"/>
          <p className="text-ink/40 font-bold">اختر عميلاً لعرض كشف حسابه</p>
        </div>
      ) : (
        <>
          {data&&(
            <div className="grid grid-cols-3 gap-3">
              <div className="glass-panel rounded-2xl p-4 border border-line"><p className="text-ink/40 text-xs mb-1">رصيد أول المدة</p><p className={`text-lg font-black ${data.opening_balance>=0?"text-amber-400":"text-red-400"}`}>{formatCurrency(data.opening_balance)}</p></div>
              <div className="glass-panel rounded-2xl p-4 border border-line"><p className="text-ink/40 text-xs mb-1">رصيد الختام</p><p className={`text-lg font-black ${data.closing_balance>=0?"text-emerald-400":"text-red-400"}`}>{formatCurrency(data.closing_balance)}</p></div>
              <div className="glass-panel rounded-2xl p-4 border border-line"><p className="text-ink/40 text-xs mb-1">الرصيد الفعلي (الدفتر)</p><p className={`text-lg font-black ${data.customer.balance>=0?"text-blue-400":"text-red-400"}`}>{formatCurrency(data.customer.balance)}</p></div>
            </div>
          )}
          <div className="glass-panel rounded-3xl overflow-hidden border border-line">
            <div className="overflow-x-auto">
              <table className="w-full text-right text-sm whitespace-nowrap">
                <thead className="bg-surface border-b border-line"><tr><th className="p-3 text-ink/50">التاريخ</th><th className="p-3 text-ink/50">النوع</th><th className="p-3 text-ink/50">البيان</th><th className="p-3 text-ink/50">مدين (له)</th><th className="p-3 text-ink/50">دائن (عليه)</th><th className="p-3 text-ink/50">الرصيد</th></tr></thead>
                <tbody>
                  {(isLoading||isFetching) ? <TableSkeleton cols={6} rows={5}/> :
                   stmt.length===0 ? <tr><td colSpan={6} className="p-12 text-center text-ink/40">لا توجد حركات في هذه الفترة</td></tr> :
                   stmt.map((row,i)=>{
                    const meta=STMT_TYPE_MAP[row.type]??{label:row.type,cls:"text-ink/50"};
                    return (
                      <tr key={`${row.date ?? ''}-${row.type}-${i}`} className="border-b border-line erp-table-row">
                        ...
                        <td className="p-3 text-ink/70">{row.description}{row.reference_no&&<span className="text-ink/30 text-xs mr-2">{row.reference_no}</span>}</td>
