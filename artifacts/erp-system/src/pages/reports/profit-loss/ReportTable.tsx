/**
 * ProfitLoss — ReportTable (extracted from ProfitLossReport)
...
import { formatCurrency, ProfitsData, fmtMonth, fmtDay, ChartTooltip } from '../shared';
  ...
  return (
    <div className="rpt-panel rounded-2xl overflow-hidden">
      ...
      >
        ...
        <ChevronDown
          className={`w-4 h-4 text-ink/30 transition-transform duration-200 ${open ? 'rotate-180' : ''}`}
        />
      </button>
      <AnimatePresence initial={false}>
        {open && (
          ...
          >
            <div className="px-5 pb-5 pt-1">{mounted && children}</div>
          </motion.div>
        )}
      </AnimatePresence>
    </div>
...
function AccountingStatement({ pl }: { pl: ProfitsData }) {
  ...
  const SectionHd = ({ label, hint }: { label: string; hint?: string }) => (
    <tr style={{ background: secBg }}>
      ...
      >
        <p style={{ fontSize: 12, fontWeight: 700, color: txtMain, letterSpacing: '0.02em' }}>
          {label}
        </p>
  ...
  const TotalRow = ({
    ...
    <tr
      style={{
        background: bg,
        borderTop: `${borderWidth}px solid ${border}`,
  ...
  const Spacer = () => (
    <tr>
      <td colSpan={2} style={{ height: 1, background: border, padding: 0 }} />
    </tr>
  );
        ...
        <table className="w-full" style={{ borderCollapse: 'collapse' }}>
          ...
          <tbody>
            ...
            <tr>
              ...
              <td style={cellNum({ color: 'var(--status-success)' })}>
                {formatCurrency(pl.total_revenue)}
              </td>
            </tr>
            ...
            {hasReturn && (
              <TotalRow label="= صافي الإيرادات" amount={formatCurrency(netRevenue)} bg={totalBg} />
            )}
            ...
            <tr style={{ background: netBg }}>
              <td
                style={cell({
                  ...
                  color: netColor,
                  borderBottom: 'none',
                  borderTop: `2px solid ${netColor}30`,
                  paddingTop: 18,
                  paddingBottom: 18,
                })}
              >
...
function BranchTable({ warehouses }: { warehouses: ProfitsData['by_warehouse'] }) {
  ...
  return (
    <div className="rpt-section rounded-2xl overflow-hidden">
      ...
      <div className="overflow-x-auto">
        <table
          className="w-full text-right text-sm whitespace-nowrap"
          style={{ borderCollapse: 'collapse' }}
        >
          <thead>
            <tr>
              {['الفرع', 'المبيعات', 'تكلفة البضاعة', 'مجمل الربح', 'الهامش', 'الفواتير'].map(
                (h) => (
                  <th key={h} className="rpt-th">
                    {h}
                  </th>
                )
              )}
          ...
          <tbody>
            {active.map((w, i) => {
              ...
              return (
                <tr
                  key={w.warehouse_id}
                  className="erp-table-row border-b border-line transition-colors"
                >
                  <td className="rpt-td">
                    <div className="flex items-center gap-2">
                      ...
                      {i === 0 && (
                        <span className="text-[10px] px-1.5 py-0.5 rounded-full bg-amber-500/12 text-amber-400 font-bold">
                          الأعلى
                        </span>
                      )}
                    </div>
                  </td>
                  <td className="rpt-td rpt-td-num font-bold tabular-nums">
                  ...
                  <td className="rpt-td font-bold tabular-nums">
                    <span className={w.gross_profit >= 0 ? 'text-emerald-400' : 'text-red-400'}>
                      {formatCurrency(w.gross_profit)}
                    </span>
                  </td>
                  <td className="rpt-td">
                    <span className={`text-xs font-bold px-2 py-0.5 rounded-full ${mgColor}`}>
                      {margin.toFixed(1)}%
                    </span>
                  </td>
          ...
          <tfoot>
            <tr className="border-t-2 border-line">
              ...
              <td className="rpt-td rpt-td-num font-black tabular-nums">
                {formatCurrency(active.reduce((s, w) => s + w.revenue, 0))}
              </td>
              <td className="rpt-td rpt-muted tabular-nums">
                {formatCurrency(active.reduce((s, w) => s + w.cost, 0))}
              </td>
              <td className="rpt-td font-black text-emerald-400 tabular-nums">
                {formatCurrency(active.reduce((s, w) => s + w.gross_profit, 0))}
              </td>
              <td colSpan={2} />
            </tr>
          </tfoot>
...
function SalesBreakdown({ pl }: { pl: ProfitsData }) {
  const segments = [
    ...
    {
      label: 'آجل',
      value: pl.credit_sales,
      color: 'var(--status-info)',
      icon: <CreditCard className="w-3.5 h-3.5" />,
    },
    {
      label: 'جزئي',
      value: pl.partial_sales,
      color: 'var(--status-warning)',
      icon: <Minus className="w-3.5 h-3.5" />,
    },
    {
      label: 'مرتجع',
      value: pl.return_amount,
      color: 'var(--status-danger)',
  ...
  return (
    <div className="space-y-3 pt-1">
      <div className="flex h-2 rounded-full overflow-hidden gap-px">
        {segments
          .filter((s) => s.label !== 'مرتجع')
          .map((s) => (
            <div
              key={s.label}
              className="h-full"
              style={{
                flexGrow: total > 0 ? s.value / total : 0,
                background: s.color,
        ...
        {segments.map((s) => {
          ...
          return (
            <div
              ...
              style={{ background: `${s.color}0c`, borderColor: `${s.color}22` }}
            >
...
function ExpenseBreakdown({
  ...
  data: ProfitsData['by_expense_category'];
  total: number;
}) {
        ...
        {data.slice(0, 8).map((item, i) => {
          ...
          return (
            <div key={item.category} className="flex items-center gap-2.5">
              ...
              <span className="rpt-strong text-xs font-bold tabular-nums">
...
function TopProducts({ products }: { products: ProfitsData['by_product'] }) {
  const top = useMemo(
    () => [...products].sort((a, b) => b.profit - a.profit).slice(0, 5),
    [products]
  );
    ...
    <div className="space-y-2 pt-1">
      {top.map((p, i) => {
        ...
        return (
          <div
            key={p.product_id}
            className="rpt-panel flex items-center gap-3 px-4 py-3 rounded-xl"
          >
            ...
            <div className="text-left shrink-0">
              <p className="font-black text-sm tabular-nums" style={{ color: accent }}>
                {formatCurrency(p.profit)}
              </p>
              <p className="text-xs" style={{ color: `${accent}80` }}>
...
function InsightsContent({ pl }: { pl: ProfitsData }) {
  const topBranch = useMemo(
    () => [...pl.by_warehouse].sort((a, b) => b.gross_profit - a.gross_profit)[0],
    [pl.by_warehouse]
  );
  ...
  if (topBranch?.revenue > 0)
    items.push({
      icon: '🏆',
      text: `أفضل فرع: ${topBranch.warehouse_name} — ربح ${formatCurrency(topBranch.gross_profit)}`,
      color: 'text-amber-300',
    });
  if (topExpCat)
    items.push({
      icon: '💸',
      text: `أعلى مصروف: ${topExpCat.category} — ${formatCurrency(topExpCat.total)}`,
      color: 'text-orange-300',
    });
  if (avgMargin < 15 && pl.total_revenue > 0)
    items.push({
      icon: '📉',
      text: `هامش الربح منخفض (${avgMargin.toFixed(1)}%) — راجع تكاليف البضاعة`,
      color: 'text-red-300',
    });
  if (pl.return_amount > pl.total_revenue * 0.1 && pl.total_revenue > 0)
    items.push({
      icon: '🔄',
      text: `نسبة مرتجعات مرتفعة: ${((pl.return_amount / pl.total_revenue) * 100).toFixed(1)}%`,
      color: 'text-orange-300',
    });
  if (pl.credit_sales > pl.cash_sales && pl.credit_sales > 0)
    items.push({
      icon: '💳',
      text: 'المبيعات الآجلة أعلى من النقدية — تابع الديون بانتظام',
      color: 'text-blue-300',
    });
  if (!items.length)
    return <p className="rpt-muted text-sm py-4 text-center">لا توجد رؤى لهذه الفترة</p>;
  return (
  ...
  return (
    <div className="pt-1">
      <div className="rpt-tab-bar flex gap-1 p-1 mb-4">
        {[
          { id: 'trend' as const, label: 'الأداء الزمني' },
          { id: 'waterfall' as const, label: 'تدفق الأرباح' },
          { id: 'compare' as const, label: 'مقارنة الفترات' },
        ].map((t) => (
          <button
            key={t.id}
            onClick={() => setTab(t.id)}
            className={`rpt-tab flex-1 ${tab === t.id ? 'active' : ''}`}
          >
            {t.label}
          </button>
        ))}
      ...
      {tab === 'compare' && (
        <CompareTab curr={pl} prev={prev} currLabel={currLabel} prevLabel={prevLabel} />
      )}
    </div>
  );
}
  ...
  const data = useMemo(() => {
    if (view === 'day')
      return [...pl.by_day]
        ...
        .map((d) => ({
          name: fmtDay(d.day),
          إيرادات: +d.revenue.toFixed(0),
          ربح: +d.profit.toFixed(0),
        }));
    return [...pl.by_month]
      .sort((a, b) => a.month.localeCompare(b.month))
      .map((m) => ({
        name: fmtMonth(m.month),
        إيرادات: +m.revenue.toFixed(0),
        ربح: +m.profit.toFixed(0),
      }));
  }, [view, pl]);
    ...
    <div>
      <div className="flex justify-end mb-3">
        <div className="rpt-tab-bar flex gap-1 p-0.5">
          {[
            { id: 'month' as const, l: 'شهري' },
            { id: 'day' as const, l: 'يومي' },
          ].map((t) => (
            <button
              key={t.id}
              onClick={() => setView(t.id)}
              className={`rpt-tab ${view === t.id ? 'active text-amber-400' : ''}`}
            >
              {t.l}
            </button>
          ))}
        ...
        <AreaChart data={data} margin={{ top: 4, right: 0, left: 0, bottom: 0 }}>
          <defs>
            <linearGradient id="gRev2" x1="0" y1="0" x2="0" y2="1">
              <stop offset="5%" stopColor="var(--status-warning)" stopOpacity={0.25} />
              <stop offset="95%" stopColor="var(--status-warning)" stopOpacity={0} />
            </linearGradient>
            <linearGradient id="gProf2" x1="0" y1="0" x2="0" y2="1">
...
const WF_CLR = {
  ...
  cost: 'var(--status-danger)',
  grossPos: 'var(--status-warning)',
  expenses: 'var(--status-warning)',
  netPos: 'var(--status-success)',
  netNeg: 'var(--status-danger)',
};
function WaterfallTab({ pl }: { pl: ProfitsData }) {
  ...
  const data = [
    ...
    },
    {
      ...
      fill: gross >= 0 ? WF_CLR.grossPos : 'var(--status-danger)',
      label: `=${formatCurrency(gross)}`,
      r: true,
    },
    {
      name: 'المصروفات',
    ...
    },
    {
      ...
      base: net >= 0 ? 0 : net,
      fill: net >= 0 ? WF_CLR.netPos : WF_CLR.netNeg,
  ...
  const lbl = (props: any) => {
    ...
    if (!item) return null;
    const cx = x + width / 2;
    const cy = height > 22 ? y + height / 2 : y - 7;
    return (
      <text
        x={cx}
        y={cy}
  ...
  return (
    <div>
      ...
      {rows.map((r) => {
        ...
        const up = diff >= 0;
        const chg = pctChange(r.curr, r.prev);
        return (
          <div
            key={r.label}
            className="flex items-center justify-between gap-3 py-2.5 rpt-divider last:border-0"
          >
            <span className="rpt-muted text-xs">{r.label}</span>
            <div className="flex items-center gap-2">
              <span className="rpt-strong text-sm font-bold tabular-nums">
...
/* ── Main Export ──────────────────────────────────────────────────────────── */
export default function ReportTable({ pl, prev, currLabel, prevLabel }: ReportTableProps) {
  return (
    <>
      ...
      <div className="space-y-2.5">
        ...
        <Accordion
          ...
          badge={
            pl.by_expense_category.length > 0 ? `${pl.by_expense_category.length} فئة` : undefined
          }
        >
        ...
        <Accordion
          ...
          badge={
            pl.by_product.length > 0 ? `${Math.min(pl.by_product.length, 5)} منتجات` : undefined
          }
        >
