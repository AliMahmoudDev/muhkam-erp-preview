...
function exportSalesExcel(rows: SaleRow[]) {
  const header = [
    'رقم الفاتورة',
    'العميل',
    'الإجمالي',
  ...
  const body = rows.map((s) => [
    ...
    s.paid_amount,
    s.remaining_amount,
    PAY_AR[s.payment_type] || s.payment_type,
  ...
  a.href = url;
  a.download = 'فواتير_المبيعات.csv';
  a.click();
    ...
    <div className="space-y-4" style={{ fontFamily: "'Tajawal','Cairo',sans-serif" }}>
      <div className="grid grid-cols-3 gap-4">
        <div className="glass-panel rounded-2xl p-4 border border-emerald-500/10">
          ...
          <p className="text-2xl font-black text-ink">{formatCurrency(totalSales)}</p>
        ...
        <div className="glass-panel rounded-2xl p-4 border border-red-500/10">
          ...
          <p className="text-2xl font-black text-ink">{formatCurrency(totalDebt)}</p>
        </div>
      </div>
      <div className="flex gap-3 flex-wrap items-center">
        ...
        <div className="flex gap-2 mr-auto">
          <button
            onClick={() => exportSalesExcel(filtered)}
          ...
          <button
            onClick={() => printSalesReport(filtered)}
              ...
              {isLoading ? (
                ...
                filtered.map((s) => (
                  <tr key={s.id} className="border-b border-line erp-table-row">
                    ...
                    <td className="p-3">
                      <InvoicePdfButton type="sales" id={s.id} />
                    </td>
                  </tr>
                ))
              )}
            ...
            {filtered.length > 0 && (
              <tfoot className="bg-surface border-t border-line">
                <tr>
                  ...
                  <td className="p-3 font-black text-ink">{formatCurrency(totalSales)}</td>
