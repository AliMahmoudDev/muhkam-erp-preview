...
function SeverityBadge({ sev }: { sev: 'OK' | 'WARNING' | 'CRITICAL' }) {
  ...
  return (
    <span
      className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs font-bold ${c.badge}`}
    >
          ...
          <div className="text-right shrink-0">
            {typeof issue.details.difference === 'number' && (
              <span className={`text-sm font-bold tabular-nums ${c.text}`}>
                {(issue.details.difference as number) > 0 ? '+' : ''}
                {formatCurrency(issue.details.difference as number)}
              </span>
            )}
          </div>
        )}
...
export default function HealthCheckReport() {
  ...
  const [expandedGroups, setExpandedGroups] = useState<Record<string, boolean>>({
    ...
    accounting_issues: true,
    cash_issues: true,
  });
  const { data, isLoading, refetch, isFetching } = useQuery<HealthCheckData>({
    queryKey: ['health-check'],
    queryFn: () =>
      authFetch(api('/api/reports/health-check')).then(async (r) => {
  ...
  const statusAR =
    status === 'OK' ? 'النظام سليم' : status === 'WARNING' ? 'يوجد تحذيرات' : 'يوجد مشاكل حرجة';
    ...
    <div className="space-y-5" dir="rtl" style={{ fontFamily: "'Tajawal','Cairo',sans-serif" }}>
      ...
      <div className="space-y-3">
        {Object.entries(groups).map(([groupKey, groupIssues]) => {
          ...
          const groupStatus: 'OK' | 'WARNING' | 'CRITICAL' = hasCritical
            ? 'CRITICAL'
            : hasWarnings
              ? 'WARNING'
              : 'OK';
