...
const toArabicType = (raw: string) => TYPE_LABELS[raw] ?? 'معاملة مالية';
...
const DATE_PILLS = [
  ...
  },
  {
    ...
    range: () => {
      const d = new Date();
      return { from: toDateStr(new Date(d.getFullYear(), d.getMonth(), 1)), to: toDateStr(d) };
    },
  },
  {
    label: 'هذه السنة',
    range: () => {
      const d = new Date();
      return { from: toDateStr(new Date(d.getFullYear(), 0, 1)), to: toDateStr(d) };
    },
  },
];
...
function exportToCSV(transactions: FinancialTransaction[]) {
  ...
  a.href = url;
  a.download = `الحركات_المالية_${toDateStr(new Date())}.csv`;
  a.click();
...
function CustomDropdown({
  ...
  return (
    <div ref={ref} className="relative w-full">
      ...
      >
        {icon && (
          <span style={{ color: 'var(--erp-text-3)' }} className="flex-shrink-0">
            {icon}
          </span>
        )}
        ...
        >
          ...
          {options.map((o) => (
            ...
            >
              ...
              {o.label}
              {value === o.value && <span className="mr-auto text-amber-400 text-xs">✓</span>}
            </button>
          ))}
        </div>
      )}
...
export default function FinancialTransactions() {
  ...
  const { data: transactions = [], isLoading } = useQuery<FinancialTransaction[]>({
    ...
    queryFn: () =>
      authFetch(api(`/api/financial-transactions${qs ? '?' + qs : ''}`)).then((r) => {
  ...
  const totalIn = useMemo(
    () => transactions.filter((t) => t.direction === 'in').reduce((s, t) => s + t.amount, 0),
    [transactions]
  );
  const totalOut = useMemo(
    () => transactions.filter((t) => t.direction === 'out').reduce((s, t) => s + t.amount, 0),
    [transactions]
  );
  ...
  return (
    <div className="space-y-5 pb-10 px-6 py-4">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-white/40 mt-0.5">
              {isLoading
                ? 'جارٍ التحميل...'
                : `${transactions.length.toLocaleString('ar-EG-u-nu-latn')} حركة${hasFilters ? ' — مفلترة' : ' — إجمالي'}`}
            </p>
          </div>
        </div>
        ...
        <div
          ...
          onMouseEnter={(e) =>
            (e.currentTarget.style.border =
              net >= 0 ? '1px solid rgba(251,191,36,0.4)' : '1px solid rgba(251,113,133,0.4)')
          }
          onMouseLeave={(e) => (e.currentTarget.style.border = `1px solid ${bdr}`)}
        >
          <div
      ...
      {/* ═══════════════════════════════════════
          FILTERS PANEL
          ═══════════════════════════════════════ */}
      <div className="rounded-2xl p-4" style={{ background: cardBg, border: `1px solid ${bdr}` }}>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 mt-3">
          ...
          <div className="relative">
            ...
            <input
              ...
              onBlur={(e) => {
                e.currentTarget.style.border = filters.from
                  ? '1px solid #F59E0B'
                  : `1px solid ${bdr}`;
                e.currentTarget.style.boxShadow = filters.from
          ...
          <div className="relative">
            ...
            <input
              ...
              onBlur={(e) => {
                e.currentTarget.style.border = filters.to
                  ? '1px solid #F59E0B'
                  : `1px solid ${bdr}`;
                e.currentTarget.style.boxShadow = filters.to
                  ? '0 0 0 3px rgba(245,158,11,0.1)'
                  : 'none';
              }}
            />
          ...
          <div
            ...
            onBlurCapture={(e) =>
              (e.currentTarget.style.border = filters.search
                ? '1px solid #F59E0B'
                : `1px solid ${bdr}`)
            }
          >
        ...
        <div className="flex items-center justify-between gap-3 mt-3 flex-wrap">
          <div className="flex items-center gap-1.5 flex-wrap">
            <span className="text-xs ml-1" style={{ color: txt4 }}>
              سريع:
            </span>
            {DATE_PILLS.map((pill) => {
              const r = pill.range();
              ...
              return (
                <button
                  ...
                  onMouseEnter={(e) => {
                    if (!isActive) {
                      e.currentTarget.style.borderColor = pillHoverBdr;
                      e.currentTarget.style.color = pillHoverTxt;
                    }
                  }}
                  onMouseLeave={(e) => {
                    if (!isActive) {
                      e.currentTarget.style.borderColor = pillBdr;
                      e.currentTarget.style.color = pillTxt;
                    }
                  }}
                >
                  {pill.label}
          ...
          <table className="w-full text-right text-white/80 whitespace-nowrap" dir="rtl">
            <thead>
              ...
              >
                ...
                <th
                  className="px-4 py-3.5 font-medium text-xs text-white/30 text-right"
                  style={{ minWidth: 120 }}
                >
                  النوع
                </th>
                <th className="px-4 py-3.5 font-medium text-xs text-white/30 text-right">
                  الخزينة
                </th>
                <th
                  className="px-4 py-3.5 font-medium text-xs text-white/30 text-right"
              ...
              {isLoading ? (
                    ...
                    <div className="flex flex-col items-center gap-5">
                      ...
                      <div>
                        ...
                        <p className="text-white/25 text-sm mt-1.5">
                          جرّب تغيير الفلاتر أو توسيع نطاق التاريخ
                        </p>
                      </div>
                      {hasFilters && (
                        <button
                ...
                paginated.map((t) => {
                  ...
                  const refLabel = t.reference_type
                    ? (REF_TYPE_LABELS[t.reference_type] ?? toArabicType(t.reference_type))
                    : null;
                  ...
                  return (
                    <>
                      ...
                      >
                        ...
                        <td className="px-4 py-3.5">
                          <span
                            className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-lg text-xs font-medium border ${badge.bg}`}
                          >
                            <span
                              className={`w-1.5 h-1.5 rounded-full flex-shrink-0 ${badge.dot}`}
                            />
                            {typeLabel}
                          </span>
                        </td>
                        ...
                        <td className="px-4 py-3.5" style={{ maxWidth: 150 }}>
                          {t.customer_name ? (
                            <span
                              className="text-white/70 text-sm block truncate"
                        ...
                        <td className="px-4 py-3.5">
                          {t.direction === 'in' ? (
                            ...
                            <span className="text-white/40 font-bold tabular-nums">
                              {formatCurrency(t.amount)}
                            </span>
                          )}
                        </td>
                        ...
                        <td className="px-4 py-3.5" style={{ maxWidth: 200 }}>
                          {t.description ? (
                            <span
                              className="text-white/50 text-sm truncate block"
                              title={t.description}
                              style={{ maxWidth: 190 }}
                            >
                              {t.description}
                              ...
                              <div className="min-w-[130px]">
                                ...
                                {t.reference_id ? (
                                  <div className="flex flex-col gap-0.5">
                                    ...
                                    <span className="text-white/80 font-mono font-semibold text-sm">
                                      #{t.reference_id}
                                    </span>
                                  </div>
                                ) : (
          ...
          >
            <p className="text-white/35 text-sm">
              ...
              <span className="text-white/65 font-semibold">
                {Math.min((page - 1) * PAGE_SIZE + 1, transactions.length).toLocaleString(
                  'ar-EG-u-nu-latn'
                )}
                –{Math.min(page * PAGE_SIZE, transactions.length).toLocaleString('ar-EG-u-nu-latn')}
              </span>{' '}
              من{' '}
              <span className="text-white/65 font-semibold">
                {transactions.length.toLocaleString('ar-EG-u-nu-latn')}
              </span>{' '}
              حركة
            </p>
            ...
            <div className="flex items-center gap-1.5">
              ...
              {Array.from({ length: totalPages }, (_, i) => i + 1)
                ...
                .map((p, idx) =>
                  p === '...' ? (
                    <span key={`ellipsis-${idx}`} className="text-white/20 text-sm px-1">
                      …
                    </span>
                  ) : (
                    <button
        ...
        {transactions.length > 0 && transactions.length <= PAGE_SIZE && (
          ...
          >
            <p className="text-sm" style={{ color: txt4 }}>
              إجمالي{' '}
              <span className="font-semibold" style={{ color: txt3 }}>
                {transactions.length}
              </span>{' '}
              حركة مالية
            </p>
