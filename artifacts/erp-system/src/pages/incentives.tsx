...
const metricTypeAr: Record<string, string> = {
  sales_amount: 'مبيعات',
  units_sold: 'وحدات مباعة',
  invoices_created: 'فواتير منشأة',
  customers_acquired: 'عملاء جدد',
  manual: 'يدوي',
};
const calcMethodAr: Record<string, string> = {
  achievement: 'إنجاز',
  slab: 'شرائح',
  tiered: 'تدرّجي',
};
...
export default function Incentives() {
  ...
  const [ruleForm, setRuleForm] = useState({
    ...
    calculation_method: 'achievement',
    currency: 'EGP',
  });
  ...
  const schemes = useQuery({
    ...
    queryFn: () => f('/api/incentive-schemes'),
  });
  const rules = useQuery({
    queryKey: ['incentive-rules', selectedScheme?.id],
    queryFn: () => f(`/api/incentive-rules/${selectedScheme?.id}`),
    enabled: selectedScheme != null,
  });
  const tracking = useQuery({
    queryKey: ['incentive-tracking', trackingEmpId, trackingMonth],
    queryFn: () => f(`/api/incentive-tracking/${trackingEmpId}?month=${trackingMonth}`),
    enabled: Boolean(trackingEmpId),
  });
  ...
  const createScheme = useMutation({
    mutationFn: (d: AnyRec) =>
      f('/api/incentive-schemes', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(d),
      }),
    ...mutOpts('incentive-schemes', 'تم إنشاء مخطط الحوافز'),
  });
  const createRule = useMutation({
    mutationFn: (d: AnyRec) =>
      f('/api/incentive-rules', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(d),
      }),
    ...mutOpts(
      ['incentive-rules', selectedScheme?.id as unknown as string],
      'تم إضافة قاعدة الحافز'
    ),
  });
  const recordMetric = useMutation({
    mutationFn: (d: AnyRec) =>
      f('/api/incentive-metrics/record', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(d),
      }),
    ...mutOpts(
      ['incentive-tracking', trackingEmpId, trackingMonth],
      'تم تسجيل المقياس واحتساب الحافز'
    ),
  });
  const assignScheme = useMutation({
    mutationFn: (d: AnyRec) =>
      f('/api/employee-incentive-assignments', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(d),
      }),
    ...mutOpts('employee-incentive-assignments', 'تم تعيين مخطط الحوافز'),
  });
        ...
        {canManage && (
          <div className="flex gap-2">
            <button
              ...
              className="erp-btn erp-btn-ghost flex items-center gap-1 text-xs text-blue-300 border-blue-500/40"
            >
              <UserPlus size={12} /> تعيين مخطط لموظف
            </button>
            <button
              onClick={() => setShowMetricForm(true)}
              className="erp-btn erp-btn-ghost flex items-center gap-1 text-xs text-emerald-300 border-emerald-500/40"
            >
              ...
              <>
                {schemesList.map((s) => (
                  ...
                  >
                    ...
                    {s.description && (
                      <div className="text-xs text-white/40 mt-1">{String(s.description)}</div>
                    )}
                    <div className="mt-2">
                      <span
                        className={
                ...
                {schemesList.length === 0 && (
                  <div className="erp-card">
                    <div className="erp-empty-state">
                      <Gift size={28} className="erp-empty-icon mb-2" />
                      <p className="erp-empty-label">لا توجد مخططات</p>
                    </div>
                  </div>
                )}
              </>
              ...
              <div className="erp-card space-y-4">
                <div className="flex items-center justify-between">
                  <h3 className="font-bold text-white">
                    {String(selectedScheme.name_ar)} — القواعد
                  </h3>
                  <div className="flex gap-2">
                    {canManage && (
                      <button
                        onClick={() => setShowRuleForm(true)}
                        className="erp-btn erp-btn-primary text-xs flex items-center gap-1"
                      >
                        <Plus size={12} /> إضافة قاعدة
                      </button>
                    )}
                    <button
                ...
                {rules.isLoading ? (
                  ...
                  <div className="erp-empty-state">
                    <BarChart2 size={28} className="erp-empty-icon mb-2" />
                    <p className="erp-empty-label">لا توجد قواعد — أضف أول قاعدة حافز</p>
                  </div>
                ) : (
                  <div className="space-y-3">
                    {rulesList.map((r) => (
                      <div
                        key={String(r.id)}
                        className="erp-card-soft p-4 rounded-xl border border-white/5"
                      >
                        <div className="flex items-center justify-between">
                          <div className="font-medium text-sm">
                            {metricTypeAr[String(r.metric_type)] ?? String(r.metric_type)}
                          </div>
                          <span className="erp-badge erp-badge-info">
                            {calcMethodAr[String(r.calculation_method)] ??
                              String(r.calculation_method)}
                          </span>
                        </div>
                        <div className="grid grid-cols-2 gap-4 mt-3 text-sm">
                          ...
                          {r.incentive_amount ? (
                            <div>
                              <span className="text-white/40">الحافز: </span>
                              <span className="text-emerald-300 font-bold font-mono">
                                {fmt(r.incentive_amount)} {String(r.currency)}
                              </span>
                            </div>
                          ) : null}
                        </div>
                          ...
                          <div className="mt-3">
                            ...
                            <div className="flex flex-wrap gap-2">
                              {safeArray(r.slabs).map((sl) => (
                                <span
                                  key={String(sl.id)}
                                  className="erp-badge erp-badge-pending text-xs"
                                >
                                  {Number(sl.from_percentage)}%
                                  {sl.to_percentage ? `–${Number(sl.to_percentage)}%` : '+'} →{' '}
                                  {fmt(sl.incentive_value)}
                                </span>
                              ))}
                            </div>
                          </div>
        ...
        <div className="space-y-4">
          <div className="flex gap-3 items-center flex-wrap">
            <input
              ...
              value={trackingEmpId}
              onChange={(e) => setTrackingEmpId(e.target.value)}
            />
            <input
              type="month"
              className="erp-input w-40"
          ...
          {!trackingEmpId ? (
            <div className="erp-card">
              <div className="erp-empty-state">
                <Trophy size={36} className="erp-empty-icon mb-2" />
                <p className="erp-empty-label">أدخل رقم الموظف لعرض تتبع الحوافز</p>
              </div>
            </div>
          ) : tracking.isLoading ? (
            <TableSkeleton />
            ...
            <div className="space-y-4">
              <div className="erp-card border-amber-500/30 p-5 flex items-center justify-between">
                <div>
                  <div className="text-sm text-white/50">
                    إجمالي الحوافز المستحقة — {trackingMonth}
                  </div>
                  <div className="text-3xl font-bold text-amber-300 mt-1 font-mono">
                    {fmt(trackingData?.total_accrued)} ج.م
                  </div>
                </div>
              ...
              {accruals.length === 0 ? (
                <div className="erp-card">
                  <div className="erp-empty-state">
                    <BarChart2 size={28} className="erp-empty-icon mb-2" />
                    <p className="erp-empty-label">لا توجد حوافز مسجّلة لهذا الشهر</p>
                  </div>
                </div>
              ) : (
                      ...
                      {accruals.map((a) => (
                        <tr key={String(a.id)} className="erp-table-row">
                          ...
                          <td className="p-3 text-sm">
                            <span
                              className={
                                Number(a.achievement_percentage) >= 100
                                  ? 'text-emerald-300 font-bold'
                                  : 'text-amber-300'
                              }
                            >
                              {Number(a.achievement_percentage).toFixed(1)}%
                            </span>
                          </td>
                          <td className="p-3 text-sm text-amber-300 font-bold font-mono">
                            {fmt(a.accrued_amount)}
                          </td>
                        </tr>
                      ))}
                    </tbody>
          ...
          <div className="erp-modal rounded-2xl shadow-2xl w-full max-w-md" dir="rtl">
            <div className="flex items-center justify-between p-5 border-b border-white/10">
              <h2 className="font-bold text-white flex items-center gap-2">
                <Gift size={16} className="text-amber-400" /> مخطط حوافز جديد
              </h2>
              <button
            ...
            <div className="p-5 space-y-3">
              <Field label="الاسم (عربي) *">
                <input
                  value={schemeForm.name_ar}
                  onChange={(e) => setSchemeForm((p) => ({ ...p, name_ar: e.target.value }))}
              ...
              <Field label="الاسم (إنجليزي)">
                <input
                  value={schemeForm.name_en}
                  onChange={(e) => setSchemeForm((p) => ({ ...p, name_en: e.target.value }))}
                  className="erp-input w-full"
                />
              </Field>
              <Field label="الوصف">
            ...
            <div className="flex gap-2 p-5 border-t border-white/10">
              <button
                onClick={() => {
                  createScheme.mutate(schemeForm, {
                    onSuccess: () => {
                      setShowSchemeForm(false);
                      setSchemeForm({ name_ar: '', name_en: '', description: '' });
                    },
                  });
                }}
              ...
              <button onClick={() => setShowSchemeForm(false)} className="erp-btn erp-btn-ghost">
                إلغاء
              </button>
            </div>
          </div>
      ...
      {showRuleForm && selectedScheme && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
          <div className="erp-modal rounded-2xl shadow-2xl w-full max-w-md" dir="rtl">
            <div className="flex items-center justify-between p-5 border-b border-white/10">
              <h2 className="font-bold text-white flex items-center gap-2">
                <BarChart2 size={16} className="text-amber-400" /> قاعدة حافز —{' '}
                {String(selectedScheme.name_ar)}
              </h2>
              <button
            ...
            <div className="p-5 space-y-3">
              <Field label="نوع المقياس">
                <select
                  ...
                  onChange={(e) => setRuleForm((p) => ({ ...p, metric_type: e.target.value }))}
                  className="erp-input w-full"
                >
                  {Object.entries(metricTypeAr).map(([k, v]) => (
              ...
              <Field label="طريقة الحساب">
                <select
                  ...
                  onChange={(e) =>
                    setRuleForm((p) => ({ ...p, calculation_method: e.target.value }))
                  }
                  className="erp-input w-full"
                >
                  {Object.entries(calcMethodAr).map(([k, v]) => (
                    <option key={k} value={k}>
                      {v}
                    </option>
                  ))}
              ...
              <Field label="مبلغ الحافز">
                <input
                  type="number"
                  value={ruleForm.incentive_amount}
                  onChange={(e) => setRuleForm((p) => ({ ...p, incentive_amount: e.target.value }))}
                  className="erp-input w-full"
                  placeholder="اتركه فارغاً للشرائح"
                />
              </Field>
            </div>
            <div className="flex gap-2 p-5 border-t border-white/10">
              <button
                onClick={() => {
                  createRule.mutate(
                    { ...ruleForm, incentive_scheme_id: selectedScheme.id },
                    {
                      onSuccess: () => {
              ...
              <button onClick={() => setShowRuleForm(false)} className="erp-btn erp-btn-ghost">
                إلغاء
              </button>
            </div>
          </div>
      ...
      {showMetricForm && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
          <div className="erp-modal rounded-2xl shadow-2xl w-full max-w-md" dir="rtl">
            <div className="flex items-center justify-between p-5 border-b border-white/10">
              <h2 className="font-bold text-white flex items-center gap-2">
                <BarChart2 size={16} className="text-amber-400" /> تسجيل مقياس أداء
              </h2>
              <button
            ...
            <div className="p-5 space-y-3">
              <Field label="رقم الموظف *">
                <input
                  type="number"
                  value={metricForm.employee_id}
                  onChange={(e) => setMetricForm((p) => ({ ...p, employee_id: e.target.value }))}
                  className="erp-input w-full"
                  required
                />
              </Field>
              <Field label="رقم قاعدة الحافز *">
                <input
                  type="number"
                  value={metricForm.incentive_rule_id}
                  onChange={(e) =>
                    setMetricForm((p) => ({ ...p, incentive_rule_id: e.target.value }))
                  }
                  className="erp-input w-full"
                  required
                />
              ...
              <Field label="التاريخ *">
                <input
                  ...
                  onChange={(e) => setMetricForm((p) => ({ ...p, metric_date: e.target.value }))}
                  className="erp-input w-full"
                  required
                />
              </Field>
              <Field label="قيمة المقياس *">
                <input
                  type="number"
                  value={metricForm.metric_value}
                  onChange={(e) => setMetricForm((p) => ({ ...p, metric_value: e.target.value }))}
                  className="erp-input w-full"
                  required
                />
              </Field>
            ...
            <div className="flex gap-2 p-5 border-t border-white/10">
              <button
                onClick={() => {
                  recordMetric.mutate(metricForm, {
                    onSuccess: () => {
              ...
              <button onClick={() => setShowMetricForm(false)} className="erp-btn erp-btn-ghost">
                إلغاء
              </button>
            </div>
          </div>
      ...
      {showAssignForm && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
          <div className="erp-modal rounded-2xl shadow-2xl w-full max-w-md" dir="rtl">
            <div className="flex items-center justify-between p-5 border-b border-white/10">
              <h2 className="font-bold text-white flex items-center gap-2">
                <UserPlus size={16} className="text-amber-400" /> تعيين مخطط حوافز لموظف
              </h2>
              <button
            ...
            <div className="p-5 space-y-3">
              ...
              <Field label="المخطط *">
                <select
                  value={assignForm.incentive_scheme_id}
                  onChange={(e) =>
                    setAssignForm((p) => ({ ...p, incentive_scheme_id: e.target.value }))
                  }
                  className="erp-input w-full"
                  required
                >
            ...
            <div className="flex gap-2 p-5 border-t border-white/10">
              <button
                ...
                disabled={assignScheme.isPending}
                className="erp-btn erp-btn-primary flex-1"
              >
                {assignScheme.isPending ? 'جاري...' : 'تعيين'}
              </button>
              <button onClick={() => setShowAssignForm(false)} className="erp-btn erp-btn-ghost">
                إلغاء
              </button>
            </div>
          </div>
