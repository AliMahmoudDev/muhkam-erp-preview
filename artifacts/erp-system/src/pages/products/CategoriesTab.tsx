...
export function CategoriesTab() {
  ...
  const updateMutation = useMutation({
    mutationFn: async ({ id, data }: { id: number; data: Record<string, unknown> }) => {
      const r = await authFetch(api(`/api/categories/${id}`), { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(data) });
  ...
  const createMutation = useMutation({
    mutationFn: async ({ data }: { data: Record<string, unknown> }) => {
      const r = await authFetch(api('/api/categories'), { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(data) });
      const j = await r.json();
      if (!r.ok) throw new Error(j.error || 'خطأ');
      return j;
    },
  ...
  useEffect(() => { if (editingId !== null) inputRef.current?.focus(); }, [editingId]);
  ...
  const saveEdit = (id: number) => {
    ...
    updateMutation.mutate({ id, data: { name: trimmed } }, {
      onSuccess: () => { toast({ title: '✅ تم تعديل اسم التصنيف' }); queryClient.invalidateQueries({ queryKey: ['/api/categories'] }); queryClient.invalidateQueries({ queryKey: ['/api/products'] }); cancelEdit(); },
      onError: (e: unknown) => { toast({ title: extractErr(e, 'خطأ في التعديل'), variant: 'destructive' }); cancelEdit(); },
    });
  };

  const handleDelete = (id: number) => {
    deleteMutation.mutate({ id }, {
      onSuccess: () => { toast({ title: '✅ تم حذف التصنيف' }); queryClient.invalidateQueries({ queryKey: ['/api/categories'] }); setConfirmDeleteId(null); },
      onError: (e: unknown) => { toast({ title: extractErr(e, 'خطأ في الحذف'), variant: 'destructive' }); setConfirmDeleteId(null); },
    });
  };
  ...
  const handleCreate = () => {
    ...
    createMutation.mutate({ data: { name: trimmed } }, {
      onSuccess: () => { toast({ title: '✅ تم إنشاء التصنيف' }); queryClient.invalidateQueries({ queryKey: ['/api/categories'] }); setNewName(''); },
      onError: (e: unknown) => { toast({ title: extractErr(e, 'خطأ في الإنشاء'), variant: 'destructive' }); },
    });
  };

  const confirmTarget = confirmDeleteId !== null ? categories.find((c) => c.id === confirmDeleteId) : null;
  ...
  return (
    <div className="space-y-6 max-w-2xl">
                ...
                {canManage && (
                  <div className="flex items-center gap-1 shrink-0">
                    {editingId === cat.id ? (
                      ...
                      <>
                        <button onClick={() => startEdit(cat.id, cat.name)} className="p-1.5 rounded-lg text-blue-400 hover:bg-blue-400/10 transition-colors" title="تعديل الاسم"><Pencil className="w-4 h-4" /></button>
                        <button onClick={() => setConfirmDeleteId(cat.id)} disabled={(cat.product_count ?? 0) > 0} className="p-1.5 rounded-lg text-red-400 hover:bg-red-400/10 transition-colors disabled:opacity-30 disabled:cursor-not-allowed" title={(cat.product_count ?? 0) > 0 ? 'لا يمكن الحذف — مرتبط بمنتجات' : 'حذف التصنيف'}><Trash2 className="w-4 h-4" /></button>
                      </>
                    )}
                  </div>
                )}
      ...
      {confirmTarget && (
        <ConfirmModal title="حذف التصنيف" description={`هل أنت متأكد من حذف «${confirmTarget.name}»؟ لا يمكن التراجع.`} isPending={deleteMutation.isPending} onConfirm={() => handleDelete(confirmTarget.id)} onCancel={() => setConfirmDeleteId(null)} />
      )}
    </div>
  );
}
