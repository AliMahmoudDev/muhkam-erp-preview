...
import { ROLES, PERMISSION_GROUPS, PERMISSION_TEMPLATES, TEMPLATE_LABELS } from './_constants';
...
export default function UsersTab() {
  ...
  const handleSubmit = () => {
    ...
    if (!/^01[0125]\d{8}$/.test(form.phone.trim())) {
      toast({ title: 'رقم الهاتف يجب أن يكون 11 رقم مصري صحيح', variant: 'destructive' });
      return;
    }
    if (!editId && !form.pin.trim()) {
      toast({ title: 'الرقم السري مطلوب', variant: 'destructive' });
      return;
    }
    if (form.role === 'employee' && !form.employee_id) {
      toast({ title: "يجب ربط الحساب بملف موظف عند اختيار دور 'موظف'", variant: 'destructive' });
      return;
    }
    ...
    if ((form.role === 'cashier' || form.role === 'salesperson') && !form.safe_id) {
      toast({ title: 'اختر الخزنة أولاً', variant: 'destructive' });
      return;
    }
    const payload = {
      name: form.name,
    ...
    if (editId) {
      updateUser.mutate(
        { id: editId, body: payload },
        {
          ...
          onError: (e: unknown) =>
            toast({ title: (e as Error)?.message || 'فشل التعديل', variant: 'destructive' }),
        }
      );
    } else {
  ...
  const pinStrength = (pin: string) => {
    if (pin.length < 4)
      return { w: '25%', color: 'bg-red-500', label: 'ضعيف جداً (4 أحرف على الأقل)' };
                ...
                <div className="p-5 pb-4">
                  <div className="flex items-start gap-3 mb-3">
                    ...
                    <div className="relative shrink-0">
                      <div
                        className={`w-12 h-12 rounded-xl ${role.avatarBg} flex items-center justify-center`}
                      >
                        <span className={`font-black text-lg ${role.avatarText}`}>
                          {getInitials(u.name)}
                        </span>
                      </div>
                      {lastLogin.online && (
                  ...
                  <div className="flex flex-wrap items-center gap-1.5">
                    <span
                      className={`px-2.5 py-1 rounded-lg text-[11px] font-bold border ${role.badge}`}
                    >
                      {role.label}
                    </span>
                    {permCount > 0 && (
        ...
        >
          <div className="flex-1 overflow-y-auto p-6 space-y-4">
            <div className="grid grid-cols-2 gap-4">
              ...
              <div>
                <FieldLabel>رقم الهاتف / اسم الدخول</FieldLabel>
                <SInput
              ...
              <div>
                <FieldLabel>
                  المخزن{' '}
                  {(form.role === 'cashier' || form.role === 'salesperson') && (
            ...
            <div
              className={
                form.role === 'employee'
                  ? 'ring-2 ring-amber-500/40 rounded-xl p-3 bg-amber-500/5'
                  : ''
              }
            >
              <FieldLabel>
                  ...
                  const filtered = permSearch.trim()
                    ? PERMISSION_GROUPS.map((g) => ({
                        ...
                        permissions: g.permissions.filter((p) =>
                          p.label.includes(permSearch.trim())
                        ),
                      })).filter((g) => g.permissions.length > 0)
                    : PERMISSION_GROUPS;
                  return filtered.length === 0 ? (
                    <p className="text-center py-4 text-xs" style={{ color: 'var(--erp-text-4)' }}>
                      ...
                      {filtered.map((group) => (
                        <PermissionGroupCard
                          ...
                          onChange={(key, val) =>
                            setForm((f) => ({
                              ...f,
                              permissions: { ...f.permissions, [key]: val },
                            }))
                          }
                        />
                      ))}
          ...
          <div className="flex gap-3 px-6 py-4 border-t border-line shrink-0">
            ...
            <GhostBtn onClick={resetForm} className="flex-1">
              إلغاء
            </GhostBtn>
          </div>
        </Modal>
      ...
      {deleteTarget && (
        <Modal title="تأكيد الحذف" icon={Trash2} onClose={() => setDeleteTarget(null)}>
          <div className="p-6 text-center space-y-4">
            ...
            <div>
              ...
              <p className="text-ink/40 text-sm mt-1">
                سيتم حذف <span className="text-ink font-semibold">{deleteTarget.name}</span> نهائياً
              </p>
            </div>
          </div>
          <div className="flex gap-3 px-6 py-4 border-t border-line">
            ...
            <GhostBtn onClick={() => setDeleteTarget(null)} className="flex-1">
              إلغاء
            </GhostBtn>
          </div>
        </Modal>
