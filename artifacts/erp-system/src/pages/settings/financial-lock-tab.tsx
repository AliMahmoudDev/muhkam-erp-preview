...
import { Loader2, Lock, LockOpen, AlertOctagon, Info, ExternalLink, Shield } from 'lucide-react';
...
export default function FinancialLockTab() {
  ...
  const lockMutation = useMutation({
    mutationFn: async (date: string) => {
      ...
      if (!r.ok) {
        ...
        throw new Error(e.error ?? 'فشل الإغلاق');
      }
    },
    onSuccess: () => {
  ...
  const unlockMutation = useMutation({
    mutationFn: async (reason: string) => {
      ...
      if (!r.ok) {
        const e = await r.json();
        throw new Error(e.error ?? 'فشل فتح الفترة');
      }
    },
    onSuccess: () => {
  ...
  return (
    <div className="space-y-5" dir="rtl">
      ...
      <div className="flex items-start gap-3 p-4 rounded-2xl bg-amber-500/8 border border-amber-500/20">
        ...
        <div>
          ...
          <p className="text-amber-300/60 text-xs mt-1 leading-relaxed">
            بعد إغلاق الفترة لا يمكن تعديل السجلات القديمة مباشرة. يتم التصحيح فقط من خلال قيود أو
            سندات عكسية في فترة مفتوحة. إلغاء الإغلاق يتطلب تقديم سبب ويُسجَّل في سجل التدقيق.
          </p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {/* ── Lock Action ── */}
        <div
          className="rounded-2xl border border-line p-5 space-y-4"
      ...
      {showUnlockDialog && (
        ...
        >
          <div
            className="w-full max-w-md rounded-2xl border border-line shadow-2xl p-6 space-y-5"
            style={{ background: 'var(--erp-bg-card)' }}
          >
            <div className="flex items-center gap-3">
