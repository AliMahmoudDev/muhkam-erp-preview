...
function Toggle({ on, onToggle }: { on: boolean; onToggle: () => void }) {
  return (
    ...
    >
      <span
        className={`absolute top-0.5 w-5 h-5 rounded-full bg-white shadow transition-all duration-300 ${on ? 'right-0.5' : 'left-0.5'}`}
      />
    </button>
  );
}
...
export default function InvoiceTab() {
  ...
  const toggle = (key: keyof InvoiceSettings) => update(key, form[key] === '1' ? '0' : '1');
        ...
        <div className="space-y-5">
          ...
          <Section icon={Copy} title="عدد النسخ">
            <div className="flex items-center gap-4">
              ...
              <div className="mt-5 text-ink/30 text-sm leading-relaxed">
                يُطبع <span className="text-amber-400 font-bold">{form.invoice_copies || '1'}</span>{' '}
                {form.invoice_copies === '1' ? 'نسخة' : 'نسخ'} تلقائياً عند الطباعة
              </div>
            </div>
          </Section>
          ...
          <div className="flex items-start gap-3 p-4 rounded-xl bg-surface border border-line">
            ...
            <p className="text-ink/30 text-xs leading-relaxed">
              إعدادات الفاتورة تُطبّق على جميع الفواتير المطبوعة من نظام المبيعات والمشتريات. بعض
              الإعدادات قد تتأثر بمعلومات الشركة الموجودة في تبويب بيانات الشركة.
            </p>
          </div>
        </div>
      )}
