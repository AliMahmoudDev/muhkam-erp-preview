...
export default function CurrencyTab() {
  ...
  const [thousandsSep, setThousandsSep] = useState<ThousandsSeparator>(
    settings.thousandsSeparator ?? 'comma'
  );
  ...
  const handleLogoFile = async (file: File) => {
    ...
    try {
      ...
      setCustomLogo(uploaded.url);
      setLogoPreview(uploaded.url);
      toast({ title: 'تم رفع الشعار بنجاح' });
    } catch (err) {
      toast({
        title: err instanceof Error ? err.message : 'فشل رفع الشعار',
        variant: 'destructive',
      });
    }
  };
  ...
  const saveRate = async (code: string) => {
    ...
    if (!v || v <= 0) {
      toast({
        ...
        description: 'أدخل سعر صرف أكبر من صفر',
        variant: 'destructive',
      });
      return;
    }
  ...
  const saveAllRates = async () => {
    ...
    if (!toSave.length) {
      toast({
        title: 'لا توجد أسعار للحفظ',
        description: 'أدخل سعراً واحداً على الأقل',
        variant: 'destructive',
      });
      return;
    }
    setSavingRate((s) => {
      const n = { ...s };
      toSave.forEach((c) => (n[c.code] = true));
      return n;
    });
    let ok = 0;
    let fail = 0;
    for (const c of toSave) {
      try {
        ...
        setSavingRate((s) => ({ ...s, [c.code]: false }));
      }
    }
  ...
  return (
    <div className="space-y-6" dir="rtl">
      <PageHeader
        ...
        sub="تخصيص العملة والتواريخ والتنسيقات والمظهر العام للنظام"
      />
              ...
              <div>
                ...
                <div className="flex items-start gap-4">
                  ...
                  >
                    {logoPreview ? (
                      <img
                        src={resolveUploadedFileUrl(logoPreview)}
                        alt="logo"
                        style={{
                          width: '100%',
                          height: '100%',
            ...
            <div className="shrink-0 flex flex-col items-center gap-3 lg:w-52">
              ...
              >
                ...
                >
                  ...
                  >
                    <img
                      src={logoPreview ? resolveUploadedFileUrl(logoPreview) : FALLBACK_LOGO}
                      alt="preview"
                      style={{
                        width: '100%',
                        height: '100%',
                ...
                {[
                  ...
                  <div key={i} className="flex items-center gap-2.5 px-4 py-2.5">
                    ...
                    />
                    <div
                      style={{ height: 6, borderRadius: 3, background: item.c, width: item.w }}
                    />
                  </div>
                ))}
                <div style={{ height: 8 }} />
