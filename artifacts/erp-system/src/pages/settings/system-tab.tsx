      ...
      <div className="flex gap-3">
        {SUB_TABS.map((t) => {
          ...
          return (
            ...
            >
              <div
                className={`w-8 h-8 rounded-lg flex items-center justify-center shrink-0 ${isActive ? 'bg-amber-500/20' : 'bg-surface'}`}
              >
                <Icon className={`w-4 h-4 ${isActive ? 'text-amber-400' : 'text-ink/40'}`} />
              </div>
              <div>
                <p className={`font-bold text-sm ${isActive ? 'text-amber-400' : 'text-ink/60'}`}>
                  {t.label}
                </p>
