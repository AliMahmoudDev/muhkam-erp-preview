      ...
      <div className="flex gap-1 p-1 rounded-xl bg-surface border border-line">
        {([
          ...
          <button
            ...
            className={`flex-1 flex items-center gap-2 px-3 py-2.5 rounded-lg text-right transition-all ${mainTab === id ? 'bg-surface text-ink' : 'text-ink/40 hover:text-ink/60'}`}
          >
            <span className={mainTab === id ? 'text-amber-400' : 'text-ink/25'}>{icon}</span>
            <div>
              <p className={`text-xs font-bold ${mainTab === id ? 'text-ink' : 'text-ink/50'}`}>{label}</p>
              <p className="text-ink/25 text-[10px]">{sub}</p>
            </div>
          </button>
        ))}
      </div>
