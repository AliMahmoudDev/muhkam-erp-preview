...
function ImportProductsTab({
  ...
  onExport,
  onImport,
  onDownloadProductTemplate: onDownloadTemplate,
}: ImportProductsProps) {
  return (
...
function ImportPurchasesTab({
  ...
  onUpdatePurRow,
  onPurchaseConfirm,
  onDownloadPurchaseTemplate: onDownloadTemplate,
  onReset,
  onSetPurSupplier,
  onSetPurPayType,
      ...
      {!purParsed ? (
        ...
        <>
          <div className="flex items-center justify-between p-3 rounded-xl bg-surface border border-line">
            ...
            <button
              onClick={onReset}
              className="text-xs text-ink/35 hover:text-ink transition-colors"
            >
              إلغاء
            </button>
          </div>
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <SInput
                ...
                value={purSupplier}
                onChange={(e) => onSetPurSupplier(e.target.value)}
              />
            </div>
            <div>
              ...
              <thead>
                <tr className="bg-surface border-b border-line">
                  {['SKU', 'الصنف', 'الكمية', 'السعر', 'الإجمالي', 'الحالة'].map((h) => (
                    <th key={h} className="px-3 py-2.5 text-right text-ink/35 font-medium">
                      {h}
                    </th>
                  ))}
                </tr>
              ...
              <tbody>
                {purRows.map((r) => {
                  ...
                  return (
                    <tr
                      key={r.idx}
                      className={`border-b border-line ${hasErr ? 'bg-red-500/5' : 'hover:bg-surface'}`}
                    >
                      <td className="px-3 py-2 text-ink/40 font-mono">{r.sku || '—'}</td>
                      <td className="px-3 py-2 text-ink/60 max-w-[100px] truncate">
                        {r.name || '—'}
                      </td>
                      ...
                      <td className="px-3 py-2 text-ink/45 font-mono">
                        {isNaN(total) ? '—' : total.toFixed(2)}
                      </td>
                      <td className="px-3 py-2">
                        {hasErr ? (
                          <span className="text-red-400 text-[10px]" title={r.errors.join(' | ')}>
                            ✗ {r.errors[0]}
                          </span>
                        ) : (
      ...
      <div className="flex border-b border-line">
        {(
          ...
          >
            ...
            <div>
              <p
                className={`text-sm font-bold ${importTab === id ? 'text-amber-400' : 'text-ink/50'}`}
              >
                {label}
              </p>
