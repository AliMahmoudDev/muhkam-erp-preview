...
export function CardSkeleton() {
  return (
    <div
      className="border border-line rounded-2xl p-5 animate-pulse space-y-3"
      style={{ background: 'var(--erp-bg-card)' }}
    >
      <div className="flex items-center gap-3">
...
export function PermissionGroupCard({
  ...
  return (
    <div
      className={`rounded-xl border overflow-hidden ${c.header}`}
      style={{ background: 'var(--erp-bg-card)' }}
    >
      <div
      ...
      >
        ...
        <div className="flex items-center gap-3">
          <button
            ...
            onClick={(e) => {
              e.stopPropagation();
              keys.forEach((k) => onChange(k, !allOn));
            }}
