...
export default function BackupModSelectModal({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative w-full max-w-md bg-canvas border border-amber-500/30 rounded-2xl shadow-2xl overflow-hidden">
        <div className="flex items-center justify-between px-5 py-4 border-b border-amber-500/20 bg-amber-500/5">
          ...
          <button
            onClick={onClose}
            className="p-1.5 rounded-lg text-ink/30 hover:text-ink hover:bg-surface"
          >
            <X className="w-4 h-4" />
          </button>
        </div>
              ...
              return (
                ...
                >
                  ...
                  <div className="flex-1">
                    <p className={`text-sm font-bold ${on ? 'text-amber-300' : 'text-ink/60'}`}>
                      {g.label}
                    </p>
                    <p className="text-ink/25 text-xs">{g.note}</p>
                  </div>
                  <div
                    className={`w-5 h-5 rounded border flex items-center justify-center shrink-0 ${on ? 'bg-amber-500 border-amber-500' : 'border-line'}`}
                  >
