...
export function ExpenseReportsModal({ show, onClose, categories, companyName }: ExpenseReportsModalProps) {
    ...
    <div className="fixed inset-0 z-50 flex items-start justify-center pt-6 pb-4 px-4 bg-black/60 backdrop-blur-sm modal-overlay overflow-y-auto">
      <div className="glass-panel rounded-3xl w-full max-w-4xl border border-line flex flex-col">
        ...
        <div className="px-6 py-4 border-b border-line">
          ...
          <div className="flex gap-3 mt-3">
            <button onClick={handleFetchReport} disabled={reportLoading}
              ...
              {reportLoading
                ? <><span className="w-4 h-4 border-2 border-line border-t-white rounded-full animate-spin" />جاري التحميل...</>
        ...
        <div className="overflow-auto max-h-[55vh]">
          {reportData === null ? (
            <div className="flex items-center justify-center py-16 text-ink/30 text-sm">اضغط "عرض التقرير" لتحميل البيانات</div>
          ) : reportData.length === 0 ? (
            <div className="flex items-center justify-center py-16 text-ink/30 text-sm">لا توجد مصروفات بهذه الفلاتر</div>
          ) : (
            <table className="w-full text-sm">
              ...
              <tbody>
                {reportData.map((row, i) => (
                  <tr key={row.id} className={`border-t border-line ${i % 2 === 0 ? '' : 'bg-surface'} hover:bg-surface transition-colors`}>
              ...
              <tfoot>
                <tr className="border-t-2 border-line bg-surface font-bold text-sm">
                  <td colSpan={4} className="px-4 py-2.5 text-ink/60">الإجمالي ({reportData.length} سجل)</td>
