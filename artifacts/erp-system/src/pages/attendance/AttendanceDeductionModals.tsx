...
export function DeductionSettingsModal({
  settings, tiers, onClose, onSaveSettings, onSaveTiers, savingSettings, savingTiers,
}: {
  settings?: AnyRec; tiers: AnyRec[]; onClose: () => void;
  onSaveSettings: (s: AnyRec) => void; onSaveTiers: (t: AnyRec[]) => void;
  savingSettings: boolean; savingTiers: boolean;
}) {
  const [grace, setGrace] = useState(String(settings?.['grace_minutes'] ?? 10));
  const [weeklyOff, setWeeklyOff] = useState<string[]>(String(settings?.['weekly_off_days'] ?? '5').split(',').filter(Boolean));
  ...
  useEffect(() => {
    if (settings) {
      ...
      setWeeklyOff(String(settings['weekly_off_days'] ?? '5').split(',').filter(Boolean));
  ...
  useEffect(() => { if (tiers.length > 0) setEditTiers(tiers.map((t) => ({ ...t }))); }, [tiers]);

  const DAYS = [
    { v: '0', label: 'أحد' }, { v: '1', label: 'اثنين' }, { v: '2', label: 'ثلاثاء' },
    { v: '3', label: 'أربعاء' }, { v: '4', label: 'خميس' }, { v: '5', label: 'جمعة' }, { v: '6', label: 'سبت' },
  ];
  const toggleDay = (v: string) => setWeeklyOff((p) => (p.includes(v) ? p.filter((x) => x !== v) : [...p, v]));
  const addTier = (applies_to: 'late' | 'early') => setEditTiers((p) => [...p, { applies_to, min_minutes: 0, max_minutes: null, amount: 0, is_active: true }]);
  const updTier = (idx: number, k: string, v: unknown) => setEditTiers((p) => p.map((t, i) => (i === idx ? { ...t, [k]: v } : t)));
  ...
  const renderTierRow = (t: AnyRec, idx: number) => (
    <div key={idx} className="grid grid-cols-[1fr_1fr_1fr_auto_auto] gap-2 items-center">
      <input type="number" min="0" className="erp-input text-xs" placeholder="من (دقيقة)" value={String(t['min_minutes'] ?? '')}
        onChange={(e) => updTier(editTiers.indexOf(t), 'min_minutes', e.target.value === '' ? 0 : Number(e.target.value))} />
      <input type="number" min="0" className="erp-input text-xs" placeholder="إلى (فارغ = مفتوح)" value={t['max_minutes'] == null ? '' : String(t['max_minutes'])}
      ...
      <button onClick={() => delTier(editTiers.indexOf(t))} className="text-red-400 hover:text-red-300 p-1" title="حذف"><Trash2 size={14} /></button>
    </div>
  );
      ...
      <div className="erp-modal rounded-xl max-w-3xl w-full my-4">
        ...
        <div className="p-5 space-y-5 max-h-[75vh] overflow-y-auto">
          <section className="space-y-3">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
              <Field label="خصم يوم غياب كامل">
                <input type="number" min="0" step="0.01" className="erp-input w-full" value={absFull} onChange={(e) => setAbsFull(e.target.value)} placeholder="0 = لا يحتسب تلقائياً" />
              </Field>
              <Field label="خصم نصف يوم غياب">
                <input type="number" min="0" step="0.01" className="erp-input w-full" value={absHalf} onChange={(e) => setAbsHalf(e.target.value)} placeholder="0 = لا يحتسب تلقائياً" />
              </Field>
            </div>
            <button onClick={() => onSaveSettings({ grace_minutes: Number(grace), weekly_off_days: weeklyOff.join(','), absence_full_day_amount: Number(absFull), absence_half_day_amount: Number(absHalf), apply_early_leave: applyEarly })}
              disabled={savingSettings} className="erp-btn erp-btn-primary text-sm">
          ...
          <section className="space-y-3">
            ...
            <button onClick={() => addTier('late')} className="erp-btn erp-btn-ghost text-xs flex items-center gap-1"><Plus size={12} /> إضافة شريحة تأخير</button>
            {applyEarly && (
              <>
                ...
                <button onClick={() => addTier('early')} className="erp-btn erp-btn-ghost text-xs flex items-center gap-1"><Plus size={12} /> إضافة شريحة انصراف مبكر</button>
              </>
            )}
            <button onClick={() => onSaveTiers(editTiers)} disabled={savingTiers} className="erp-btn erp-btn-primary text-sm mt-3">
...
export function DeductionCalcModal({
  employees, today, onClose, onPreview, previewData, previewing, onApply, applying, inline = false,
}: {
  employees: AnyRec[]; today: string; onClose: () => void;
  onPreview: (month: string, employee_id: number | null) => void;
  previewData?: AnyRec; previewing: boolean;
  onApply: (items: AnyRec[]) => void; applying: boolean; inline?: boolean;
}) {
  ...
  const toggleExcluded = (k: string) => setExcluded((p) => { const n = new Set(p); if (n.has(k)) n.delete(k); else n.add(k); return n; });
  ...
  const typeLabel = (t: string) => t === 'late' ? 'تأخير' : t === 'early' ? 'انصراف مبكر' : 'غياب';
  const typeColor = (t: string) => t === 'late' ? 'text-amber-300' : t === 'early' ? 'text-orange-300' : 'text-red-400';
    ...
    <>
      ...
      <div className="p-5 space-y-4 max-h-[75vh] overflow-y-auto">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-3 items-end">
          <Field label="الشهر">
            <input type="month" className="erp-input w-full" value={month} onChange={(e) => setMonth(e.target.value)} />
          </Field>
          <Field label="الموظف (اختياري)">
            <select className="erp-input w-full" value={empId} onChange={(e) => setEmpId(e.target.value)}>
              <option value="">— كل الموظفين —</option>
              {employees.map((e) => (<option key={String(e['id'])} value={String(e['id'])}>{String(e['first_name_ar'] ?? '')} {String(e['last_name_ar'] ?? '')}</option>))}
            </select>
          </Field>
          <button onClick={() => { setExcluded(new Set()); onPreview(month, empId ? Number(empId) : null); }} disabled={previewing}
                ...
                <tr>
                  <th className="p-2 text-center w-10">
                    <input type="checkbox" checked={selectableItems.length > 0 && excluded.size === 0}
                      onChange={(e) => setExcluded(e.target.checked ? new Set() : new Set(selectableItems.map(itemKey)))} />
                  </th>
      ...
      <div className="flex gap-2 p-4 border-t border-white/10">
        ...
        {!inline && <button onClick={onClose} className="erp-btn erp-btn-ghost">إلغاء</button>}
      </div>
    </>
  );

  if (inline) return <div className="erp-card rounded-xl overflow-hidden" dir="rtl">{inner}</div>;
  return (
    <div className="fixed inset-0 z-50 bg-black/70 flex items-start justify-center overflow-y-auto p-4" dir="rtl">
