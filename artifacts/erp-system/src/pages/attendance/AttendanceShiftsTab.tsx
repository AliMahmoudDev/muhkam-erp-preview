...
import { Plus, X, Briefcase, Users, Save, ChevronDown, ChevronUp, Pencil, Trash2 } from 'lucide-react';
...
export function AttendanceShiftsTab({
  canManage, empList, today, shifts, shiftsLoading,
    ...
    <div className="space-y-6">
      <div className="erp-card p-5 space-y-4">
        <div className="flex items-center justify-between">
          <h3 className="font-bold text-ink flex items-center gap-2"><Briefcase size={16} className="text-amber-400" /> المناوبات المحفوظة</h3>
          {canManage && (
        ...
        {shiftsLoading ? (
          ...
          <div className="text-center py-8 space-y-2">
            ...
            <p className="text-ink/40 text-sm">لا توجد مناوبات — أضف مناوبة أولى لتحديد مواعيد الدوام</p>
          </div>
        ) : (
          <div className="space-y-3">
            {shifts.map((s) => {
              ...
              return (
                <div key={String(s['id'])} className="flex items-start gap-4 p-4 bg-surface border border-line rounded-xl hover:border-amber-400/30 transition-colors">
                  ...
                  {canManage && (
                    <div className="flex gap-2 shrink-0">
                      ...
                      <button onClick={() => { if (confirm('حذف هذه المناوبة؟')) doDeleteShift.mutate(Number(s['id'])); }}
                        className="erp-btn erp-btn-ghost p-1.5 text-red-400 hover:text-red-300" title="حذف"><Trash2 size={14} /></button>
                    </div>
                  )}
                </div>
              );
      ...
      {canManage && shifts.length > 0 && (
        <div className="erp-card p-5 space-y-4">
          <div className="flex items-center justify-between">
            <h3 className="font-bold text-ink flex items-center gap-2"><Users size={16} className="text-emerald-400" /> تعيين موظف على مناوبة</h3>
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-3 items-end p-4 bg-surface rounded-xl border border-line">
              ...
              <Field label="المناوبة *">
                <select className="erp-input w-full" value={assignForm.shift_id} onChange={(e) => setAssignForm((p) => ({ ...p, shift_id: e.target.value }))}>
                  ...
                  {shifts.map((s) => (<option key={String(s['id'])} value={String(s['id'])}>{String(s['name_ar'])} ({String(s['start_time'])}–{String(s['end_time'])})</option>))}
                </select>
              </Field>
              ...
              <button
                onClick={() => {
                  ...
                  doAssignShift.mutate(
                    { employee_id: Number(assignForm.employee_id), shift_schedule_id: Number(assignForm.shift_id), assigned_date: assignForm.assigned_date },
                    { onSuccess: () => { setShowAssignForm(false); setAssignForm({ employee_id: '', shift_id: '', assigned_date: today }); } }
                  );
                }}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm p-4">
          <div className="erp-modal rounded-2xl shadow-2xl w-full max-w-lg" dir="rtl">
            <div className="flex items-center justify-between p-5 border-b border-line">
              <h2 className="font-bold text-ink flex items-center gap-2"><Briefcase size={16} className="text-amber-400" /> {editShiftId ? 'تعديل مناوبة' : 'مناوبة جديدة'}</h2>
              <button onClick={() => setShowShiftForm(false)} className="text-ink/40 hover:text-ink"><X size={18} /></button>
            </div>
