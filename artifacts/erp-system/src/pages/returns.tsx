...
function ReturnDetailModal({ type, id, onClose }: { type: Tab; id: number; onClose: () => void }) {
  ...
  const originRef =
    type === 'sales' ? data?.sale_no : (data as PurchaseReturn | undefined)?.invoice_no;
      ...
      >
        <div className="flex items-center justify-between px-5 py-4 border-b border-line bg-surface">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="font-bold text-ink">{isLoading ? '...' : data?.return_no}</p>
              <p className="text-xs text-ink/40">
                {type === 'sales' ? 'مرتجع مبيعات' : 'مرتجع مشتريات'}
              </p>
            </div>
          </div>
          <div className="flex items-center gap-2">
            {data && (
              <button
                onClick={handlePrint}
                className="flex items-center gap-1.5 text-xs bg-surface hover:bg-surface text-ink/70 hover:text-ink px-3 py-1.5 rounded-lg transition-colors"
              >
            ...
            <button
              onClick={onClose}
              className="p-1.5 rounded-lg hover:bg-surface text-ink/50 hover:text-ink transition-colors"
            >
              <X className="w-4 h-4" />
            </button>
          </div>
            ...
            <>
              <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
                {[
                  ...
                  {
                    label: 'نوع الاسترداد',
                    value: REFUND_LABELS[data.refund_type ?? ''] ?? data.refund_type ?? '—',
                  },
                  ...
                  <div key={f.label} className="rounded-lg p-3 bg-surface border border-line">
                    <p className="text-[10px] text-ink/40 mb-0.5">{f.label}</p>
                    <p className="text-sm font-semibold text-ink/90">{f.value}</p>
                  </div>
                ))}
              </div>
              {items.length > 0 ? (
                <div className="rounded-xl overflow-hidden border border-line">
                  <table className="w-full text-right text-sm">
                    <thead className="bg-surface">
                      <tr>
                        <th className="py-2.5 px-4 text-xs font-medium text-ink/50">
                          المنتج / الخدمة
                        </th>
                        <th className="py-2.5 px-4 text-xs font-medium text-ink/50 text-center">
                          الكمية
                        </th>
                        <th className="py-2.5 px-4 text-xs font-medium text-ink/50">سعر الوحدة</th>
                        <th className="py-2.5 px-4 text-xs font-medium text-ink/50">الإجمالي</th>
                      </tr>
                    </thead>
                    <tbody>
                      {items.map((item, idx) => (
                        <tr
                          key={item.id}
                          className={idx < items.length - 1 ? 'border-b border-line' : ''}
                        >
                          <td className="py-3 px-4 text-ink/80 font-medium">{item.product_name}</td>
                    ...
                    <tfoot className="bg-surface border-t border-line">
                      <tr>
                        <td colSpan={3} className="py-3 px-4 text-ink/50 text-sm">
                          الإجمالي
                        </td>
                ...
                <p className="text-center text-ink/30 text-sm py-4">لا توجد أصناف مسجّلة</p>
              )}
            </>
          ) : (
            <p className="text-center text-ink/30 text-sm py-8">تعذّر تحميل التفاصيل</p>
          )}
        </div>
      </div>
    </div>
...
function MonthlyBreakdown({
  ...
  return (
    <div className="glass-panel rounded-2xl border border-line overflow-hidden">
      <button
        onClick={() => setOpen((p) => !p)}
        className="w-full flex items-center justify-between px-4 py-3 text-sm font-bold text-ink/70 hover:text-ink transition-colors"
      >
        <span className="flex items-center gap-2">
          <Calendar className="w-4 h-4" /> التوزيع الشهري
        </span>
        ...
        <div className="px-4 pb-4 space-y-2">
          {monthly.map(([month, { count, total }]) => (
            <div key={month} className="flex items-center justify-between text-sm">
              <span className="text-ink/50">{fmtMonth(month + '-01')}</span>
              <div className="flex items-center gap-3">
                <span className="text-ink/30 text-xs">{count} مرتجع</span>
        ...
        <div className="px-4 pb-4 space-y-2">
          {grouped.map(([name, { count, total }]) => (
            <div key={name} className="flex items-center justify-between text-sm">
              <span className="text-ink/70 truncate max-w-[200px]">{name}</span>
              <div className="flex items-center gap-3">
                <span className="text-ink/30 text-xs">{count} مرة</span>
                <span className={`font-bold ${colorClass}`}>{formatCurrency(total)}</span>
              </div>
            </div>
          ))}
...
function ReasonBreakdown({ rows }: { rows: (SaleReturn & PurchaseReturn)[] }) {
  ...
  return (
    <div className="glass-panel rounded-2xl border border-line overflow-hidden">
      ...
      >
        <span className="flex items-center gap-2">
          <Package className="w-4 h-4" /> أسباب الإرجاع
        </span>
        {open ? <ChevronUp className="w-4 h-4" /> : <ChevronDown className="w-4 h-4" />}
      </button>
      {open && (
        <div className="px-4 pb-4 space-y-2">
          {grouped.map(([reason, { count, total }]) => (
            <div key={reason} className="flex items-center justify-between text-sm">
              <span className="text-ink/70 truncate max-w-[200px]">{reason}</span>
              <div className="flex items-center gap-3">
                <span className="text-ink/30 text-xs">{count} مرة</span>
                <span className="font-bold text-ink/60">{formatCurrency(total)}</span>
              </div>
            </div>
          ))}
        </div>
...
export default function Returns() {
  ...
  /* فلترة وترتيب */
  const activeRows = (tab === 'sales' ? salesReturns : purchaseReturns) as (SaleReturn &
    PurchaseReturn)[];
  ...
  const filtered = useMemo(() => {
    ...
    rows = [...rows].sort((a, b) => {
      ...
      if (sortKey === 'date') {
        va = a.date || a.created_at;
        vb = b.date || b.created_at;
      } else if (sortKey === 'amount') {
        va = a.total_amount;
        vb = b.total_amount;
      } else {
        va = a.customer_name || a.supplier_name || '';
        vb = b.customer_name || b.supplier_name || '';
      }
    ...
    <div className="space-y-5" dir="rtl">
      ...
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-xl font-bold text-ink">سجل المرتجعات</h2>
            <p className="text-xs text-ink/30 mt-0.5">
              مرجع كامل — المرتجعات تُنشأ من صفحتَي المبيعات والمشتريات
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        <div className="glass-panel rounded-2xl p-4 border border-orange-500/15">
          <div className="flex items-center gap-2 mb-1">
            ...
            <p className="text-ink/40 text-xs font-bold">مرتجعات مبيعات</p>
          </div>
          <p className="text-xl font-black text-orange-400">{formatCurrency(totalSalesRet)}</p>
          <p className="text-ink/30 text-xs mt-0.5">{salesReturns.length} مرتجع</p>
        </div>
        <div className="glass-panel rounded-2xl p-4 border border-blue-500/15">
          <div className="flex items-center gap-2 mb-1">
            <ArrowDownToLine className="w-4 h-4 text-blue-400" />
            <p className="text-ink/40 text-xs font-bold">مرتجعات مشتريات</p>
          </div>
          <p className="text-xl font-black text-blue-400">{formatCurrency(totalPurchRet)}</p>
          <p className="text-ink/30 text-xs mt-0.5">{purchaseReturns.length} مرتجع</p>
        </div>
        <div className="glass-panel rounded-2xl p-4 border border-red-500/10 col-span-2">
          <div className="flex items-center gap-2 mb-1">
            <TrendingDown className="w-4 h-4 text-red-400" />
            <p className="text-ink/40 text-xs font-bold">إجمالي المرتجعات</p>
          </div>
          <p className="text-2xl font-black text-red-400">
            {formatCurrency(totalSalesRet + totalPurchRet)}
          </p>
          <p className="text-ink/30 text-xs mt-0.5">
            {salesReturns.length + purchaseReturns.length} مرتجع إجمالاً
          </p>
        </div>
      </div>

      {/* ── تاب بار ── */}
      <div className="flex gap-2 bg-surface rounded-2xl p-1.5 border border-line w-fit">
        {(
          [
            {
              id: 'sales',
          ...
          >
            ...
            {t.label}
            <span className={`text-xs ${tab === t.id ? 'text-black/60' : t.color}`}>
              ({t.count})
            </span>
          </button>
        ))}
      ...
      {urlQ && (
        <div className="flex items-center gap-3 px-4 py-3 rounded-2xl bg-amber-500/10 border border-amber-500/25 text-amber-300 text-sm">
          ...
          <span>
            عرض مرتجعات الفاتورة: <strong className="font-mono">{urlQ}</strong>
          </span>
          <button
            onClick={() => {
              setSearch('');
            ...
            className="mr-auto text-ink/40 hover:text-ink text-xs underline"
          >
            عرض الكل
          </button>
        </div>
      ...
      {/* ── أدوات الفلترة ── */}
      <div className="glass-panel rounded-2xl p-3 border border-line flex flex-wrap gap-2 items-center">
        {/* بحث */}
        ...
        <div className="flex items-center gap-1.5">
          <span className="text-ink/30 text-xs">من</span>
          <input
            type="date"
            value={dateFrom}
        ...
        {/* إلى */}
        <div className="flex items-center gap-1.5">
          <span className="text-ink/30 text-xs">إلى</span>
          <input
            type="date"
            value={dateTo}
        ...
        {isFiltered && (
          <button
            ...
            className="text-xs text-ink/40 hover:text-ink flex items-center gap-1 px-2 py-1.5 rounded-lg hover:bg-surface transition-colors"
          >
      ...
      {isFiltered && (
        <div className="flex items-center justify-between text-sm px-1">
          <span className="text-ink/40">
            {filtered.length} نتيجة من {activeRows.length}
          </span>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-right text-ink/80 whitespace-nowrap">
            <thead className="bg-surface border-b border-line">
              <tr>
                <th className="p-4 font-semibold text-ink/50 text-sm">رقم المرتجع</th>
                <th className="p-4 font-semibold text-ink/50 text-sm">الفاتورة الأصلية</th>
                <th
                  className="p-4 font-semibold text-ink/50 text-sm cursor-pointer hover:text-ink/80 select-none"
                ...
                <th className="p-4 font-semibold text-ink/50 text-sm">نوع الاسترداد</th>
                <th className="p-4 font-semibold text-ink/50 text-sm">السبب</th>
                <th
                  className="p-4 font-semibold text-ink/50 text-sm cursor-pointer hover:text-ink/80 select-none"
                ...
                filtered.map((r) => {
                  ...
                  return (
                    <tr key={r.id} className="border-b border-line erp-table-row">
                      ...
                      <td className="p-4">
                        {originRef ? (
                          <span className="font-mono text-xs px-2 py-1 rounded-lg bg-surface text-ink/50 border border-line">
                            {originRef}
                          </span>
                        ) : (
                          <span className="text-ink/20 text-xs">مستقل</span>
                        )}
                      </td>
                      ...
                      <td className="p-4">
                        {r.refund_type ? (
                          <span
                            className={`text-xs px-2 py-1 rounded-lg font-bold border ${REFUND_COLORS[r.refund_type] ?? 'bg-surface text-ink/40 border-line'}`}
                          >
                      ...
                      <td className="p-4 text-ink/50 text-sm max-w-[160px] truncate">
                        {r.reason || '—'}
                      </td>
                      <td className="p-4 text-ink/40 text-sm">{fmt(r.date || r.created_at)}</td>
                      <td className="p-4">
                        <button
                          onClick={() => setDetailId(r.id)}
                          title="تفاصيل وطباعة"
                          className="p-1.5 rounded-lg hover:bg-surface text-ink/30 hover:text-orange-400 transition-colors"
                        >
