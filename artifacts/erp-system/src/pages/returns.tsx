...
function ReturnDetailModal({ type, id, onClose }: { type: Tab; id: number; onClose: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 p-4" onClick={onClose}>
      <div className="w-full max-w-2xl bg-[#1a1a2e] rounded-2xl border border-line shadow-2xl overflow-hidden"
        ...
        <div className="flex items-center justify-between px-5 py-4 border-b border-line bg-surface">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-ink/40">{type === 'sales' ? 'مرتجع مبيعات' : 'مرتجع مشتريات'}</p>
            </div>
          </div>
          <div className="flex items-center gap-2">
            ...
            <button onClick={onClose} className="p-1.5 rounded-lg hover:bg-surface text-ink/50 hover:text-ink transition-colors">
              <X className="w-4 h-4" />
            </button>
          </div>
        </div>
                ...
                <div className="rounded-xl overflow-hidden border border-line">
                  <table className="w-full text-right text-sm">
                    <thead className="bg-surface">
                      <tr>
                        <th className="py-2.5 px-4 text-xs font-medium text-ink/50">المنتج / الخدمة</th>
                        <th className="py-2.5 px-4 text-xs font-medium text-ink/50 text-center">الكمية</th>
                    ...
                    <tbody>
                      {items.map((item, idx) => (
                        <tr key={item.id} className={idx < items.length - 1 ? 'border-b border-line' : ''}>
                          ...
                          <td className="py-3 px-4 text-ink/60">{formatCurrency(item.unit_price)}</td>
                    ...
                    <tfoot className="bg-surface border-t border-line">
                      <tr>
                        <td colSpan={3} className="py-3 px-4 text-ink/50 text-sm">الإجمالي</td>
    ...
    <div className="space-y-5" dir="rtl">
      ...
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-ink/30 mt-0.5">مرجع كامل — المرتجعات تُنشأ من صفحتَي المبيعات والمشتريات</p>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        ...
        <div className="glass-panel rounded-2xl p-4 border border-red-500/10 col-span-2">
          ...
          <p className="text-ink/30 text-xs mt-0.5">{salesReturns.length + purchaseReturns.length} مرتجع إجمالاً</p>
        </div>
      </div>
        ...
        {([
          ...
          <button key={t.id} onClick={() => { setTab(t.id); setSearch(''); setDateFrom(''); setDateTo(''); setFilterRefund('all'); }}
            className={`flex items-center gap-2 px-5 py-2 rounded-xl text-sm font-bold transition-all ${
              tab === t.id ? 'bg-amber-500 text-black' : 'text-ink/50 hover:text-ink hover:bg-surface'
            }`}>
      ...
      {isFiltered && (
        <div className="flex items-center justify-between text-sm px-1">
          <span className="text-ink/40">{filtered.length} نتيجة من {activeRows.length}</span>
          ...
          <table className="w-full text-right text-ink/80 whitespace-nowrap">
            <thead className="bg-surface border-b border-line">
              <tr>
                ...
                <th className="p-4 font-semibold text-ink/50 text-sm">
                  الفاتورة الأصلية
                </th>
                <th className="p-4 font-semibold text-ink/50 text-sm cursor-pointer hover:text-ink/80 select-none"
                ...
                filtered.map((r) => {
                  ...
                  return (
                    <tr key={r.id} className="border-b border-line erp-table-row">
                      ...
                      <td className="p-4">
                        {r.refund_type ? (
                          <span className={`text-xs px-2 py-1 rounded-lg font-bold border ${REFUND_COLORS[r.refund_type] ?? 'bg-surface text-ink/40 border-line'}`}>
                            {REFUND_LABELS[r.refund_type] ?? r.refund_type}
                          </span>
                        ) : '—'}
                      </td>
                      <td className="p-4 text-ink/50 text-sm max-w-[160px] truncate">{r.reason || '—'}</td>
