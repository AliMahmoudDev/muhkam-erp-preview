  ...
  return (
    <div className="grid grid-cols-3 gap-4">
      <div
        ...
        onKeyDown={(e) => {
          if (e.key === 'Enter' || e.key === ' ') setShowZeroOnly(true);
        }}
      ...
      >
        ...
        <div className="flex-1 min-w-0">
          ...
          <p className={`text-2xl font-bold ${zeroCount > 0 ? 'text-red-400' : 'text-ink/30'}`}>
            {zeroCount}
          </p>
      ...
      <div
        ...
        onKeyDown={(e) => {
          if (e.key === 'Enter' || e.key === ' ') setShowZeroOnly(false);
        }}
        className={`rounded-2xl p-4 border flex items-center gap-3 transition-all cursor-pointer select-none hover:-translate-y-0.5 hover:brightness-110 hover:shadow-[0_6px_24px_rgba(0,0,0,0.35)] ${
          !showZeroOnly
      ...
      >
        ...
        <div className="flex-1 min-w-0">
          ...
          <p className={`text-2xl font-bold ${lowCount > 0 ? 'text-amber-400' : 'text-ink/30'}`}>
            {lowCount}
          </p>
          <p className="text-ink/20 text-[10px] mt-0.5">انقر للتصفية</p>
        </div>
      ...
      >
        ...
        <div className="flex-1 min-w-0">
          ...
          <p
            className={`text-2xl font-bold ${zeroCount + lowCount > 0 ? 'text-ink' : 'text-ink/30'}`}
          >
            {zeroCount + lowCount}
          </p>
