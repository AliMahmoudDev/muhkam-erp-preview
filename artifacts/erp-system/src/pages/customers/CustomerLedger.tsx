...
export function CustomerStatementModal({
  customerId, customerName, customerPhone, customerBalance: initialBalance, isSupplier, onClose, safes,
}: {
  customerId: number; customerName: string; customerPhone: string;
  customerBalance: number; isSupplier: boolean; onClose: () => void;
  ...
  const [directPayForm, setDirectPayForm] = useState<DirectPayForm>({ amount: '', safe_id: '', notes: '' });
  ...
  } = useCustomerLedger({ customerId, customerName, isSupplier, activeTab, reportFrom, reportTo, initialBalance });
  ...
  const handlePrint = () => printCustomerStatement({ customerName, customerPhone, customerBalance, rows: rowsWithBalance, summaryCards, companyName, companySlogan });
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/70 backdrop-blur-sm modal-overlay">
      <div className="glass-panel rounded-3xl p-0 w-full max-w-4xl border border-white/10 shadow-2xl max-h-[92vh] overflow-hidden flex flex-col">
          ...
          <div>
            ...
            {isSupplier ? (
              <div className="mt-2 space-y-0.5 text-xs">
                ...
                {totalPurchaseReturns > 0 && <p className="text-white/50">مرتجع مشتريات: <span className="text-violet-400 font-bold">{formatCurrency(totalPurchaseReturns)}</span></p>}
                ...
                <p className={`font-bold text-sm mt-1 ${customerBalance > 0 ? 'text-green-400' : customerBalance < 0 ? 'text-red-400' : 'text-white/40'}`}>
                  الرصيد الصافي: {customerBalance > 0 ? `عليه لنا ${formatCurrency(customerBalance)}` : customerBalance < 0 ? `له علينا ${formatCurrency(Math.abs(customerBalance))}` : 'متسوّى'}
                </p>
              </div>
            ) : (
              <p className={`text-sm mt-1 font-semibold ${customerBalance > 0 ? 'text-yellow-400' : customerBalance < 0 ? 'text-orange-400' : 'text-white/40'}`}>
                الرصيد: {customerBalance > 0 ? `${formatCurrency(customerBalance)} — العميل مدين` : customerBalance < 0 ? `${formatCurrency(Math.abs(customerBalance))} — دائن للعميل` : 'متسوّى'}
              </p>
            )}
          </div>
          <div className="flex items-center gap-2">
            <button onClick={handleWhatsApp} title={customerPhone ? 'إرسال ملخص الكشف على واتساب' : 'لا يوجد رقم هاتف مسجل'}
            ...
            <button onClick={handlePrint} className="flex items-center gap-1.5 bg-blue-500/20 text-blue-400 hover:bg-blue-500/30 px-3 py-2 rounded-xl text-sm font-bold border border-blue-500/30 transition-all">
        ...
        <div className="overflow-y-auto p-6 space-y-6 flex-1">
          <LedgerSummary
            totalSales={totalSales} sales={sales}
            totalPurchases={totalPurchases} purchases={purchases}
            totalReceipts={totalReceipts} receipts={receipts}
            totalPayments={totalPayments} payments={payments}
            totalReturns={totalReturns} returns_={returns_}
            totalSupplierPayments={totalSupplierPayments} supplierPayments={supplierPayments}
            totalPurchaseReturns={totalPurchaseReturns} purchaseReturns={purchaseReturns}
            customerBalance={customerBalance} isSupplier={isSupplier}
          />

          <LedgerFilters
            activeTab={activeTab} setActiveTab={setActiveTab}
            rowsWithBalance={rowsWithBalance} isSupplier={isSupplier} customerBalance={customerBalance}
            reportFrom={reportFrom} setReportFrom={setReportFrom}
            reportTo={reportTo} setReportTo={setReportTo}
            reportData={reportData} reportLoading={reportLoading} reportStmt={reportStmt}
          />

          {activeTab === 'ledger' && (
            <LedgerTable
              ledgerData={ledgerData} ledgerLoading={ledgerLoading}
              isSupplier={isSupplier} safes={safes}
              showDirectPayment={showDirectPayment} setShowDirectPayment={setShowDirectPayment}
              directPayForm={directPayForm} setDirectPayForm={setDirectPayForm}
