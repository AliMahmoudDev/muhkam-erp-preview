...
export function LedgerSummary({
  ...
  return (
    <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
      ...
      {isSupplier ? (
        <div
          className={`${customerBalance > 0 ? 'bg-green-500/10 border-green-500/20' : customerBalance < 0 ? 'bg-red-500/10 border-red-500/20' : 'bg-surface border-line'} border rounded-2xl p-3 text-center`}
        >
          <p
            className={`text-xs mb-1 ${customerBalance > 0 ? 'text-green-400' : customerBalance < 0 ? 'text-red-400' : 'text-ink/40'}`}
          >
            الرصيد الصافي
          </p>
          <p
            className={`font-black ${customerBalance > 0 ? 'text-green-400' : customerBalance < 0 ? 'text-red-400' : 'text-ink/40'}`}
          >
        ...
        <div
          className={`${customerBalance > 0 ? 'bg-amber-500/10 border-amber-500/20' : customerBalance < 0 ? 'bg-blue-500/10 border-blue-500/20' : 'bg-surface border-line'} border rounded-2xl p-3 text-center`}
        >
          <p
            className={`text-xs mb-1 ${customerBalance > 0 ? 'text-amber-400' : customerBalance < 0 ? 'text-blue-400' : 'text-ink/40'}`}
          >
            الرصيد الصافي
          </p>
          <p
            className={`font-black ${customerBalance > 0 ? 'text-amber-400' : customerBalance < 0 ? 'text-blue-400' : 'text-ink/40'}`}
          >
            {formatCurrency(Math.abs(customerBalance))}
          </p>
          <p className="text-ink/40 text-xs">
