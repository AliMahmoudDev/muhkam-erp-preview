...
import { TX_TYPE_CONFIG, REPORT_TYPE_MAP, type TxRow, type ReportData, type ReportStmtRow } from './hooks/useCustomerLedger';
...
export function LedgerFilters({
  ...
  rowsWithBalance, isSupplier, customerBalance,
  reportFrom, setReportFrom, reportTo, setReportTo,
  reportData, reportLoading, reportStmt,
}: LedgerFiltersProps) {
  return (
    <>
          ...
          <div className="flex flex-wrap gap-2 text-xs">
            {Object.entries(TX_TYPE_CONFIG).map(([key, cfg]) => (
              <span key={key} className={`px-2 py-0.5 rounded-lg border ${cfg.bg} ${cfg.color}`}>
                {cfg.icon}{' '}
                {key === 'sale' ? 'مبيعات' : key === 'purchase' ? 'مشتريات' : key === 'receipt' ? 'قبض' : key === 'payment' ? 'توريد' : key === 'supplier_payment' ? 'تسديد دفعة' : key === 'purchase_return' ? 'مرتجع مشتريات' : key === 'return_credit' ? 'مرتجع مبيعات' : 'مرتجع نقدي'}
              </span>
            ))}
          </div>
              ...
              <table className="w-full text-right text-sm">
                ...
                <tbody>
                  {rowsWithBalance.map((r, i) => {
                    ...
                    return (
                      <tr key={`${r.type}-${r.ref ?? ''}-${i}`} className="border-b border-line erp-table-row">
                        <td className="p-3 text-ink/50 text-xs whitespace-nowrap">{r.date ? r.date.split('T')[0] : '—'}</td>
                        <td className="p-3">
                          <span className={`px-2 py-0.5 rounded-lg text-xs font-bold border ${cfg.bg} ${cfg.color}`}>{cfg.icon} {r.label}</span>
                        </td>
                        ...
                        <td className="p-3 text-center font-black">
                          {isSupplier ? (
                            <span className={r.balance > 0 ? 'text-green-400' : r.balance < 0 ? 'text-red-400' : 'text-ink/40'}>
                              {r.balance !== 0 ? `${formatCurrency(Math.abs(r.balance))} ${r.balance > 0 ? 'عليه لنا' : 'له علينا'}` : 'صفر'}
                            </span>
                          ) : (
                            <span className={r.balance > 0 ? 'text-yellow-400' : r.balance < 0 ? 'text-blue-400' : 'text-ink/40'}>
                              {r.balance !== 0 ? `${formatCurrency(Math.abs(r.balance))} ${r.balance > 0 ? 'عليه' : 'دائن'}` : 'صفر'}
                            </span>
                          )}
                        </td>
                      </tr>
                ...
                <tfoot className="bg-surface border-t border-line">
                  <tr>
                    <td colSpan={3} className="p-3 text-ink/60 font-bold text-right">الإجمالي</td>
                    ...
                    <td className="p-3 text-center font-black">
                      {isSupplier ? (
                        <span className={customerBalance > 0 ? 'text-green-400' : customerBalance < 0 ? 'text-red-400' : 'text-ink/40'}>
                          {formatCurrency(Math.abs(customerBalance))} {customerBalance > 0 ? 'عليه لنا' : customerBalance < 0 ? 'له علينا' : ''}
                        </span>
                      ) : (
                        <span className={customerBalance > 0 ? 'text-yellow-400' : customerBalance < 0 ? 'text-blue-400' : 'text-ink/40'}>
      ...
      {activeTab === 'report' && (
        <div className="space-y-4">
          <div className="flex flex-wrap items-center gap-2">
            ...
            <input type="date" value={reportFrom} onChange={(e) => setReportFrom(e.target.value)} className="glass-input rounded-xl px-3 py-1.5 text-sm text-ink" />
            <span className="text-ink/40 text-xs font-bold">إلى</span>
            <input type="date" value={reportTo} onChange={(e) => setReportTo(e.target.value)} className="glass-input rounded-xl px-3 py-1.5 text-sm text-ink" />
          </div>
          ...
          <div className="glass-panel rounded-2xl overflow-hidden border border-line">
            <div className="overflow-x-auto">
              <table className="w-full text-right text-sm whitespace-nowrap">
                ...
                <tbody>
                  {reportLoading ? (
                    <tr><td colSpan={6} className="p-8 text-center text-ink/40 text-xs">جاري التحميل...</td></tr>
                  ) : reportStmt.length === 0 ? (
                    <tr><td colSpan={6} className="p-12 text-center text-ink/40">لا توجد حركات في هذه الفترة</td></tr>
                  ) : (
                    reportStmt.map((row, i) => {
                      const meta = REPORT_TYPE_MAP[row.type] ?? { label: row.type, cls: 'text-ink/50' };
                      return (
                        <tr key={`${row.date ?? ''}-${row.type}-${row.reference_no ?? ''}-${i}`} className="border-b border-line erp-table-row">
                          ...
                          <td className="p-3 text-ink/70">
                            {row.description}
                            {row.reference_no && <span className="text-ink/30 text-xs mr-2">{row.reference_no}</span>}
                          </td>
                          ...
                          <td className={`p-3 font-black ${row.balance >= 0 ? 'text-ink' : 'text-red-400'}`}>{formatCurrency(row.balance)}</td>
                        </tr>
                      );
                    })
                  )}
