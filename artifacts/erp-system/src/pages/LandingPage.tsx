...
/* Feature cards */
.lp-card { transition:transform .3s cubic-bezier(.16,1,.3,1),box-shadow .3s ease,border-color .3s ease; cursor:pointer; }
.lp-card:hover {
...
══════════════════════════════════════════════ */
const BENTO = [
  /* Row 1: wide(2) + single(1) = 3 */
  { icon:'📊', title:'المحاسبة الكاملة',
    desc:'قيد مزدوج تلقائي، ميزانية عمومية، تقارير مالية كاملة، مراكز التكلفة والميزانية التقديرية.',
  ...
  { icon:'📱', title:'تطبيق موبايل لفريقك كله',
    desc:'iOS وAndroid — حضور GPS، بطاقات صيانة، فواتير، ومتابعة الأداء في راحة يدك أينما كنت.',
    grad:'linear-gradient(135deg,rgba(168,85,247,.14),rgba(99,102,241,.08))', accent:'#a855f7',
    cls:'lp-full', tags:['iOS','Android','حضور GPS','بطاقات صيانة','فواتير','تقارير'] },
];
    ...
    <>
      ...
      <section dir="rtl" style={{
        ...
        <div style={{ position:'relative', zIndex:10, maxWidth:780, width:'100%' }}>
          ...
          <div style={{ display:'flex', flexWrap:'wrap', gap:16, justifyContent:'center', marginBottom:28, animation:'lp-slide-up 1s .24s ease both' }}>
            <button onClick={goLogin} className="lp-shim" style={{ ...amberBtn, fontSize:16 }}
      ...
      <section id="features" dir="rtl" style={{ padding:'110px 28px', background:'#000', ...mono }}>
        <div style={{ maxWidth:1200, margin:'0 auto' }}>
          ...
          {/* 3-column bento grid — always fills rows cleanly */}
          <div className="lp-bento" style={{ display:'grid', gridTemplateColumns:'repeat(3,1fr)', gap:16 }}>
            {BENTO.map((f, i) => (
              ...
              >
                ...
                <div style={{ position:'absolute', bottom:0, right:0, width:120, height:120, background:`radial-gradient(circle at bottom right,${f.accent}20,transparent 70%)`, pointerEvents:'none' }} />

                <div style={{ fontSize: f.cls==='lp-full' ? 38 : 32, marginBottom:14, lineHeight:1 }}>{f.icon}</div>
                <h3 style={{ fontSize: f.cls==='lp-full' ? 22 : 18, fontWeight:800, color:'#fff', marginBottom:10, lineHeight:1.3 }}>{f.title}</h3>
                <p style={{ fontSize:14, color:'rgba(255,255,255,.5)', lineHeight:1.75, flex:1 }}>{f.desc}</p>
                  ...
                  <div style={{ marginTop:18, display:'flex', gap:8, flexWrap:'wrap' }}>
                    {f.tags.map(tag => (
                      <span key={tag} style={{
                        ...
                        color:f.accent, fontSize:12, fontWeight:700,
                      }}>{tag}</span>
                    ))}
                  </div>
                )}

                {/* "ابدأ الآن" hover arrow */}
                <div style={{ marginTop:18, display:'flex', alignItems:'center', gap:6, color:`${f.accent}`, fontSize:13, fontWeight:700, opacity:0.7 }}>
                  <span>ابدأ الآن</span><span style={{ fontSize:16 }}>←</span>
                </div>
              </div>
            ))}
          </div>
      ...
      <section dir="rtl" style={{ padding:'80px 28px', background:'rgba(255,255,255,.012)', ...mono }}>
        <div style={{ maxWidth:1100, margin:'0 auto', display:'flex', flexDirection:'column', gap:110 }}>
          ...
          {/* Spotlight 1 — Repair (text right / mockup left in RTL) */}
          ...
          {/* Spotlight 2 — Accounting (flip via ltr grid) */}
        ...
        <div style={{ maxWidth:880, margin:'0 auto' }}>
          ...
          <div className="lp-grid-2" style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:22 }}>
            ...
            >
              ...
              <button onClick={goLogin} style={{ padding:14, borderRadius:13, border:'1px solid rgba(255,255,255,.16)', background:'transparent', color:'#fff', fontSize:14, fontWeight:700, cursor:'pointer', ...mono, transition:'all .2s' }}
            ...
            <div className="lp-fade lp-d2" style={{ borderRadius:26, padding:3, position:'relative', background:'linear-gradient(135deg,#f59e0b,#f97316,#ef4444,#f59e0b)', backgroundSize:'200% 200%', animation:'lp-gradient-pan 4s ease infinite' }}>
              ...
              <div style={{ background:'rgba(8,8,8,.96)', borderRadius:23, padding:32, display:'flex', flexDirection:'column', height:'100%', boxSizing:'border-box', backdropFilter:'blur(20px)' }}>
                ...
                <button onClick={goLogin} className="lp-shim" style={{ padding:14, borderRadius:13, border:'none', cursor:'pointer', ...mono, background:'linear-gradient(135deg,#f59e0b,#f97316)', color:'#000', fontSize:14, fontWeight:900, boxShadow:'0 0 32px rgba(245,158,11,.45)', transition:'transform .2s,box-shadow .2s' }}
      ...
      <section dir="rtl" style={{ padding:'132px 28px', textAlign:'center', background:'rgba(255,255,255,.012)', position:'relative', overflow:'hidden', ...mono }}>
        ...
        <div style={{ position:'absolute', top:'50%', left:'50%', transform:'translate(-50%,-50%)', width:600, height:600, borderRadius:'50%', border:'1px solid rgba(245,158,11,.04)', animation:'lp-spin-slow 30s linear infinite', pointerEvents:'none' }} />
        ...
        <div className="lp-fade" style={{ position:'relative', zIndex:10, maxWidth:640, margin:'0 auto' }}>
          ...
          <button onClick={goLogin} className="lp-shim" style={{ ...amberBtn, fontSize:18, padding:'18px 56px' }}
