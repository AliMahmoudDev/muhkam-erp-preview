...
export default function RepairTrack() {
  ...
  useEffect(() => {
    ...
    if (!token) {
      setError('رابط التتبع غير صالح — أعد مسح رمز QR');
      setLoading(false);
      return;
    }
    let alive = true;
    (async () => {
      try {
        ...
        if (!alive) return;
        if (r.status === 401) {
          setError('رابط التتبع غير صالح أو منتهي الصلاحية — أعد مسح رمز QR');
        } else if (r.status === 404) {
          setError('لم يتم العثور على طلب بهذا الرقم');
        } else if (r.status === 429) {
          setError('محاولات كثيرة — انتظر دقيقة وحاول مجدداً');
        } else if (!r.ok) {
          setError('تعذر تحميل بيانات التتبع');
        } else {
          const j = (await r.json()) as TrackData;
          setData(j);
        }
      } catch {
        if (alive) setError('خطأ في الاتصال — حاول لاحقاً');
      } finally {
        ...
        {!loading && !error && data && (
          <div className="space-y-4">
            ...
            {data.status && (
              ...
              >
                <p className="text-[11px] text-ink/40 uppercase tracking-wider mb-2">
                  الحالة الحالية
                </p>
                <div className="flex items-center gap-3">
                  <div
                    ...
                    style={{
                      ...
                      boxShadow: `0 0 12px ${data.status.color}`,
                    }}
                  />
                ...
                {data.estimated_delivery && data.status.key !== 'delivered' && (
                  <p className="text-[11px] text-ink/40 mt-3 flex items-center gap-1.5">
                    <Clock className="w-3 h-3" /> التسليم المتوقع:{' '}
                    {fmtDateOnly(data.estimated_delivery)}
                  </p>
                )}
                {data.delivered_at && (
                        ...
                        <li key={h.at ?? `history-${i}`} className="flex items-start gap-3">
                          ...
                          <div className="flex-1 min-w-0">
                            <p className="text-[12px] text-ink/70">
                              تحوّلت الحالة إلى{' '}
                              <span className="font-bold text-ink/90">{h.to?.label ?? '—'}</span>
                            </p>
            ...
            {data.qa_report && (
              <div className="rounded-2xl border border-line bg-surface overflow-hidden">
                ...
                <div className="px-5 py-4 space-y-2">
                  <p className="text-[13px] text-ink/70 whitespace-pre-wrap leading-relaxed">
                    {data.qa_report}
                  </p>
                  {data.qa_inspector_name && (
                    <p className="text-[11px] text-ink/40 border-t border-line pt-2 mt-2">
                      الفاحص:{' '}
                      <span className="text-ink/60 font-medium">{data.qa_inspector_name}</span>
                    </p>
                  )}
                </div>
              </div>
