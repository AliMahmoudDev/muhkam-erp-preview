...
export function RegisterForm({ onSuccess, onSwitch }: RegisterFormProps) {
    ...
    return (
      <div style={{ textAlign: 'center', padding: '20px 0' }} dir="rtl">
        <div style={{
          ...
          <svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="#3B82F6" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true">
            <polyline points="20 6 9 17 4 12" />
          </svg>
        </div>
        <h3 style={{ fontSize: 18, fontWeight: 700, color: '#F8FAFC', marginBottom: 8, letterSpacing: '-0.01em' }}>
        ...
        {createdUsername && (
          <div style={{
            ...
            <p style={{ fontSize: 12, color: '#64748B', fontWeight: 600, marginBottom: 4 }}>اسم المستخدم الخاص بك</p>
            <p style={{ fontSize: 16, fontWeight: 700, color: '#60A5FA', letterSpacing: '0.04em', direction: 'ltr', fontFamily: "'Inter', monospace" }}>{createdUsername}</p>
            <p style={{ fontSize: 11, color: '#475569', marginTop: 6 }}>احتفظ به — ستحتاجه لتسجيل الدخول</p>
          </div>
        )}
  ...
  return (
    <form onSubmit={handleSubmit} noValidate dir="rtl" aria-label="نموذج إنشاء حساب جديد">
      ...
      {/* Submit */}
      <button type="submit" disabled={loading} className="ent-btn-primary" style={{ marginBottom: 14 }}>
        {loading ? (
          <>
      ...
      <div style={{ textAlign: 'center', fontSize: 13, color: '#64748B' }}>
        لديك حساب بالفعل؟{' '}
        <button type="button" onClick={onSwitch} className="ent-link">سجّل الدخول</button>
      </div>
    </form>
  );
}
