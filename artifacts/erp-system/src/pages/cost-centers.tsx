...
export default function CostCentersPage() {
  ...
  const { data: centers = [], isLoading } = useQuery<CostCenter[]>({
    ...
    queryFn: () =>
      authFetch(api('/api/cost-centers')).then(async (r) => {
  ...
  const addMutation = useMutation({
    mutationFn: (data: typeof form) =>
      authFetch(api('/api/cost-centers'), { method: 'POST', body: JSON.stringify(data) }).then(
        async (r) => {
  ...
  const deleteMutation = useMutation({
    mutationFn: (id: number) =>
      authFetch(api(`/api/cost-centers/${id}`), { method: 'DELETE' }).then(async (r) => {
        if (!r.ok) {
          const d = await r.json();
    ...
    <div className="erp-page">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="space-y-3">
          <h2 className="text-sm font-medium text-ink/60 uppercase tracking-wider">
            مراكز التكلفة
          </h2>
          {isLoading ? (
            ...
            centers.map((cc) => (
              ...
              >
                <div className="flex items-center justify-between mb-1">
                  ...
                  <Badge
                    className={
                      cc.is_active ? 'bg-green-500/15 text-green-400' : 'bg-surface text-ink/40'
                    }
                  >
            ...
            <>
              <div className="flex items-center gap-3 flex-wrap">
                ...
                <div className="flex gap-2 items-center">
                  <Input
                    type="date"
                    className="bg-surface border-line text-ink h-8 text-xs w-36"
                    value={dateFrom}
                  ...
                  <Button
                    ...
                    onClick={() => {
                      if (confirm('حذف مركز التكلفة؟')) deleteMutation.mutate(selected.id);
                    }}
                  >
                  ...
                  <div className="grid grid-cols-3 gap-3">
                    {[
                      ...
                      },
                      {
                        ...
                        color: report.summary.net_income >= 0 ? 'text-blue-400' : 'text-red-400',
                        icon: BarChart3,
                      },
                    ].map((card) => (
                      <div
                        ...
                        <tbody className="divide-y divide-white/5">
                          {report.lines.map((line) => (
                            <tr key={line.account_code} className="hover:bg-surface">
                              ...
                              <td className="px-4 py-2.5 font-mono text-ink/70">
                                {formatCurrency(line.total_debit)}
                              </td>
                              <td className="px-4 py-2.5 font-mono text-ink/70">
                                {formatCurrency(line.total_credit)}
                              </td>
                              <td
                                className={`px-4 py-2.5 font-mono font-semibold ${line.net >= 0 ? 'text-green-400' : 'text-red-400'}`}
                              >
      ...
      <Dialog open={showAdd} onOpenChange={setShowAdd}>
        <DialogContent className="bg-card border-border text-card-foreground max-w-md" dir="rtl">
          <DialogHeader>
            <DialogTitle>إضافة مركز تكلفة</DialogTitle>
          </DialogHeader>
