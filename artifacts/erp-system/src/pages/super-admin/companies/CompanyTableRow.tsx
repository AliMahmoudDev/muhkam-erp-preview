...
export function CompanyTableRow({
  co, idx, isExpanded, setExpandedId,
  expiryInfo, coMutate, resetPassword,
  setPanelTab, setSubModal, setSubForm,
  setDeleteTarget, setDeleteCoErr,
  DEFAULT_FEATS_ULTIMATE, DEFAULT_FEATS_ADVANCED,
}: {
  co: Company; idx: number;
  isExpanded: boolean;
  ...
  return (
    <div style={{ borderBottom: `1px solid ${C.border}` }}>
      ...
      >
        ...
        <div style={{ textAlign: 'center' }}>
          <div style={{ fontSize: '11px', fontWeight: 700, color: C.muted }}>{translatePlan(co.plan_type)}</div>
          <span style={{ display: 'inline-block', marginTop: '3px', padding: '2px 8px', borderRadius: '10px', fontSize: '10px', fontWeight: 700, background: co.edition === 'advanced' ? 'rgba(245,158,11,0.15)' : 'rgba(99,102,241,0.15)', color: co.edition === 'advanced' ? '#fcd34d' : '#a5b4fc', border: `1px solid ${co.edition === 'advanced' ? 'rgba(245,158,11,0.3)' : 'rgba(99,102,241,0.3)'}` }}>
      ...
      {isExpanded && (
        <div style={{ padding: '16px 24px 20px', background: 'rgba(15,23,42,0.6)', borderTop: `1px solid ${C.border}` }}>
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: '10px', alignItems: 'center' }}>
            {!co.is_active && (
              <ActionBtn label="تفعيل الشركة" icon="✅" color={C.success} onClick={() => coMutate.mutate({ url: `/api/super/companies/${co.id}/activate` })} />
            )}
            {co.is_active && (
              <ActionBtn label="إيقاف الشركة" icon="⛔" color={C.danger} onClick={() => coMutate.mutate({ url: `/api/super/companies/${co.id}/suspend` })} />
            )}
            <ActionBtn
              ...
              onClick={() => {
                ...
                const feats: CompanyFeatures = co.features ? { ...defaultFeats, ...co.features } : { ...defaultFeats };
                setPanelTab(0);
                setSubModal(co);
                setSubForm({ plan_type: co.plan_type, edition: ed, extend_mode: 'days', extend_days: 30, end_date: co.end_date?.slice(0, 10) ?? '', is_active: co.is_active, features: feats });
              }}
            />
