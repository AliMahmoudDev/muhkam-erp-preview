  ...
  return (
    ...
    >
      <div
        ...
        style={{ background: 'var(--text-1)', borderRadius: '24px', padding: '40px', maxWidth: '500px', width: '100%', direction: 'rtl', boxShadow: '0 30px 80px rgba(0,0,0,0.4)', border: '2px solid #f97316' }}
      >
        <div style={{ textAlign: 'center', marginBottom: '28px' }}>
          ...
          <div style={{ fontSize: '13px', color: 'var(--text-2)', marginTop: '4px', fontFamily: FONT }}>{createResult.company_name}</div>
        </div>
        <div style={{ background: '#fff7ed', border: '1.5px solid #fed7aa', borderRadius: '16px', padding: '20px', marginBottom: '20px' }}>
          <div style={{ fontSize: '12px', color: 'var(--status-danger)', fontWeight: 700, fontFamily: FONT, marginBottom: '14px', display: 'flex', alignItems: 'center', gap: '6px' }}>
          ...
          {[
            ...
            <div key={row.label} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '10px 0', borderBottom: '1px solid #fed7aa' }}>
              <span style={{ fontSize: '12px', color: 'var(--status-warning)', fontFamily: FONT }}>{row.label}</span>
              <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                {row.mono ? (
                  <code style={{ fontSize: row.secret ? '18px' : '14px', fontWeight: 800, color: row.secret ? '#ea580c' : 'var(--bg-elevated)', background: row.secret ? '#fff7ed' : 'var(--text-1)', padding: '4px 10px', borderRadius: '8px', letterSpacing: row.secret ? '2px' : '0.5px', fontFamily: 'monospace' }}>
                    {row.value}
                  </code>
                ) : (
                  <span style={{ fontSize: '14px', fontWeight: 700, color: 'var(--bg-elevated)', fontFamily: FONT }}>{row.value}</span>
                )}
                <button onClick={() => void navigator.clipboard.writeText(row.value)} title="نسخ" style={{ padding: '4px 8px', borderRadius: '6px', border: '1px solid #fed7aa', background: 'var(--text-1)', cursor: 'pointer', fontSize: '12px' }}>📋</button>
              </div>
            </div>
          ))}
        </div>
        <button
          ...
          style={{ width: '100%', padding: '13px', borderRadius: '12px', border: 'none', background: 'linear-gradient(135deg,#f97316,#ea580c)', color: 'var(--text-1)', fontSize: '14px', fontWeight: 700, cursor: 'pointer', fontFamily: FONT, marginBottom: '10px' }}
        >
          📋 نسخ كل بيانات الدخول
        </button>
        <div style={{ background: '#fef3c7', borderRadius: '10px', padding: '12px', fontSize: '12px', color: 'var(--status-warning)', fontFamily: FONT, lineHeight: '1.6', marginBottom: '16px' }}>
          ⚠️ احفظ كلمة المرور الآن — لن تظهر مجدداً.
        </div>
        <button onClick={() => setCreateResult(null)} style={{ width: '100%', padding: '12px', borderRadius: '12px', border: '1px solid #e2e8f0', background: 'var(--text-1)', color: 'var(--text-2)', fontSize: '14px', fontWeight: 700, cursor: 'pointer', fontFamily: FONT }}>
          تم — إغلاق
        </button>
      </div>
    </div>
