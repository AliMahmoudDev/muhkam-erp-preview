...
interface Props {
  ...
  mgName: string; setMgName: (v: string) => void;
  mgUser: string; setMgUser: (v: string) => void;
  mgPin: string;  setMgPin: (v: string) => void;
  mgPin2: string; setMgPin2: (v: string) => void;
  mgErr: string;
  eName: string; setEName: (v: string) => void;
  eUser: string; setEUser: (v: string) => void;
  ePin: string;  setEPin: (v: string) => void;
  ePin2: string; setEPin2: (v: string) => void;
  eErr: string;
  handleAddMgr: () => void;
  handleEditMgr: () => void;
  resetAddForm: () => void;
...
export function ManagerModals({
  ...
  editMgr, setEditMgr,
  deleteMgr, setDeleteMgr, deleteMgrErr, setDeleteMgrErr,
  ...
  handleAddMgr, handleEditMgr,
  resetAddForm, resetEditForm,
  mgCreatePending, mgUpdatePending,
  mgDeleteMutate, mgDeletePending,
}: Props) {
  return (
    <>
      {deleteMgr && (
      ...
      {showAddMgr && (
        <Modal
          ...
          onClose={() => { setShowAddMgr(false); resetAddForm(); }}
        >
          ...
          {mgErr && (
            <div style={{
              ...
              fontSize: '13px', color: C.danger, marginBottom: '14px',
            }}>⚠️ {mgErr}</div>
          )}
          <div style={{ display: 'flex', gap: '10px' }}>
            ...
            <button
              onClick={() => { setShowAddMgr(false); resetAddForm(); }}
              style={{
      ...
      {editMgr && (
        <Modal
          ...
          onClose={() => { setEditMgr(null); resetEditForm(); }}
        >
          ...
          {ePin && (
            <DarkInput label="تأكيد الرقم السري الجديد" value={ePin2} onChange={setEPin2} type="password" placeholder="أعد كتابة الرقم السري الجديد" />
          )}
          {eErr && (
            <div style={{
              padding: '10px 14px', borderRadius: '10px',
          ...
          <div style={{ display: 'flex', gap: '10px' }}>
            ...
            <button
              onClick={() => { setEditMgr(null); resetEditForm(); }}
              style={{
                flex: 1, padding: '12px', borderRadius: '10px',
                border: `1px solid ${C.border}`, background: 'transparent',
                color: C.muted, fontSize: '14px', fontWeight: 700,
