...
interface Props {
  ...
  annCompanyId: string; setAnnCompanyId: (v: string) => void;
  annTitle: string;     setAnnTitle: (v: string) => void;
  annBody: string;      setAnnBody: (v: string) => void;
  annExpires: string;   setAnnExpires: (v: string) => void;
  annSaving: boolean;
  onSave: () => void;
...
export function TabAnnouncements({
  annData,
  annType, setAnnType,
  annTarget, setAnnTarget,
  annCompanyId, setAnnCompanyId,
  annTitle, setAnnTitle,
  annBody, setAnnBody,
  annExpires, setAnnExpires,
  annSaving, onSave, onToggle, onDelete,
}: Props) {
  return (
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '12px' }}>
            ...
            {annTarget === 'specific' && (
              <div>
                ...
                <input
                  type="number" value={annCompanyId}
                  onChange={e => setAnnCompanyId(e.target.value)}
                  placeholder="مثال: 5"
                  style={{ width: '100%', padding: '10px 14px', borderRadius: '10px', border: `1px solid ${C.border}`, background: C.bg, color: C.text, fontSize: '13px', fontFamily: FONT, boxSizing: 'border-box' }}
                />
              </div>
            )}
          </div>
          ...
          <div>
            ...
            <textarea
              ...
              placeholder="اكتب تفاصيل الإشعار هنا..." rows={3}
              style={{ width: '100%', padding: '10px 14px', borderRadius: '10px', border: `1px solid ${C.border}`, background: C.bg, color: C.text, fontSize: '13px', fontFamily: FONT, resize: 'vertical', boxSizing: 'border-box' }}
            />
          </div>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr auto', gap: '12px', alignItems: 'flex-end' }}>
            <div>
              ...
              <input
                type="datetime-local" value={annExpires} onChange={e => setAnnExpires(e.target.value)}
                style={{ width: '100%', padding: '10px 14px', borderRadius: '10px', border: `1px solid ${C.border}`, background: C.bg, color: C.text, fontSize: '13px', fontFamily: FONT, boxSizing: 'border-box' }}
              />
            </div>
