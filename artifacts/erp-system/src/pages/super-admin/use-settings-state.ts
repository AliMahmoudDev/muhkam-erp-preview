...
import { type BackupFile, type TgConfig, type TgBotStatus, type ActiveTab, authHeaders } from './types';
...
export function useSettingsState(
  ...
  const { data: backupData, refetch: refetchBackups } = useQuery<{ backups: BackupFile[]; total: number }>({
    ...
    queryFn: () => fetcher('/api/super/backup/list') as Promise<{ backups: BackupFile[]; total: number }>,
  ...
  const { data: tgConfigData, isLoading: tgLoading, isError: tgError, error: tgErrorObj, refetch: tgRefetch } =
  ...
  const { data: tgBotData, isLoading: tgBotLoading, refetch: tgBotRefetch } =
  ...
  useEffect(() => {
    if (tgBotData) {
      ...
      if (!tgBotData.connected) { setTgBotToken(''); setTgChatId(tgBotData.chat_id ?? ''); }
    }
  }, [tgBotData]);
  ...
  async function saveSupportSettings() {
    ...
    try {
      const res = await authFetch(api('/api/super/support-settings'), {
        ...
        body: JSON.stringify({ support_whatsapp: supportWa.trim(), support_email: supportEmail.trim() }),
      });
  ...
  async function triggerBackup() {
    ...
    try {
      const res = await authFetch(api('/api/super/backup/create'), { method: 'POST', headers: authHeaders() });
      const data = await res.json() as { success?: boolean; message?: string; filename?: string; size_mb?: string; error?: string };
      if (data.success) { showToast(`✅ ${data.message ?? 'تم إنشاء النسخة الاحتياطية'} (${data.size_mb} MB)`); void refetchBackups(); }
      else showToast(data.error ?? 'فشل إنشاء النسخة الاحتياطية', 'error');
    } catch { showToast('فشل إنشاء النسخة الاحتياطية', 'error'); }
  ...
  async function downloadBackup(filename: string) {
    ...
    try {
      const res = await authFetch(api(`/api/super/backup/download/${encodeURIComponent(filename)}`), { headers: authHeaders() });
      if (!res.ok) { showToast('فشل التنزيل', 'error'); return; }
      const blob = await res.blob();
      const a = document.createElement('a');
      a.href = URL.createObjectURL(blob); a.download = filename; a.click(); URL.revokeObjectURL(a.href);
    } catch { showToast('خطأ في التنزيل', 'error'); }
    finally { setDownloadingFile(null); }
  }

  function openRestorePicker() {
    setRestoreOk(null); setRestoreErr(null);
  ...
  async function handleRestoreFileChange(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    if (!file.name.endsWith('.json') && !file.name.endsWith('.json.enc')) {
      showToast('يجب اختيار ملف .json أو .json.enc', 'error'); return;
    }
    setPendingRestoreFile(file); setRestoreCode(''); setRestoreModal(true);
  }

  async function confirmRestore() {
    if (!pendingRestoreFile || restoreCode !== 'RESTORE') return;
    setRestoreModal(false); setRestoring(true); setRestoreOk(null); setRestoreErr(null);
    try {
      ...
      setRestoreErr(msg); showToast('فشل الاستعادة: ' + msg, 'error');
    } finally { setRestoring(false); setPendingRestoreFile(null); }
  }
  ...
  async function loadEncKey() {
    ...
    try {
      ...
      setEncEnabled(data.enabled); setEncKey(data.key ?? ''); setEncKeyVisible(true);
    } catch { showToast('فشل جلب مفتاح التشفير', 'error'); }
    finally { setEncKeyLoading(false); }
  }

  function copyEncKey() {
    if (!encKey) return;
    void navigator.clipboard.writeText(encKey).then(() => { setEncKeyCopied(true); setTimeout(() => setEncKeyCopied(false), 2500); });
  }

  function downloadEncKey() {
    if (!encKey) return;
    const now = new Date();
    const content = ['# مفتاح تشفير النسخ الاحتياطية — MUHKAM ERP', `# تاريخ التصدير: ${now.toLocaleString('ar-EG')}`, '# ⚠️ احتفظ بهذا الملف في مكان آمن — بدونه لا يمكن فك تشفير أي نسخة احتياطية', '', encKey].join('\n');
    const blob = new Blob([content], { type: 'text/plain;charset=utf-8' });
    const url = URL.createObjectURL(blob);
    const a = document.createElement('a');
    a.href = url; a.download = `muhkam-backup-key-${now.toISOString().slice(0, 10)}.key`;
  ...
  function emailEncKey() {
    ...
    const body = encodeURIComponent(`مفتاح تشفير النسخ الاحتياطية لنظام مُحكم:\n\n${encKey}\n\nالتاريخ: ${new Date().toLocaleString('ar-EG')}\n\n⚠️ تحذير: احتفظ بهذا المفتاح في مكان آمن.`);
  ...
  async function confirmTotpSetup() {
    ...
    setSecLoading(true); setSecMsg(null);
    try {
      const res = await authFetch(api('/api/auth/2fa/verify'), { method: 'POST', headers: authHeaders(), body: JSON.stringify({ token: totpInput }) });
      const data = await res.json() as { success?: boolean; message?: string; error?: string };
      if (data.success) { setSecMsg({ text: data.message ?? 'تم تفعيل 2FA ✅', ok: true }); setTotpSetupData(null); setTotpInput(''); void refetchTotpStatus(); }
  ...
  async function confirmDisableTotp() {
    ...
    setSecLoading(true); setSecMsg(null);
    try {
      const res = await authFetch(api('/api/auth/2fa/disable'), { method: 'POST', headers: authHeaders(), body: JSON.stringify({ token: disableTotpInput }) });
      const data = await res.json() as { success?: boolean; message?: string; error?: string };
      if (data.success) { setSecMsg({ text: data.message ?? 'تم إيقاف 2FA', ok: true }); setShowDisable(false); setDisableTotpInput(''); void refetchTotpStatus(); }
      else setSecMsg({ text: data.error ?? 'رمز خاطئ', ok: false });
    } catch { setSecMsg({ text: 'فشل الاتصال', ok: false }); }
    finally { setSecLoading(false); }
  }
  ...
  async function saveTelegramSettings() {
    ...
    setTgSaving(true);
    try {
      await authFetch(api('/api/super/telegram-settings'), { method: 'PUT', headers: authHeaders(), body: JSON.stringify(tgConfig) });
  ...
  async function saveTgCredentials() {
    if (!tgBotToken.trim() || !tgChatId.trim()) { showToast('أدخل رمز البوت ومعرّف المحادثة', 'error'); return; }
    setTgCredSaving(true); setTgTestResult(null);
    try {
      const res = await authFetch(api('/api/super/telegram-config'), { method: 'PUT', headers: authHeaders(), body: JSON.stringify({ bot_token: tgBotToken.trim(), chat_id: tgChatId.trim() }) });
      ...
      setTgBotStatus(json); setTgBotToken('');
      void tgBotRefetch();
      showToast(json.connected ? `✅ تم الاتصال بـ @${json.bot_username ?? 'البوت'}` : '⚠️ تم الحفظ لكن البوت لم يتصل', json.connected ? 'success' : 'error');
    } catch { showToast('فشل حفظ البيانات', 'error'); }
    finally { setTgCredSaving(false); }
  }

  async function testTelegramConnection() {
    setTgTesting(true); setTgTestResult(null);
    try {
      const res = await authFetch(api('/api/super/telegram-test'), { method: 'POST', headers: authHeaders(), body: JSON.stringify({}) });
    ...
    } catch { setTgTestResult({ ok: false, msg: 'تعذّر الاتصال بالخادم' }); }
  ...
  return {
    ...
    triggerBackup, downloadBackup, openRestorePicker, restoreInputRef,
    ...
    disableTotpInput, setDisableTotpInput,
    secLoading, secMsg, setSecMsg, showDisable, setShowDisable,
    startTotpSetup, confirmTotpSetup, confirmDisableTotp,
    /* telegram */
