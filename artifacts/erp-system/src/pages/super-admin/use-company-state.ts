...
export function useCompanyState(
  ...
  const [subForm, setSubForm] = useState<SubForm>({
    ...
    end_date: '',
    is_active: true,
    features: DEFAULT_FEATS_ULTIMATE,
  });
  ...
  const {
    ...
    isLoading: coLoading,
    isError: coError,
    isFetching: coFetching,
    refetch: coRefetch,
    dataUpdatedAt: coUpdatedAt,
  } = useQuery<Company[]>({
  ...
  const { data: panelAuditResp, isLoading: panelAuditLoading } = useQuery<AuditLogResp>({
    ...
    queryFn: () =>
      fetcher('/api/super/audit-log?record_type=company&limit=500') as Promise<AuditLogResp>,
  ...
  const { data: snapshotData, isLoading: snapshotLoading } = useQuery<SnapshotData>({
    ...
    queryFn: () =>
      fetcher(`/api/super/companies/${snapshotCompany}/snapshot`) as Promise<SnapshotData>,
  ...
  const filtered = companies.filter((co) => {
    ...
    // "paid" is not a status value — it mirrors the backend stats definition
    // (plan_type === 'paid' AND active) so the card count matches the list.
    if (statusFilter === 'paid') return co.plan_type === 'paid' && co.status === 'active';
    return co.status === statusFilter;
  });
  ...
  const resetPassword = useMutation({
    mutationFn: ({ id, company_name }: { id: number; company_name: string }) =>
      authFetch(api(`/api/super/companies/${id}/reset-admin-password`), { method: 'POST' }).then(
        async (r) => {
          const d = await r.json();
    ...
    onSuccess: (data) => {
      setResetPassResult(data as ResetPassResult);
