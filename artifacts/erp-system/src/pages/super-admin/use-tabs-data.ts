...
interface TrialMonitoringData {
  redis_ok: boolean; status: 'normal' | 'warning' | 'paused';
  registrations_in_window: number; alert_threshold: number; block_threshold: number;
  pause_until: string | null; pause_remaining_seconds: number;
  warning_fired_at: string | null; pause_reason: string | null;
  top_ips: { ip: string; count: number }[];
  top_fingerprints: { fingerprint: string; count: number }[];
  suspicious_companies: { id: number; name: string; email: string | null; trial_score: number; is_suspicious: boolean; verification_status: string }[];
...
interface RedisHealthData { status: 'ok' | 'down'; latency_ms?: number; message?: string; }
...
export function useTabsData(
  ...
  interface UnblockRow { id: number; email: string; ip: string; fingerprint: string | null; override_reason: string | null; created_at: string; }
  interface UnblockResult { active_blocks: number; overridden_blocks: number; rows: UnblockRow[]; }
  ...
  async function saveAnnouncement() {
    ...
    try {
      const res = await authFetch(api('/api/super/announcements'), {
        ...
        body: JSON.stringify({ title: annTitle.trim(), body: annBody.trim(), type: annType, target: annTarget, company_id: annTarget !== 'all' && annCompanyId ? Number(annCompanyId) : undefined, expires_at: annExpires || undefined }),
      });
  ...
  async function toggleAnn(id: number, is_active: boolean) {
    await authFetch(api(`/api/super/announcements/${id}`), { method: 'PATCH', headers: { ...authHeaders(), 'Content-Type': 'application/json' }, body: JSON.stringify({ is_active: !is_active }) });
    void refetchAnn();
  }

  async function deleteAnn(id: number) {
    await authFetch(api(`/api/super/announcements/${id}`), { method: 'DELETE', headers: authHeaders() });
    void refetchAnn();
  }
  ...
  async function savePlan(plan: PlanSetting) {
    ...
    try {
      const res = await authFetch(api('/api/super/plan-settings/' + plan.key), {
        ...
        body: JSON.stringify({ name_ar: plan.name_ar, description: plan.description, price: Number(plan.price), includes_mobile: plan.includes_mobile, is_active: plan.is_active }),
      });
  ...
  return {
    ...
    /* alerts */
    alertsData, alertsLoading, refetchAlerts,
    alertSearch, setAlertSearch, alertTypeFilter, setAlertTypeFilter,
    /* monitoring */
    monData, monLoading, monError, refetchMon,
    /* health */
    healthData, healthLoading, refetchHealth, healthUpdated,
    redisHealth,
    /* audit */
    auditData, auditLoading, refetchAudit, auditLimit, setAuditLimit, auditAction, setAuditAction,
    /* overview audit */
    overviewAudit,
    /* announcements */
    annData, annTitle, setAnnTitle, annBody, setAnnBody, annType, setAnnType,
    annTarget, setAnnTarget, annCompanyId, setAnnCompanyId, annExpires, setAnnExpires,
    annSaving, saveAnnouncement, toggleAnn, deleteAnn,
    /* plans */
    planSettings, planSettingsLoading, editingPlan, setEditingPlan, planSaving, savePlan,
    /* unblock (monitoring) */
    unblockIP, setUnblockIP, unblockEmail, setUnblockEmail,
    unblockResult, setUnblockResult, unblockLoading, setUnblockLoading,
    unblockMsg, setUnblockMsg,
  };
}
