...
interface Props {
  ...
  /* Support */
  supportWa: string; setSupportWa: (v: string) => void;
  supportEmail: string; setSupportEmail: (v: string) => void;
  settingSaving: boolean; saveSupportSettings: () => void;
  /* Backup */
  backupData: { backups: BackupFile[]; total: number } | undefined;
  creatingBackup: boolean; downloadingFile: string | null;
  restoring: boolean; restoreOk: string | null; restoreErr: string | null;
  triggerBackup: () => void; downloadBackup: (filename: string) => void; openRestorePicker: () => void;
  restoreInputRef: React.RefObject<HTMLInputElement | null>;
  handleRestoreFileChange: (e: React.ChangeEvent<HTMLInputElement>) => void;
  /* Restore modal */
  restoreModal: boolean; setRestoreModal: (v: boolean) => void;
  pendingRestoreFile: File | null; setPendingRestoreFile: (v: File | null) => void;
  restoreCode: string; setRestoreCode: (v: string) => void; confirmRestore: () => void;
  /* Encryption key */
  encEnabled: boolean; encKey: string | null; encKeyLoading: boolean;
  encKeyVisible: boolean; encKeyCopied: boolean;
  loadEncKey: () => void; copyEncKey: () => void; downloadEncKey: () => void; emailEncKey: () => void;
  /* Security */
  totpStatus: { totp_enabled: boolean } | undefined;
  totpSetupData: { qr_code: string; secret: string } | null;
  totpInput: string; setTotpInput: (v: string) => void;
  disableTotpInput: string; setDisableTotpInput: (v: string) => void;
  secLoading: boolean; secMsg: { text: string; ok: boolean } | null;
  setSecMsg: (v: { text: string; ok: boolean } | null) => void;
  showDisable: boolean; setShowDisable: (v: boolean) => void;
  startTotpSetup: () => void; confirmTotpSetup: () => void; confirmDisableTotp: () => void;
  /* Audit log */
  auditData: AuditLogResp | undefined; auditLoading: boolean; refetchAudit: () => void;
  auditAction: string; setAuditAction: (v: string) => void;
  auditLimit: number; setAuditLimit: (v: number) => void;
  /* Telegram */
  tgConfig: TgConfig | null; setTgConfig: (fn: (c: TgConfig | null) => TgConfig | null) => void;
  tgSaving: boolean; tgBotStatus: TgBotStatus | null;
  tgBotLoading: boolean; tgBotRefetch: () => void;
  tgLoading: boolean; tgError: boolean; tgErrorObj: Error | null; tgRefetch: () => void;
  tgBotToken: string; setTgBotToken: (v: string) => void;
  tgChatId: string; setTgChatId: (v: string) => void;
  tgShowToken: boolean; setTgShowToken: (fn: (v: boolean) => boolean) => void;
  tgCredSaving: boolean; saveTgCredentials: () => void;
  tgTesting: boolean; testTelegramConnection: () => void;
...
export function TabSettings({
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
      ...
      <input ref={restoreInputRef} type="file" accept=".json,.json.enc" style={{ display: 'none' }}
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(5, 1fr)', gap: '16px' }}>
        {([
          ...
          return (
            <div key={card.key} onClick={() => setSettingsActiveCard(isActive ? null : card.key)}
              style={{ background: isActive ? `${card.color}1A` : C.card, border: `1.5px solid ${isActive ? card.color : C.border}`, borderRadius: '16px', padding: '22px 20px', cursor: 'pointer', transition: 'all 0.2s', display: 'flex', flexDirection: 'column', gap: '10px', boxShadow: isActive ? `0 0 0 3px ${card.color}30, 0 8px 32px ${card.color}25` : 'none', transform: isActive ? 'translateY(-2px)' : '' }}
      ...
      {/* Content panel */}
      {settingsActiveCard && (
        <div style={{ background: C.card, borderRadius: '18px', border: `1.5px solid ${hc}50`, overflow: 'hidden', animation: 'sa-panel-in 0.25s ease-out both' }}>
          {/* Panel header */}
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '14px 24px', borderBottom: `1px solid ${C.border}`, background: `${hc}12` }}>
            ...
            <button onClick={() => setSettingsActiveCard(null)}
              style={{ background: 'transparent', border: `1px solid ${hc}50`, borderRadius: '8px', color: hc, fontSize: '16px', cursor: 'pointer', lineHeight: 1, padding: '4px 10px', fontWeight: 700 }}>✕</button>
          </div>

          {settingsActiveCard === 'support' && (
            <SupportPanel supportWa={supportWa} setSupportWa={setSupportWa} supportEmail={supportEmail} setSupportEmail={setSupportEmail} settingSaving={settingSaving} saveSupportSettings={saveSupportSettings} />
          )}
          {settingsActiveCard === 'backup' && (
            <BackupPanel backupData={backupData} creatingBackup={creatingBackup} downloadingFile={downloadingFile} restoring={restoring} restoreOk={restoreOk} restoreErr={restoreErr} triggerBackup={triggerBackup} downloadBackup={downloadBackup} openRestorePicker={openRestorePicker} encEnabled={encEnabled} encKey={encKey} encKeyLoading={encKeyLoading} encKeyVisible={encKeyVisible} encKeyCopied={encKeyCopied} loadEncKey={loadEncKey} copyEncKey={copyEncKey} downloadEncKey={downloadEncKey} emailEncKey={emailEncKey} />
          )}
          {settingsActiveCard === 'security' && (
            <SecurityPanel totpStatus={totpStatus} totpSetupData={totpSetupData} totpInput={totpInput} setTotpInput={setTotpInput} disableTotpInput={disableTotpInput} setDisableTotpInput={setDisableTotpInput} secLoading={secLoading} secMsg={secMsg} setSecMsg={setSecMsg} showDisable={showDisable} setShowDisable={setShowDisable} startTotpSetup={startTotpSetup} confirmTotpSetup={confirmTotpSetup} confirmDisableTotp={confirmDisableTotp} />
          )}
          {settingsActiveCard === 'audit_log' && (
            <AuditLogPanel auditData={auditData} auditLoading={auditLoading} refetchAudit={refetchAudit} auditAction={auditAction} setAuditAction={setAuditAction} auditLimit={auditLimit} setAuditLimit={setAuditLimit} />
          )}
          {settingsActiveCard === 'telegram' && (
            <TelegramPanel tgConfig={tgConfig} setTgConfig={setTgConfig} tgSaving={tgSaving} tgBotStatus={tgBotStatus} tgBotLoading={tgBotLoading} tgBotRefetch={tgBotRefetch} tgLoading={tgLoading} tgError={tgError} tgErrorObj={tgErrorObj} tgRefetch={tgRefetch} tgBotToken={tgBotToken} setTgBotToken={setTgBotToken} tgChatId={tgChatId} setTgChatId={setTgChatId} tgShowToken={tgShowToken} setTgShowToken={setTgShowToken} tgCredSaving={tgCredSaving} saveTgCredentials={saveTgCredentials} tgTesting={tgTesting} testTelegramConnection={testTelegramConnection} tgTestResult={tgTestResult} saveTelegramSettings={saveTelegramSettings} />
          )}
        </div>
      )}
    </div>
