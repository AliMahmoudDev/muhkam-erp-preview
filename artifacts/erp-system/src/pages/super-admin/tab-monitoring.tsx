...
interface Props {
  ...
  monError: boolean;
  onRefetch: () => void;
  unblockIP: string;      setUnblockIP: (v: string) => void;
  unblockEmail: string;   setUnblockEmail: (v: string) => void;
  unblockResult: UnblockResult | null;  setUnblockResult: (v: UnblockResult | null) => void;
  unblockLoading: boolean; setUnblockLoading: (v: boolean) => void;
...
export function TabMonitoring({
  monData, monLoading, monError, onRefetch,
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
      ...
      {monData && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
          ...
          {/* §1 STATUS HERO */}
          <div id="mon-status" className="mon-sec" style={{ background: C.card, border: `1px solid ${st.border}`, borderTop: `3px solid ${st.color}`, borderRadius: '16px', padding: '24px', position: 'relative', overflow: 'hidden' }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: '20px', position: 'relative' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '18px' }}>
                <div style={{ width: '64px', height: '64px', borderRadius: '16px', background: st.bg, border: `1px solid ${st.border}`, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '30px', animation: 'mon-glow-beat 3s ease infinite', '--glow': st.glow } as React.CSSProperties}>
                  {st.icon}
                </div>
                <div>
              ...
              <div style={{ padding: '20px 24px' }}>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr auto', gap: '12px', alignItems: 'flex-end', marginBottom: '14px' }}>
                  <div>
                    ...
                    <input value={unblockIP} onChange={e => setUnblockIP(e.target.value)} onKeyDown={e => e.key === 'Enter' && void doLookup()} placeholder="41.234.56.78" dir="ltr" className="mon-input" style={inputSt} />
                  </div>
                  <div>
            ...
            <SectionCard id="mon-ips-inner2" accentColor="#6366f1">
              ...
              <div style={{ padding: '20px 24px' }}>
                {(monData.top_ips ?? []).length === 0 ? (
                  <div style={{ textAlign: 'center', padding: '32px 0', color: C.muted, fontSize: '13px' }}>{monData.redis_ok ? '✅ لا توجد بيانات في النافذة الحالية' : '⚠️ Redis غير متاح'}</div>
                ...
                {(monData.recent_blocks ?? []).length === 0 ? (
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                    {(monData.recent_blocks ?? []).map((b, i) => (
                      <div key={i} className="mon-row" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr auto auto', gap: '12px', alignItems: 'center', padding: '13px 18px', borderRadius: '14px', background: 'rgba(255,255,255,0.03)', border: `1px solid ${C.border}`, transition: 'background 0.15s' }}>
                        <div>
