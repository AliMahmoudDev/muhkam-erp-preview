...
const fmt = (n: number, currency = 'EGP') =>
  n.toLocaleString('ar-EG-u-nu-latn', { minimumFractionDigits: 0, maximumFractionDigits: 0 }) +
  ' ' +
  currency;
...
export default function Branches() {
  ...
  return (
    <div className="flex h-full overflow-hidden" dir="rtl">
      {/* ══ Sidebar: Branch List ══════════════════════════════ */}
      <div
        className={`flex flex-col border-l border-[var(--erp-border)] transition-all duration-200 ${selectedId ? 'w-72 shrink-0' : 'flex-1'}`}
      >
        {/* Header */}
        <div className="shrink-0 px-4 pt-5 pb-4 border-b border-[var(--erp-border)]">
          ...
          <div className="grid grid-cols-4 gap-1.5">
            {[
              ...
              <div
                key={s.label}
                className="rounded-lg p-2 text-center border border-[var(--erp-border)]"
        ...
        {(unlinkedW > 0 || unlinkedS > 0) && (
          <div className="mx-3 mt-3 flex items-start gap-2 rounded-xl p-2.5 bg-amber-500/8 border border-amber-500/20">
            ...
            <p className="text-[10px] text-amber-300/80 leading-relaxed">
              {unlinkedW > 0 && `${unlinkedW} مخزن`}
              {unlinkedW > 0 && unlinkedS > 0 && ' و'}
              {unlinkedS > 0 && ` ${unlinkedS} خزينة`} غير مربوطة بفرع
            </p>
          </div>
        )}
        ...
        {showForm && isAdmin && (
          <div
            className="mx-3 mt-3 rounded-xl p-3.5 border border-amber-500/25"
          ...
          >
            <div className="flex items-center justify-between mb-3">
              ...
              <button onClick={resetForm} className="text-[10px] text-ink/40 hover:text-ink/60">
                إلغاء
              </button>
            </div>
            <form onSubmit={handleSubmit} className="space-y-2">
              ...
              {editId !== null && (
                <div className="flex items-center gap-2">
                  ...
                  >
                    <span
                      className={`inline-block h-3.5 w-3.5 transform rounded-full bg-white transition-transform ${form.is_active ? '-translate-x-5' : '-translate-x-1'}`}
                    />
                  </button>
              ...
              >
                <div className="flex items-start justify-between gap-2">
                  <div className="flex items-center gap-2.5 min-w-0">
                    <div
                      className={`w-8 h-8 rounded-lg flex items-center justify-center shrink-0 ${b.is_active ? 'bg-amber-500/15' : 'bg-surface'}`}
                    >
                      <Building2
                        className={`w-4 h-4 ${b.is_active ? 'text-amber-400' : 'text-ink/20'}`}
                      />
                    </div>
                  ...
                  <div className="flex items-center gap-1.5 shrink-0">
                    <span
                      className={`text-[9px] font-bold px-1.5 py-0.5 rounded-full ${b.is_active ? 'text-emerald-400 bg-emerald-500/10' : 'text-red-400 bg-red-500/10'}`}
                    >
                      {b.is_active ? 'نشط' : 'موقوف'}
                    </span>
                    <ChevronRight
                      className={`w-3.5 h-3.5 text-ink/20 transition-transform ${b.id === selectedId ? 'rotate-180' : ''}`}
                    />
                  </div>
                </div>
      ...
      {selectedBranch ? (
        ...
        <div className="flex-1 flex flex-col items-center justify-center text-center p-8">
          ...
          <p className="text-ink/20 text-xs mt-1">
            لعرض التفاصيل الكاملة — الموظفون، المخازن، الخزائن، والإحصاءات
          </p>
        </div>
      )}
      ...
      <div className="shrink-0 px-5 pt-4 pb-0 border-b border-[var(--erp-border)]">
        <div className="flex items-start justify-between mb-3">
          <div className="flex items-center gap-3">
            <div
              className={`w-10 h-10 rounded-xl flex items-center justify-center ${branch.is_active ? 'bg-amber-500/15' : 'bg-surface'}`}
            >
              <Building2
                className={`w-5 h-5 ${branch.is_active ? 'text-amber-400' : 'text-ink/30'}`}
              />
            </div>
            <div>
              <div className="flex items-center gap-2">
                ...
                <span
                  className={`text-[10px] font-bold px-2 py-0.5 rounded-full ${branch.is_active ? 'text-emerald-400 bg-emerald-500/10 border border-emerald-500/20' : 'text-red-400 bg-red-500/10 border border-red-500/20'}`}
                >
                  {branch.is_active ? (
                    <>
                    ...
                    <>
                      <XCircle className="w-2.5 h-2.5 inline ml-0.5" />
                      موقوف
                    </>
                  )}
                </span>
              ...
              <div className="flex items-center gap-3 mt-0.5">
                ...
                <span className="flex items-center gap-1 text-[11px] text-ink/30">
                  <Calendar className="w-3 h-3" />
                  {new Date(branch.created_at).toLocaleDateString('ar-EG-u-nu-latn')}
                </span>
              </div>
            </div>
          </div>
        ...
        <div className="flex gap-1">
          {TABS.map((t) => (
            ...
            >
              ...
              {t.count !== undefined && (
                <span
                  className={`text-[9px] px-1.5 py-0.5 rounded-full font-black ${tab === t.key ? 'bg-amber-500/20 text-amber-300' : 'bg-surface text-ink/30'}`}
                >
                  {t.count}
                </span>
              )}
...
function OverviewTab({ data }: { data: BranchOverview }) {
  ...
  const kpis = [
    {
      ...
      value: activeEmp,
      sub: `من ${data.employees.length} إجمالي`,
  ...
  const stats30 = [
    {
      label: 'مبيعات (30 يوم)',
      value: fmt(s.sales_total),
      sub: `${s.sales_count} فاتورة`,
      color: 'var(--status-success)',
      icon: ShoppingCart,
      trend: 'up',
    },
      ...
      <div>
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
          {kpis.map((k) => (
            <div
              key={k.label}
              className="rounded-xl p-3.5 border border-[var(--erp-border)]"
              style={{ background: k.color + '0D' }}
            >
              <div className="flex items-center justify-between mb-2">
                <k.icon className="w-4 h-4" style={{ color: k.color }} />
              </div>
              <p className="text-xl font-black text-ink">{k.value}</p>
              <p className="text-xs font-bold mt-0.5" style={{ color: k.color }}>
                {k.label}
              </p>
      ...
      <div>
        ...
        <div className="grid grid-cols-2 gap-3">
          {stats30.map((s) => (
            <div
              key={s.label}
              className="rounded-xl p-3.5 border border-[var(--erp-border)] bg-[var(--erp-bg-card)]"
            >
...
function EmployeesTab({ employees }: { employees: EmployeeRow[] }) {
  ...
  return (
    <div className="space-y-2">
      {employees.map((e) => {
        const st = STATUS_LABEL[e.employment_status] ?? {
          label: e.employment_status,
          color: 'var(--text-2)',
        };
        return (
          <div
            key={e.id}
            className="rounded-xl border border-[var(--erp-border)] bg-[var(--erp-bg-card)] px-4 py-3"
          >
            <div className="flex items-start justify-between gap-3">
              ...
              <div className="flex flex-col items-end gap-1.5 shrink-0">
                ...
                <span className="text-xs font-black text-emerald-400 tabular-nums">
                  {fmt(Number(e.salary), e.currency)}
                  {e.salary_type === 'daily'
                    ? '/يوم'
