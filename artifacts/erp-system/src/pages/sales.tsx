          ...
          <div className="glass-panel rounded-3xl w-full max-w-xl border border-white/10 shadow-2xl flex flex-col max-h-[90vh]">
            ...
            <div className="overflow-y-auto flex-1 p-5 space-y-4">
              ...
              {saleDetail && returnItems.length === 0 && (
                <div className="py-8 text-center space-y-2">
                  ...
                  <p className="text-white/40 text-sm">
                    جميع أصناف هذه الفاتورة تم إرجاعها بالكامل
                  </p>
                </div>
              )}
              ...
              {returnItems.length > 0 && saleDetail && (
                <>
                  <div>
                    <label className="text-white/50 text-xs font-semibold block mb-2">
                      نوع الاسترداد
                    </label>
                  ...
                  {refundType === 'cash' && (
                    <div>
                      <label className="text-white/50 text-xs font-semibold block mb-1">
                        الخزينة الصارفة *
                      </label>
                      <select
                    ...
                    <div className="bg-white/3 border border-white/8 rounded-xl px-3 py-2 text-xs space-y-1">
                      <div className="flex justify-between text-white/40">
                        ...
                        <span className="tabular-nums">
                          {formatCurrency(invoiceAlreadyReturned)}
                        </span>
                      </div>
                  ...
                  {activeReturnItems.length > 0 && (
                    <div
                      className={`rounded-2xl px-4 py-3 flex justify-between items-center border transition-all ${
                        isOverInvoiceLimit
                    ...
                    >
                      <div>
                        <span
                          className={`text-sm font-bold ${isOverInvoiceLimit ? 'text-red-400' : 'text-white/60'}`}
                        >
            ...
            {returnItems.length > 0 && saleDetail && (
              <div className="p-5 border-t border-white/10 shrink-0 flex gap-3">
                <button
                  ...
                  disabled={
                    createMutation.isPending || activeReturnItems.length === 0 || isOverInvoiceLimit
                  }
