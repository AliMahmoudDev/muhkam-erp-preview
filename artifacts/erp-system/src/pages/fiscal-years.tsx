...
export default function FiscalYears() {
  ...
  const createMut = useMutation({
    mutationFn: (body: object) =>
      authFetch(api("/api/fiscal-years"), { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(body) })
        .then(async r => { const d = await r.json(); if (!r.ok) throw new Error(d.error ?? "خطأ"); return d; }),
    onSuccess: () => { void qc.invalidateQueries({ queryKey: ["fiscal-years"] }); setShowAdd(false); setForm({ year_label: "", start_date: "", end_date: "", notes: "" }); toast("تم إنشاء السنة المالية"); },
    onError: (e: Error) => toast(e.message, "error"),
  });

  const closeMut = useMutation({
    mutationFn: (id: number) =>
      authFetch(api(`/api/fiscal-years/${id}/close`), { method: "PATCH" })
        .then(async r => { const d = await r.json(); if (!r.ok) throw new Error(d.error ?? "خطأ"); return d; }),
    onSuccess: () => { void qc.invalidateQueries({ queryKey: ["fiscal-years"] }); toast("تم إقفال السنة المالية"); },
    onError: (e: Error) => toast(e.message, "error"),
  });
  ...
  const setCurrent = useMutation({
    mutationFn: (id: number) =>
      authFetch(api(`/api/fiscal-years/${id}/set-current`), { method: "PATCH" })
        .then(async r => { const d = await r.json(); if (!r.ok) throw new Error(d.error ?? "خطأ"); return d; }),
    onSuccess: () => { void qc.invalidateQueries({ queryKey: ["fiscal-years"] }); toast("تم تعيين السنة الحالية"); },
    onError: (e: Error) => toast(e.message, "error"),
  });

  const deleteMut = useMutation({
    mutationFn: (id: number) =>
      authFetch(api(`/api/fiscal-years/${id}`), { method: "DELETE" })
        .then(async r => { const d = await r.json(); if (!r.ok) throw new Error(d.error ?? "خطأ"); return d; }),
    ...
    <div className="space-y-6 p-4" dir="rtl" style={{ fontFamily: "'Tajawal','Cairo',sans-serif" }}>
      ...
      {isLoading ? (
        ...
        <div className="space-y-3">
          {years.map(fy => (
            <div key={fy.id} className={`rounded-2xl p-4 border transition-all ${
              fy.is_current
                ...
                {isAdmin && (
                  <div className="flex items-center gap-2 shrink-0">
                    {fy.is_open && !fy.is_current && (
                      <button onClick={() => setCurrent.mutate(fy.id)} disabled={setCurrent.isPending}
