  ...
  return (
    ...
    >
      <div style={{ display: 'flex', gap: 8, marginBottom: 10 }}>
        {(
          [
            ['إيرادات', '١٢٤٫٨ك', '#10B981'],
            ['مصروفات', '٤٨٫٢ك', '#EF4444'],
            ['أرباح', '٧٦٫٦ك', '#3B82F6'],
          ...
          <div style={{ height: 4, background: '#1E293B', borderRadius: 2 }}>
            <div
              style={{
                height: '100%',
                width: `${pct}%`,
                background: pct < 30 ? '#EF4444' : '#10B981',
                borderRadius: 2,
              }}
            />
          </div>
...
export function MiniHR() {
  return (
    ...
    >
      {(
        [
          ['أحمد محمد', 'مدير مبيعات', [1, 1, 1, 1, 0]],
          ['سارة خالد', 'محاسبة', [1, 1, 1, 1, 1]],
          ['محمود علي', 'فني', [1, 1, 0, 1, 1]],
          ['نور حسن', 'كاشير', [1, 1, 1, 0, 1]],
        ] as [string, string, number[]][]
      ).map(([name, role, days]) => (
        <div key={name} style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 7 }}>
          ...
          <div style={{ flex: 1, minWidth: 0 }}>
            <div style={{ fontSize: 11, fontWeight: 600, color: '#E2E8F0', lineHeight: 1.2 }}>
              {name}
            </div>
    ...
    >
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between' }}>
        {['س', 'أح', 'إث', 'ث', 'خ', 'ج', 'سب'].map((d) => (
          <span key={d} style={{ fontSize: 8, color: '#475569', flex: 1, textAlign: 'center' }}>
            {d}
          </span>
        ))}
      </div>
...
export function MiniBank() {
  return (
    ...
    >
      {(
        [
          ['دفع مورد', '-٨٫٥٠٠', '#EF4444'],
          ['تحصيل عميل', '+٢٢٫٠٠٠', '#10B981'],
          ['مصاريف إدارية', '-١٫٢٠٠', '#EF4444'],
        ] as [string, string, string][]
      ).map(([desc, amount, color]) => (
        <div
          key={desc}
          style={{
            display: 'flex',
...
export function MiniMobile() {
  const phone = (content: React.ReactNode) => (
    ...
    >
      ...
      >
        <div
          style={{ width: 22, height: 2, background: 'rgba(59, 130, 246, 0.5)', borderRadius: 1 }}
        />
      </div>
