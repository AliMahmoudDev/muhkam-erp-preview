  ...
  if (isError || !stats) {
    return (
      <div className="db-error-state" dir="rtl">
        <AlertTriangle
          style={{ width: 44, height: 44, color: 'var(--status-warning)', margin: '0 auto 16px', opacity: 0.7 }}
        />
      ...
      <div className="erp-hero-strip erp-hero-strip--4col">
        ...
        <div className="erp-hero-cell">
          ...
          <div>
            ...
            <p className="hero-value" style={{ color: netIsPositive ? 'var(--status-success)' : 'var(--status-danger)' }}>
          ...
          >
            <Package
              style={{
                width: 16,
                height: 16,
                color: (stats.low_stock_products?.length ?? 0) === 0 ? 'var(--status-success)' : 'var(--status-warning)',
              }}
            />
          </div>
          <div>
                ...
                .map((tx: { id: number; type: string; amount: number; created_at: string }) => {
                  ...
                  return (
                    <div key={tx.id} className="db-tx-row">
                      ...
                      >
                        <TxIcon
                          style={{ width: 16, height: 16, color: isIncome ? 'var(--status-success)' : 'var(--status-danger)' }}
                        />
                      </div>
                      ...
                      <div className="flex items-center gap-1">
                        {isIncome ? (
                          <ArrowUpRight style={{ width: 14, height: 14, color: 'var(--status-success)' }} />
                        ) : (
                          <ArrowDownRight style={{ width: 14, height: 14, color: 'var(--status-danger)' }} />
                        )}
                        <span
                          className="db-tx-amount"
                          style={{ color: isIncome ? 'var(--status-success)' : 'var(--status-danger)' }}
                        >
              ...
              {stats.low_stock_products.slice(0, 7).map((prod) => {
                ...
                return (
                  <div key={prod.id} className="db-tx-row">
                    ...
                    >
                      <Package
                        style={{ width: 16, height: 16, color: outOfStock ? 'var(--status-danger)' : 'var(--status-warning)' }}
                      />
                    </div>
                    <div className="flex-1 min-w-0">
