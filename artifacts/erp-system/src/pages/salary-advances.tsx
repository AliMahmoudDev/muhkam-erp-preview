...
function statusBadge(s: string) {
  switch (s) {
    case "approved": case "active": return "erp-badge erp-badge-success";
    case "pending":                 return "erp-badge erp-badge-warning";
    case "rejected":                return "erp-badge erp-badge-danger";
    case "completed":               return "erp-badge erp-badge-info";
...
export default function SalaryAdvances() {
  ...
  const TABS = [
    ...
    ...(canManage ? [{ key: "pending", label: `طلبات الاعتماد${pendingList.length > 0 ? ` (${pendingList.length})` : ""}` }] : []),
    ...
    <div className="p-4 space-y-4" dir="rtl">
      ...
      <div className="flex items-center justify-between flex-wrap gap-3">
        ...
        <div className="flex gap-2">
          {canManage && (
            <button onClick={() => { setSettingsForm({ max_advance_percentage: String(cfg?.max_advance_percentage ?? 50), max_concurrent_advances: String(cfg?.max_concurrent_advances ?? 2), min_salary_for_advance: String(cfg?.min_salary_for_advance ?? 3000), repayment_tenure_months: String(cfg?.repayment_tenure_months ?? 1) }); setShowSettingsForm(true); }} className="erp-btn erp-btn-ghost flex items-center gap-1 text-xs">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        {[
          ...
          { label: "معلّقة",                val: advancesList.filter(a => a.status === "pending").length,               color: "text-amber-300" },
          { label: "نشطة",                  val: advancesList.filter(a => a.status === "active").length,                color: "text-emerald-300" },
      ...
      <div className="flex gap-1 border-b border-white/10 mb-2 flex-wrap">
        {TABS.map(t => (
          <button key={t.key} onClick={() => setActiveTab(t.key)}
            ...
            {t.label}
            {t.key === "pending" && pendingList.length > 0 && <span className="mr-1 bg-red-500 text-white rounded-full w-4 h-4 text-xs inline-flex items-center justify-center">{pendingList.length}</span>}
          </button>
        ))}
      </div>
      ...
      {activeTab === "list" && (
        <div className="space-y-4">
          ...
          <div className="erp-card overflow-x-auto">
            {advances.isLoading ? <TableSkeleton /> : advancesList.length === 0 ? (
              <div className="erp-empty-state"><Banknote size={36} className="erp-empty-icon mb-2" /><p className="erp-empty-label">لا توجد سلف</p></div>
            ) : (
                  ...
                  {advancesList.map(a => (
                    <tr key={String(a.id)} className="erp-table-row">
                      <td className="p-3 text-sm">
                        <div>{String(a.first_name_ar ?? "")} {String(a.last_name_ar ?? "")}</div>
                        <div className="text-xs text-white/40 font-mono">{String(a.employee_code ?? "")}</div>
                      </td>
                      <td className="p-3 text-sm text-white/60">{typeAr(String(a.advance_type))}</td>
                      <td className="p-3 text-sm text-amber-300 font-bold font-mono">{fmt(a.requested_amount)} {String(a.currency)}</td>
                      <td className="p-3 text-sm font-mono">{Number(a.remaining_balance) > 0 ? <span className="text-red-400">{fmt(a.remaining_balance)}</span> : <span className="text-emerald-300">مسدّد</span>}</td>
                      <td className="p-3"><span className={statusBadge(String(a.status))}>{statusAr(String(a.status))}</span></td>
                      <td className="p-3 text-xs text-white/40 font-mono">{String(a.requested_date)}</td>
                      {canManage && (
                        <td className="p-3">
                          <div className="flex gap-1 flex-wrap">
                            {a.status === "active" && <button onClick={() => setShowPaymentModal(a.id as number)} className="erp-btn erp-btn-ghost p-1 text-xs text-emerald-400 border border-emerald-500/30">دفعة</button>}
                            {(a.status === "active" || a.status === "approved") && <button onClick={() => cancelAdv.mutate(a.id as number)} className="erp-btn erp-btn-ghost p-1 text-xs text-red-400 border border-red-500/30">إلغاء</button>}
                          </div>
                        </td>
                      )}
                    </tr>
      ...
      {activeTab === "pending" && canManage && (
        <div className="space-y-3">
          {pending.isLoading ? <TableSkeleton /> : pendingList.length === 0 ? (
            <div className="erp-card"><div className="erp-empty-state"><Clock size={36} className="erp-empty-icon mb-2" /><p className="erp-empty-label">لا توجد طلبات معلّقة</p></div></div>
          ) : (
            pendingList.map(a => (
              <div key={String(a.id)} className="erp-card border-amber-500/20">
                <div className="flex items-start justify-between gap-4">
                  <div>
                    <div className="font-bold text-sm">{String(a.first_name_ar ?? "")} {String(a.last_name_ar ?? "")} <span className="text-xs text-white/40 font-mono">({String(a.employee_code ?? "")})</span></div>
                    <div className="text-xs text-white/50 mt-1">الراتب الأساسي: <span className="font-mono">{fmt(a.salary)} ج.م</span></div>
                    <div className="text-amber-300 font-bold mt-2 text-lg font-mono">{fmt(a.requested_amount)} {String(a.currency)}</div>
                    <div className="text-xs text-white/40 mt-1">النوع: {typeAr(String(a.advance_type))} • التاريخ: <span className="font-mono">{String(a.requested_date)}</span></div>
                    {a.reason && <div className="text-sm text-white/60 mt-2 bg-white/5 rounded p-2">{String(a.reason)}</div>}
                  </div>
                  <div className="flex gap-2 shrink-0">
                    <button onClick={() => approveAdv.mutate(a.id as number)} disabled={approveAdv.isPending} className="erp-btn erp-btn-primary text-xs">قبول</button>
                    <button onClick={() => setRejectId(a.id as number)} className="erp-btn erp-btn-ghost text-xs text-red-400 border border-red-500/30">رفض</button>
                  </div>
                </div>
              </div>
            ))
      ...
      {activeTab === "ledger" && (
        <div className="space-y-4">
          <div className="flex gap-3 items-center">
            <input type="number" className="erp-input w-48" placeholder="رقم الموظف" value={ledgerEmpId} onChange={e => setLedgerEmpId(e.target.value)} />
            <span className="text-white/30 text-sm">أدخل رقم الموظف لعرض كشف حسابه</span>
          </div>
          {ledgerEmpId && ledger.isLoading ? <TableSkeleton /> : ledgerList.length > 0 ? (
            <div className="erp-card overflow-x-auto">
              <table className="erp-table w-full">
                <thead><tr className="erp-table-header">
                  <th className="p-3 text-right text-xs">التاريخ</th>
                ...
                <tbody>
                  {ledgerList.map(l => {
                    const ltAr: Record<string, string> = { advance_granted: "سلفة ممنوحة", deduction: "خصم من المرتب", manual_payment: "دفعة يدوية", reversal: "استرداد" };
                    const isDebit = String(l.ledger_type) === "advance_granted";
                    return (
                      <tr key={String(l.id)} className="erp-table-row">
                        <td className="p-3 text-sm text-white/60 font-mono">{String(l.ledger_date)}</td>
                        <td className="p-3 text-sm">{ltAr[String(l.ledger_type)] ?? String(l.ledger_type)}</td>
                        <td className={`p-3 text-sm font-bold font-mono ${isDebit ? "text-amber-300" : "text-emerald-300"}`}>{isDebit ? "+" : "-"}{fmt(l.amount)}</td>
            ...
            <div className="erp-card"><div className="erp-empty-state"><BookOpen size={36} className="erp-empty-icon mb-2" /><p className="erp-empty-label">لا توجد حركات لهذا الموظف</p></div></div>
          ) : null}
        </div>
      )}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
          <div className="bg-[#181c2e] border border-white/10 rounded-2xl shadow-2xl w-full max-w-md" dir="rtl">
            ...
            <div className="p-5 space-y-3">
              {cfg && <div className="bg-amber-500/10 border border-amber-500/20 rounded-lg p-3 text-xs text-amber-300">الحد الأقصى: {Number(cfg.max_advance_percentage)}% من الراتب • الحد الأدنى للراتب: <span className="font-mono">{fmt(cfg.min_salary_for_advance)}</span> ج.م</div>}
              ...
              <Field label="المبلغ المطلوب *"><input type="number" value={requestForm.requested_amount} onChange={e => setRequestForm(p => ({ ...p, requested_amount: e.target.value }))} className="erp-input w-full" required /></Field>
              <Field label="النوع">
                <select value={requestForm.advance_type} onChange={e => setRequestForm(p => ({ ...p, advance_type: e.target.value }))} className="erp-input w-full">
                  {[["personal", "شخصي"], ["emergency", "طارئ"], ["medical", "علاجي"], ["educational", "تعليمي"], ["other", "أخرى"]].map(([v, l]) => <option key={v} value={v}>{l}</option>)}
                </select>
              </Field>
              <Field label="السبب *"><input value={requestForm.reason} onChange={e => setRequestForm(p => ({ ...p, reason: e.target.value }))} className="erp-input w-full" required /></Field>
            </div>
            <div className="flex gap-2 p-5 border-t border-white/10">
              <button onClick={() => { createAdvance.mutate(requestForm, { onSuccess: () => { setShowRequestForm(false); setRequestForm({ employee_id: "", requested_amount: "", advance_type: "personal", reason: "" }); } }); }} disabled={createAdvance.isPending} className="erp-btn erp-btn-primary flex-1">
      ...
      {showSettingsForm && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
          <div className="bg-[#181c2e] border border-white/10 rounded-2xl shadow-2xl w-full max-w-md" dir="rtl">
            ...
            <div className="p-5 space-y-3">
              <Field label="الحد الأقصى من الراتب (%)"><input type="number" value={settingsForm.max_advance_percentage} onChange={e => setSettingsForm(p => ({ ...p, max_advance_percentage: e.target.value }))} className="erp-input w-full" /></Field>
              <Field label="الحد الأقصى للسلف المتزامنة"><input type="number" value={settingsForm.max_concurrent_advances} onChange={e => setSettingsForm(p => ({ ...p, max_concurrent_advances: e.target.value }))} className="erp-input w-full" /></Field>
              <Field label="الحد الأدنى للراتب للأهلية (ج.م)"><input type="number" value={settingsForm.min_salary_for_advance} onChange={e => setSettingsForm(p => ({ ...p, min_salary_for_advance: e.target.value }))} className="erp-input w-full" /></Field>
              <Field label="مدة السداد (شهور)"><input type="number" value={settingsForm.repayment_tenure_months} onChange={e => setSettingsForm(p => ({ ...p, repayment_tenure_months: e.target.value }))} className="erp-input w-full" /></Field>
            </div>
            <div className="flex gap-2 p-5 border-t border-white/10">
              <button onClick={() => { updateSettings.mutate({ max_advance_percentage: Number(settingsForm.max_advance_percentage), max_concurrent_advances: Number(settingsForm.max_concurrent_advances), min_salary_for_advance: Number(settingsForm.min_salary_for_advance), repayment_tenure_months: Number(settingsForm.repayment_tenure_months) }, { onSuccess: () => setShowSettingsForm(false) }); }} disabled={updateSettings.isPending} className="erp-btn erp-btn-primary flex-1">
                {updateSettings.isPending ? "جاري الحفظ..." : "حفظ"}
              </button>
              <button onClick={() => setShowSettingsForm(false)} className="erp-btn erp-btn-ghost">إلغاء</button>
            </div>
          </div>
        </div>
      )}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
          <div className="bg-[#181c2e] border border-white/10 rounded-2xl shadow-2xl w-full max-w-sm" dir="rtl">
            ...
            <div className="p-4 space-y-3">
              <Field label="سبب الرفض *"><input value={rejectReason} onChange={e => setRejectReason(e.target.value)} className="erp-input w-full" placeholder="اكتب سبب الرفض..." /></Field>
            </div>
            <div className="flex gap-2 p-4 border-t border-white/10">
              <button onClick={() => { rejectAdv.mutate({ id: rejectId, reason: rejectReason }, { onSuccess: () => { setRejectId(null); setRejectReason(""); } }); }} disabled={!rejectReason.trim() || rejectAdv.isPending} className="erp-btn flex-1 bg-red-500/20 text-red-300 border border-red-500/30 hover:bg-red-500/30">
      ...
      {showPaymentModal != null && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
          <div className="bg-[#181c2e] border border-white/10 rounded-2xl shadow-2xl w-full max-w-sm" dir="rtl">
            ...
            <div className="p-4 space-y-3">
              <Field label="المبلغ *"><input type="number" value={paymentAmount} onChange={e => setPaymentAmount(e.target.value)} className="erp-input w-full" placeholder="0.00" /></Field>
            </div>
            <div className="flex gap-2 p-4 border-t border-white/10">
              <button onClick={() => { manualPay.mutate({ id: showPaymentModal, amount: Number(paymentAmount) }, { onSuccess: () => { setShowPaymentModal(null); setPaymentAmount(""); } }); }} disabled={!paymentAmount || Number(paymentAmount) <= 0 || manualPay.isPending} className="erp-btn erp-btn-primary flex-1">
                {manualPay.isPending ? "جاري التسجيل..." : "تسجيل الدفعة"}
              </button>
              <button onClick={() => { setShowPaymentModal(null); setPaymentAmount(""); }} className="erp-btn erp-btn-ghost flex-1">إلغاء</button>
            </div>
          </div>
        </div>
      )}
