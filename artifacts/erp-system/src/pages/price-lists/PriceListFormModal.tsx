...
export function PriceListFormModal({
  ...
  const [items, setItems] = useState<{ product_id: number; markup_percent: string }[]>(
    initial?.items.map((i) => ({
      ...
      markup_percent: i.markup_percent != null ? String(i.markup_percent) : '',
    })) ?? []
  );
  ...
  const updateMarkup = (productId: number, value: string) => {
    setItems((prev) =>
      prev.map((i) => (i.product_id === productId ? { ...i, markup_percent: value } : i))
    );
  };
      ...
      >
        ...
        >
          ...
          <div className="flex items-center justify-between px-5 py-4 border-b border-line">
            <div className="flex items-center gap-2.5">
              ...
              <p className="font-bold text-ink">
                {initial ? 'تعديل قائمة الأسعار' : 'إنشاء قائمة أسعار جديدة'}
              </p>
            </div>
            <button onClick={onClose} className="p-1 rounded-lg text-ink/40 hover:text-ink">
              <X className="w-4 h-4" />
            </button>
          </div>
          ...
          <div className="flex-1 overflow-y-auto p-5 space-y-4">
            {/* Name */}
            <div>
              <label className="block text-xs text-ink/50 font-semibold mb-1.5 uppercase tracking-wide">
                اسم القائمة *
              </label>
              <input
                value={name}
            ...
            <div className="flex items-center gap-3">
              ...
              >
                <span
                  className={`absolute top-1 w-4 h-4 rounded-full bg-white shadow transition-all ${isActive ? 'right-1' : 'left-1'}`}
                />
              </button>
            ...
            <div>
              <div className="flex items-center justify-between mb-2">
                <label className="text-xs text-ink/50 font-semibold uppercase tracking-wide">
              ...
              {items.length === 0 ? (
                ...
                <div className="space-y-2 max-h-64 overflow-y-auto">
                  {items.map((item) => {
                    ...
                    const previewPrice =
                      !isNaN(markup) && cost > 0 ? cost * (1 + markup / 100) : null;

                    return (
                      <div
                        key={item.product_id}
                        className="flex items-center gap-2 bg-surface rounded-xl px-3 py-2.5 border border-line"
                      >
                          ...
                          <p className="text-xs text-ink/30">
                            ...
                            {previewPrice != null && (
                              <span className="text-amber-400/70 mr-2">
                                ← سعر البيع: {formatCurrency(previewPrice)}
                              </span>
                            )}
                          </p>
                        </div>
          ...
          <div className="px-5 py-4 border-t border-line flex justify-between items-center gap-3">
            <button
              onClick={onClose}
              className="px-4 py-2 rounded-xl border border-line text-ink/50 hover:text-ink text-sm transition"
            >
              إلغاء
            </button>
            <button
