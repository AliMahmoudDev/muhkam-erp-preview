      ...
      >
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2">
            ...
            {!list.is_active && (
              <span className="text-[10px] font-bold px-2 py-0.5 rounded-full bg-surface text-ink/30 border border-line">
                معطّل
              </span>
            )}
          </div>
          {list.description && (
            <p className="text-xs text-ink/40 truncate mt-0.5">{list.description}</p>
          )}
        </div>
      ...
      {expanded && (
        <div className="border-t border-line px-4 pb-4">
          {loadingDetail ? (
            ...
            <div className="py-4 text-center text-ink/30 text-sm">
            ...
            <div className="mt-3 space-y-1.5">
              ...
              {detail.items.map((item) => {
                ...
                const previewPrice =
                  markup != null && item.cost_price > 0
                    ? item.cost_price * (1 + markup / 100)
                    : null;
                return (
                  <div
                    key={item.id}
                    className="grid grid-cols-3 px-3 py-2 rounded-xl bg-surface text-sm items-center"
                  >
                    ...
                    <div className="text-left">
                      {markup != null ? (
                        <span className="text-amber-400 font-bold">
                          {markup}%
                          {previewPrice != null && (
                            <span className="text-ink/30 font-normal text-xs mr-1">
