...
interface BudgetDetail {
  ...
  summary: { total_budgeted: number; total_actual: number; total_variance: number };
}
...
export default function BudgetsPage() {
  ...
  const { data: budgets = [], isLoading } = useQuery<Budget[]>({
    queryKey: ['budgets'],
    queryFn: () =>
      authFetch(api('/api/budgets')).then(async (r) => {
  ...
  const addMutation = useMutation({
    mutationFn: (data: typeof form) =>
      authFetch(api('/api/budgets'), { method: 'POST', body: JSON.stringify(data) }).then(
        async (r) => {
  ...
  const deleteMutation = useMutation({
    mutationFn: (id: number) =>
      authFetch(api(`/api/budgets/${id}`), { method: 'DELETE' }).then(async (r) => {
        if (!r.ok) {
          const d = await r.json();
      ...
      <div className="space-y-6 p-6">
        <div className="flex items-center gap-3">
          <Button
            size="sm"
            variant="ghost"
            className="gap-1 text-ink/60 hover:text-ink"
            onClick={() => setSelected(null)}
          >
            <ChevronLeft className="h-4 w-4" /> عودة
          </Button>
          <h1 className="text-xl font-bold text-ink">{detail?.budget.name ?? selected.name}</h1>
          ...
          <Button
            size="sm"
            variant="ghost"
            className="mr-auto text-red-400 hover:bg-red-500/10"
            onClick={() => {
              if (confirm('حذف الميزانية؟')) deleteMutation.mutate(selected.id);
            }}
          >
            ...
            {[
              {
                ...
                value: detail.summary.total_budgeted,
                color: 'text-blue-400',
              },
              {
                label: 'إجمالي الفعلي',
                value: detail.summary.total_actual,
                color: 'text-green-400',
              },
              {
                label: 'الانحراف الإجمالي',
                value: detail.summary.total_variance,
        ...
        {comp.length === 0 ? (
          <div className="text-center text-ink/50 py-12">
            ...
            <p className="text-xs mt-1 text-ink/30">
              استخدم الـ API لإضافة أرقام الميزانية للحسابات
            </p>
          </div>
        ) : (
          <>
                      ...
                      <table className="w-full text-sm">
                        <thead>
                          <tr className="bg-surface text-ink/60 text-right">
                            {['الحساب', 'الفترة', 'الميزانية', 'الفعلي', 'الانحراف', '%'].map(
                              (h) => (
                        ...
                        <tbody className="divide-y divide-white/5">
                          {rows.map((r, i) => {
                            const good =
                              r.account_type === 'revenue' ? r.variance >= 0 : r.variance <= 0;
                            return (
                              <tr
                                key={`${r.account_code ?? ''}-${r.period ?? ''}-${i}`}
                                className="hover:bg-surface"
                              >
                                <td className="px-4 py-2.5">
                                  <p className="text-ink font-medium">{r.account_name}</p>
                                  <p className="text-xs text-ink/40 font-mono">{r.account_code}</p>
                                </td>
                                <td className="px-4 py-2.5 text-ink/60 font-mono text-xs">
                                  {r.period}
                                </td>
                                <td className="px-4 py-2.5 font-mono text-ink/80">
                                  {formatCurrency(r.budgeted_amount)}
                                </td>
                                <td className="px-4 py-2.5 font-mono text-ink">
                                  {formatCurrency(r.actual_amount)}
                                </td>
                                <td
                                  className={`px-4 py-2.5 font-mono font-medium ${good ? 'text-green-400' : 'text-red-400'}`}
                                >
                                  {r.variance >= 0 ? '+' : ''}
    ...
    <div className="erp-page">
      ...
      {isLoading ? (
        ...
        <div className="text-center text-ink/50 py-20">
          ...
          <p className="text-sm text-ink/30 mt-1">أنشئ ميزانية جديدة للبدء في متابعة الأداء</p>
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {budgets.map((budget) => (
            <button
              ...
              className="p-5 rounded-xl border border-line bg-surface hover:bg-surface hover:border-blue-500/30 text-right transition-all group"
            >
              <div className="flex items-center justify-between mb-3">
                <BarChart3 className="h-5 w-5 text-blue-400" />
                <Badge className="bg-blue-500/15 text-blue-400 text-xs">
                  سنة {budget.fiscal_year}
                </Badge>
              </div>
              <h3 className="font-semibold text-ink group-hover:text-blue-300 transition-colors">
                {budget.name}
              </h3>
              ...
              {budget.notes && (
                <p className="text-xs text-ink/40 mt-2 line-clamp-2">{budget.notes}</p>
              )}
      ...
      <Dialog open={showAdd} onOpenChange={setShowAdd}>
        <DialogContent className="bg-card border-border text-card-foreground max-w-md" dir="rtl">
          <DialogHeader>
            <DialogTitle>إنشاء ميزانية تقديرية</DialogTitle>
          </DialogHeader>
