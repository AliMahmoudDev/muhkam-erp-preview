...
export function SuccessModal({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/70 backdrop-blur-sm">
      <Card className="w-full max-w-sm text-center space-y-5 p-6">
        <div className="w-20 h-20 rounded-full flex items-center justify-center mx-auto bg-emerald-500/15 border border-emerald-500/30">
          <CheckCircle2 className="w-10 h-10 text-emerald-400" />
        </div>
        <div>
          <h3 className="text-2xl font-bold">تم إصدار الفاتورة</h3>
          <p className="text-[var(--brand)] font-bold text-xl mt-1">{invoice.invoice_no}</p>
          ...
          {invoice.customer_name && (
            <p className="opacity-50 text-xs mt-1">
              العميل: <span className="font-semibold">{invoice.customer_name}</span>
            </p>
          )}
        </div>
        ...
        <div className="space-y-2.5">
          ...
          {invoice.customer_phone && (
            <a
              ...
              className="flex items-center justify-center gap-3 w-full py-3 rounded-2xl font-bold transition-all text-green-500 bg-green-500/10 border border-green-500/30 hover:bg-green-500/20"
            >
