...
export default function PurchaseFormModal({ onDone }: { onDone: () => void }) {
  ...
  const filteredSafes =
    isScopedRole && user?.safe_id ? safes.filter((s) => s.id === user.safe_id) : safes;
  const filteredWarehouses =
    isScopedRole && user?.warehouse_id
  ...
  const selectedParty = useMemo(() => {
    ...
    if (partyKey.startsWith('c:')) {
      ...
      return c
        ? { type: 'customer' as const, id: c.id, name: c.name, balance: Number(c.balance) }
        : null;
    }
    return null;
  }, [partyKey, customers]);
  ...
  const handleCheckout = async () => {
    ...
    if (isConsignment && !partyKey) {
      toast({ title: 'يجب اختيار المورد لفاتورة الائتمان', variant: 'destructive' });
      return;
    }
    if (!isConsignment && (paymentType === 'credit' || paymentType === 'partial') && !partyKey) {
      toast({ title: 'يجب اختيار الطرف الآخر للآجل أو الجزئي', variant: 'destructive' });
      return;
    }
    if (!isConsignment && (paymentType === 'cash' || paymentType === 'partial') && !safeId) {
      toast({ title: 'يجب اختيار الخزينة للدفع النقدي', variant: 'destructive' });
      return;
    }

    const actualPaid = isConsignment
      ? 0
      : paymentType === 'cash'
        ? egpTotal
        : paymentType === 'credit'
    ...
    const convertedItems = cart.map((item) => ({
      ...
      unit_price:
        currency === 'EGP' ? item.unit_price : parseFloat((item.unit_price * rate).toFixed(4)),
      total_price:
        currency === 'EGP' ? item.total_price : parseFloat((item.total_price * rate).toFixed(4)),
    ...
    const effectiveWarehouseId =
      isConsignment && finalConsignmentWarehouseId
              ...
              {filteredProducts.map((product) => (
                ...
                >
                  ...
                  <div className="flex justify-between items-center mt-2.5">
                    <span className="text-blue-300 font-black text-sm tabular-nums">
                      {formatCurrency(product.cost_price)}
                    </span>
                    <span className="text-[10px] text-ink/45 bg-surface px-1.5 py-0.5 rounded-md font-bold tabular-nums">
                      {product.quantity}
                    </span>
                  </div>
        ...
        <div className="w-full lg:w-[380px] flex flex-col purch-cart-panel rounded-2xl overflow-hidden shrink-0">
          ...
          <div className="purch-cart-header px-3 pt-3 pb-2.5 shrink-0">
            ...
            <div className="flex items-center justify-between mb-2">
              ...
              <span
                className={`text-xs font-black px-2.5 py-0.5 rounded-full ${cart.length > 0 ? 'bg-amber-500/20 text-amber-400 border border-amber-500/30' : 'bg-surface text-ink/30 border border-line'}`}
              >
                {cart.length} صنف
              </span>
            </div>
            ...
            {currency !== 'EGP' && (
              <div className="mt-1.5 flex items-center gap-2 bg-blue-500/10 border border-blue-500/20 rounded-xl px-2.5 py-1.5">
                <span className="text-blue-300 text-[10px] font-bold shrink-0">
                  صرف: 1 {currency} =
                </span>
                <input
                  type="number"
          ...
          <div className="flex-1 overflow-y-auto p-2.5 space-y-1.5 cart-items-area">
            {cart.length === 0 ? (
              ...
              cart.map((item) => (
                <div
                  key={item.product_id}
                  className="pos-cart-item flex items-center gap-1.5 px-2.5 py-2 rounded-xl"
                >
                  <div className="flex-1 min-w-0">
                    <p className="text-ink font-bold text-xs truncate leading-tight">
                      {item.product_name}
                    </p>
                    <p className="text-ink/40 text-[10px] tabular-nums">
                      {item.unit_price.toFixed(2)} {currSym}
                    </p>
                  </div>
                  <div className="flex items-center gap-0.5 shrink-0">
                    ...
                    <span className="text-ink font-black text-xs w-5 text-center tabular-nums">
                      {item.quantity}
                    </span>
                    <button
                      onClick={() => updateQty(item.product_id, 1)}
                  ...
                  <div className="shrink-0 w-[68px]">
                    ...
                    <p className="text-blue-300 font-black text-[10px] text-right tabular-nums mt-0.5">
                      ...
                      {currency !== 'EGP' && (
                        <span className="text-ink/30 text-[9px] mr-0.5">{currSym}</span>
                      )}
                    </p>
                  </div>
                  <button
          ...
          <div className="purch-cart-footer p-3 space-y-2">
            <div className="grid grid-cols-1 gap-1.5">
              ...
              {(paymentType === 'cash' || paymentType === 'partial') &&
                selectRow(
                  'الخزينة',
                  <Vault className="w-3.5 h-3.5 text-amber-400/70" />,
                  <SearchableSelect
                    items={filteredSafes.map((s) => ({
                      value: String(s.id),
            ...
            {!isConsignment && (
              <div className="flex gap-1">
                {[
                  ...
                  <button
                    key={opt.v}
                    onClick={() => setPaymentType(opt.v as 'cash' | 'credit' | 'partial')}
            ...
            <div className="sale-total-box rounded-2xl overflow-hidden">
              <div className="px-3 py-2.5 space-y-1.5">
                ...
                <div className="flex items-center justify-between">
                  ...
                  <span className="font-black text-ink text-lg tabular-nums">
                    {formatCurrency(egpTotal)}
                  </span>
                </div>
                {paymentType === 'cash' && (
                  ...
                  <>
                    ...
                    <div className="flex justify-between text-[10px]">
                      ...
                      <span className="text-orange-400 font-bold">
                        − {formatCurrency(egpTotal - (parseFloat(paidAmount) || 0))}
                      </span>
                    </div>
                  </>
                )}
                {partyKey && customerImpact !== 0 && (
                  <div className="flex justify-between text-[10px] border-t border-line pt-1.5">
                    ...
                    <span className="text-orange-400 font-bold">
                      {formatCurrency(Math.abs(customerImpact))} (علينا)
                    </span>
                  </div>
                )}
