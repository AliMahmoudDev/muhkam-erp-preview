      ...
      {showForm && (
        <div className="glass-panel rounded-3xl p-6 border border-line space-y-5">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
              <label className="block text-ink/60 text-xs font-semibold mb-1">فاتورة الشراء (اختياري)</label>
          ...
          <div>
            ...
            {cart.length === 0 ? (
              ...
              <div className="space-y-2">
                {cart.map((item, idx) => (
                  <div key={idx} className="flex items-center gap-3 bg-surface rounded-2xl px-4 py-3 border border-line">
                    <div className="flex-1 min-w-0">
                      <p className="text-ink font-bold text-sm truncate">{item.product_name}</p>
                      <p className="text-ink/40 text-xs">{formatCurrency(item.unit_price)} / وحدة</p>
                    </div>
          ...
          <table className="w-full text-right text-ink/80 whitespace-nowrap text-sm">
            ...
            <tbody>
              {isLoading ? <TableSkeleton cols={6} rows={4} />
                : returns.length === 0
                  ? <tr><td colSpan={6} className="p-8 text-center text-ink/40">لا توجد مرتجعات بعد</td></tr>
                  : returns.map(r => (
                    <tr key={r.id} className="border-b border-line erp-table-row">
                      ...
                      <td className="p-3 text-ink/50">{r.date || r.created_at?.slice(0, 10) || '—'}</td>
                    </tr>
                  ))}
            </tbody>
          </table>
