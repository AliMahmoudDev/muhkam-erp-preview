  ...
  return (
    <div className="flex flex-col h-full overflow-hidden">
        ...
        <div className="flex items-center justify-between px-3 pt-3 pb-2 gap-2">
          ...
          <div className="flex items-center gap-1.5 shrink-0">
            <button
              ...
              className="flex items-center gap-1 px-2 py-1 rounded-lg border border-line text-ink/50 text-[10px] font-bold hover:bg-surface transition-all"
            >
      ...
      <div className="flex-1 overflow-y-auto p-3 space-y-3">
        ...
        {/* Engineer Reports */}
        <div className="glass-panel rounded-2xl border border-line bg-surface/30 overflow-hidden">
          <button
            onClick={() => setReportOpen((v) => !v)}
            className="w-full flex items-center justify-between px-4 py-3 text-right hover:bg-surface transition-all"
          >
            <p className="text-[11px] text-ink/60 font-bold flex items-center gap-1.5">
              <ClipboardList className="w-3.5 h-3.5" /> تقارير مهندس الصيانة
              {engineerReports.length > 0 && (
                <span className="text-[9px] px-1.5 py-0.5 rounded-full bg-surface text-ink/50 font-medium tabular-nums border border-line">
            ...
            <div className="flex items-center gap-2">
              {reportOpen && !addingReport && (
                <span
                  ...
                  className="flex items-center gap-1 text-[10px] px-2 py-1 rounded-lg bg-amber-500/15 border border-amber-500/25 text-amber-300 hover:bg-amber-500/25 transition-all"
                >
                  <Plus className="w-3 h-3" /> تقرير جديد
                </span>
              )}
              <ChevronRight
                className={`w-4 h-4 text-ink/30 transition-transform duration-200 ${reportOpen ? '-rotate-90' : 'rotate-90'}`}
              />
            </div>
          </button>
          {reportOpen && (
            <div className="px-4 pb-4 space-y-2.5">
                  ...
                  <div className="flex items-center justify-between gap-2">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <button
                        ...
                        className="flex items-center gap-1.5 text-[11px] font-bold px-3 py-1 rounded-lg bg-amber-500/20 border border-amber-500/30 text-amber-200 hover:bg-amber-500/30 disabled:opacity-30 transition-all"
                      >
              ...
              {engineerReports.length === 0 && !addingReport && (
                <div className="text-center py-6 space-y-2">
                  <ClipboardList className="w-6 h-6 text-ink/20 mx-auto" />
                  <p className="text-[11px] text-ink/35">لا توجد تقارير بعد</p>
                  <button
                    onClick={() => setAddingReport(true)}
                    className="text-[11px] text-ink/40 hover:text-ink transition-colors inline-flex items-center gap-1"
                  >
                ...
                return (
                  ...
                  >
                    <div className="flex items-start justify-between gap-2 mb-1.5">
                      <div className="flex items-center gap-2">
                        <div className="w-6 h-6 rounded-lg bg-surface border border-line flex items-center justify-center text-[10px] font-bold text-ink/50">
                          {author[0] ?? '?'}
                        </div>
                        <div className="flex flex-col">
                          <span className="text-[11px] font-bold text-ink/70">{author}</span>
        ...
        <div className="glass-panel rounded-2xl p-3 border border-[var(--erp-border)] space-y-3">
          ...
          <button
            onClick={handleSave}
            className="w-full flex items-center justify-center gap-2 py-2 rounded-xl bg-amber-500/15 hover:bg-amber-500/25 border border-amber-500/25 text-amber-300 font-bold text-xs transition-all"
          >
              ...
              {(typeof job.accessories === 'string' ? job.accessories.split(',') : []).map(
                (key) => {
                  ...
                  return (
                    <span
                      key={key}
                      className="px-2.5 py-1 rounded-xl text-xs font-bold bg-amber-500/15 border border-amber-500/25 text-amber-300"
                    >
                      {acc?.emoji ? `${acc.emoji} ` : '✓ '}
                      {acc?.label ?? trimmed}
                    </span>
                  ...
                  return (
                    <div key={h.id} className="flex gap-3 items-start">
                      <div className="flex flex-col items-center pt-0.5">
                        <div className="w-2 h-2 rounded-full bg-ink/20 shrink-0" />
