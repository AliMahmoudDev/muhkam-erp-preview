...
function ServiceForm({
  ...
  function onServiceTypeChange(id: string) {
    ...
    onChange({
      ...value,
      service_type_id: numId,
      service_type_name: st?.name_ar ?? value.service_type_name,
    });
  }
    ...
    <div className="rounded-xl border border-emerald-500/25 bg-emerald-500/5 p-3 space-y-2">
            ...
            <>
              ...
              {!value.technician_id && (
                <input
                  ...
                  onChange={(e) =>
                    onChange({ ...value, technician_name: e.target.value, technician_id: null })
                  }
      ...
      <div className="flex items-center justify-end gap-2 pt-0.5">
        ...
        <button
          onClick={onSave}
          disabled={
            (!value.service_type_id && !value.service_type_name.trim()) ||
            !value.technician_name.trim() ||
            saving
          }
...
export function JobServicesSection({ jobId, users, locked = false }: Props) {
  ...
  const serviceTypes: ServiceType[] = (safeArray(serviceTypesRaw) as ServiceType[]).filter(
    (s) => s.is_active
  );
  ...
  const createMut = useMutation({
    ...
    onSuccess: async ({ service, pendingPart }) => {
      ...
      if (pendingPart) {
        try {
          await addAndLinkPart(service.id, pendingPart);
          partLinked = true;
        } catch {
          /* non-fatal */
        }
  ...
  const updateMut = useMutation({
    ...
    onSuccess: async ({ serviceId, pendingPart }) => {
      ...
      if (pendingPart) {
        try {
          await addAndLinkPart(serviceId, pendingPart);
          partLinked = true;
        } catch {
          /* non-fatal */
        }
  ...
  const deleteMut = useMutation({
    mutationFn: (id: number) =>
      authFetch(api(`/api/repair-jobs/${jobId}/services/${id}`), { method: 'DELETE' }).then(
        async (r) => {
          if (!r.ok) throw new Error((await r.json()).error);
        }
      ),
    ...
    <div className="glass-panel rounded-2xl border border-emerald-500/15 bg-emerald-500/[0.03] overflow-hidden">
      ...
      >
        ...
        <div className="flex items-center gap-2">
          ...
          <ChevronRight
            className={`w-4 h-4 text-emerald-400/50 transition-transform duration-200 ${open ? '-rotate-90' : 'rotate-90'}`}
          />
        </div>
      </button>
                      ...
                      <div className="flex items-start gap-2 px-3 py-2.5 group">
                        <div className="flex-1 min-w-0 space-y-0.5">
                          <div className="flex items-center gap-2 flex-wrap">
                            <span className="text-[11px] font-bold text-ink/90">
                              {sv.service_type_name_snapshot}
                            </span>
                            <span
                              className={`text-[9px] px-1.5 py-0.5 rounded-full border font-medium ${STATUS_CONFIG[sv.status].color}`}
                            >
                          ...
                          {sv.linked_parts.length > 0 && (
                            <div className="flex flex-wrap gap-1 pt-0.5">
                              {sv.linked_parts.map((lp) => {
                                const cost =
                                  (Number(lp.unit_price) || 0) *
                                  (Number(lp.quantity_allocated) || 1);
                                return (
                                  <span
                                    key={lp.id}
                        ...
                        {!locked && !sv.commission_locked && (
                          <div className="flex items-center gap-1 shrink-0 opacity-0 group-hover:opacity-100 transition-opacity">
                            ...
                            <button
                              onClick={() => {
                                if (confirm('حذف بند الخدمة؟')) deleteMut.mutate(sv.id);
                              }}
