...
function NewWarrantyForm({ onClose }: { onClose: () => void }) {
  ...
  const [form, setForm] = useState({
    ...
    serial_number: "", device_model: "", warranty_months: "3",
    warranty_start: new Date().toISOString().split("T")[0], notes: "",
  });
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm p-4" dir="rtl">
      <div className="glass-panel rounded-3xl p-6 w-full max-w-lg border border-white/10 space-y-4">
        ...
        <div className="flex gap-3">
          <button onClick={() => mutation.mutate()} disabled={mutation.isPending || !form.product_name || !form.warranty_start}
            className="flex-1 btn-primary py-2.5 text-sm font-bold">
            {mutation.isPending ? "جاري الحفظ..." : "حفظ السجل"}
          </button>
          <button onClick={onClose} className="px-5 btn-secondary py-2.5 text-sm">إلغاء</button>
        </div>
      </div>
    </div>
  );
...
function WarrantyBadge({ record }: { record: WarrantyRecord }) {
  ...
  if (record.status !== "active" || record.warranty_end < today)
    return <span className="text-xs px-2 py-0.5 rounded-full bg-red-500/20 text-red-400 font-medium">منتهي</span>;
  if (record.days_remaining <= 30)
    return <span className="text-xs px-2 py-0.5 rounded-full bg-amber-500/20 text-amber-400 font-medium">ينتهي قريباً</span>;
  return <span className="text-xs px-2 py-0.5 rounded-full bg-emerald-500/20 text-emerald-400 font-medium">نشط</span>;
}
...
export default function Warranty({ embedded = false }: { embedded?: boolean }) {
  ...
  const filtered = records.filter(r => {
    ...
    const matchFilter =
      filter === "all" ? true :
      filter === "active" ? (r.status === "active" && r.warranty_end >= today) :
      filter === "expiring" ? (r.status === "active" && r.warranty_end >= today && r.warranty_end <= soonStr) :
      (r.status !== "active" || r.warranty_end < today);
    return matchSearch && matchFilter;
  });

  const STAT_CARDS = [
    { label: "إجمالي سجلات الضمان", value: stats?.total ?? 0, icon: Shield, color: "text-amber-400" },
    { label: "ضمانات نشطة", value: stats?.active ?? 0, icon: ShieldCheck, color: "text-emerald-400" },
    { label: "تنتهي خلال 30 يوم", value: stats?.expiring_soon ?? 0, icon: Clock, color: "text-orange-400" },
  ...
  const sendWhatsApp = (r: WarrantyRecord) => {
    ...
    const phoneFormatted = phone.startsWith("0") ? "2" + phone : phone.startsWith("2") ? phone : phone ? "2" + phone : "";
    const lines = [
            ...
            <tbody>
              {isLoading ? (
                ...
                <tr key={r.id} className="border-b border-white/5 erp-table-row">
                  ...
                  <td className="p-3"><WarrantyBadge record={r} /></td>
                  <td className="p-3">
                    <div className="flex items-center gap-1">
                      <button onClick={() => sendWhatsApp(r)} title="إرسال تذكير واتساب"
                      ...
                      <button onClick={() => deleteMutation.mutate(r.id)} title="حذف"
