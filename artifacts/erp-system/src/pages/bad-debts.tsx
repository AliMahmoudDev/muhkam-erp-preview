...
export default function BadDebts({ embedded = false }: { embedded?: boolean } = {}) {
  ...
  const updateM = useMutation({
    mutationFn: ({ id, status }: { id: number; status: string }) =>
      authFetch(api(`/api/bad-debts/${id}`), {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
            ...
            <tbody>
              {items.map((d) => (
                <tr key={d.id} className="border-b border-line text-ink/80">
                  ...
                  <td className="px-3 py-2 font-black text-red-400">
                  ...
                  <td className="px-3 py-2 text-xs text-ink/40">
                    {new Date(d.created_at).toLocaleDateString('ar-EG')}
                  </td>
                  <td className="px-3 py-2">
                    <span
                      className={`px-2 py-0.5 rounded-full text-[10px] font-bold border ${statusColor[d.status]}`}
                    >
                  ...
                  <td className="px-3 py-2">
                    {d.status === 'open' && (
                      <div className="flex gap-1">
                        <button
                          onClick={() => updateM.mutate({ id: d.id, status: 'recovered' })}
                          className="text-[10px] px-2 py-0.5 rounded-lg bg-emerald-500/20 border border-emerald-500/30 text-emerald-300"
                        >
                          استرداد
                        </button>
                        <button
                          onClick={() => updateM.mutate({ id: d.id, status: 'written_off' })}
                          className="text-[10px] px-2 py-0.5 rounded-lg bg-red-500/20 border border-red-500/30 text-red-300"
                        >
                          شطب
                        </button>
                      </div>
          ...
          >
            <div className="flex items-center justify-between">
              <h3 className="font-black text-ink flex items-center gap-2">
                <AlertTriangle className="w-4 h-4 text-red-400" /> إضافة دين
              </h3>
            ...
            <button
              onClick={() => {
                if (!newName.trim() || !newAmount) {
                  toast({ title: 'الاسم والمبلغ مطلوبان', variant: 'destructive' });
                  return;
                }
                addM.mutate({
              ...
              className="w-full py-2 rounded-xl bg-red-500/20 border border-red-500/30 text-red-300 font-bold text-sm"
            >
              حفظ
            </button>
          </div>
